...
- **Two rendering paths with distinct roles (updated 21 Apr 2026).** The React SPA is the interactive experience in `bristlenose serve`; the Jinja2 static renderer at `bristlenose/stages/s12_render/` is a first-class product for offline HTML — it's what `bristlenose run` writes to disk and what `bristlenose render` regenerates from cached JSON. CLI users (pip/Homebrew/Snap) rely on the static HTML report as their deliverable. Rules: (1) New features and design changes target the React SPA; the static render ships correct data but doesn't receive design updates. `render_html()` emits a `DeprecationWarning` **to warn React-migration-era code paths, not to deprecate the CLI offline HTML product.** (2) CSS in `bristlenose/theme/` is shared — changes apply to both paths automatically. (3) Vanilla JS in `bristlenose/theme/js/` is frozen — data-integrity fixes only. (4) When a section becomes a React island, its Jinja2 equivalent becomes dead code for React-served mode only; the static render keeps it. (5) **Serve mode never falls back to static render.** If the React SPA is missing from the bundle, `_mount_prod_report` returns 500 with a clear error page (fail-loud) rather than silently serving the static HTML — that fallback masked BUG-3 in the C3 smoke test. (6) The old monolithic `render_html.py` was refactored into `bristlenose/stages/s12_render/` package (Mar 2026): `theme_assets.py`, `html_helpers.py`, `quote_format.py`, `sentiment.py`, `dashboard.py`, `transcript_pages.py`, `standalone_pages.py`, `report.py`
