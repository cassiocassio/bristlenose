...
- **New keys must go in all 8 locale files** — en, es, fr, de, ko, ja, cs, it. Every `t("key")` call needs an entry in each. If using `dt()`, the desktop override must also go in all 8 `desktop.json` files
- **Adding a whole new language = 9 JSON files + exactly 6 registration sites.** Create `bristlenose/locales/<code>/` with all 9 namespace files (`common, cli, desktop, doctor, enums, pipeline, preflight, server, settings`), then register the code in: `SUPPORTED_LOCALES` in [frontend/src/i18n/index.ts](frontend/src/i18n/index.ts) **and** [bristlenose/i18n.py](bristlenose/i18n.py); the `expected` set in [bristlenose/doctor.py](bristlenose/doctor.py) (`Bundle: locales` check); the `LOCALE_LABELS` map in **both** [SettingsPanel.tsx](frontend/src/islands/SettingsPanel.tsx) and [SettingsModal.tsx](frontend/src/components/SettingsModal.tsx) (native name, e.g. `it: "Italiano"`); and the mock list in [LocaleStore.test.ts](frontend/src/i18n/LocaleStore.test.ts). The runtime loader needs NO edit — it's a Vite dynamic-import glob (`import(\`.../locales/${locale}/${ns}.json\`)`), so the files are picked up automatically. Lazy locale chunks are **excluded from the bundle-size budget** (the `size-limit` glob negates `common-*`/`settings-*`/`desktop-*`/`enums-*`/etc.), so a new language is size-neutral. Register Apple-HIG + QDA terms for the language in `bristlenose/locales/glossary.csv` so Weblate shows translators the agreed taxonomy. Translate in the language's **Apple-HIG register** (imperative for buttons/commands, impersonal for body text — match the platform, don't invent a register)
