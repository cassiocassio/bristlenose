...
- **The static render is a sealed byproduct, not a fallback or a user-facing surface (A3, 12 May 2026).** `bristlenose/stages/s12_render/` still writes HTML to disk on every `bristlenose run`, but the path is never surfaced. The `bristlenose render` CLI command was removed (now a hidden catch-and-interpret stub that redirects to `run`/`serve`); the `--static` flag on `run` was deleted. The legitimate offline-share product is `bristlenose serve` → browse → **Export HTML** in toolbar (which embeds the React bundle, doesn't touch `s12_render/`). Serve mode does NOT fall back to the static render when the React bundle is missing — it returns 500 with a "Build incomplete" page (changed in C3 P1). Future direction for `--static` is a *markdown* deliverable (post-100-days, see `docs/design-cli-improvements.md` §Future direction). If you find code or docs that treat the static render as a "first-class CLI product" or "fallback in serve mode," that framing is stale
...
- `bristlenose render` was removed in A3 (12 May 2026) — invoking it now hits a hidden catch-and-interpret stub that redirects to `run` / `serve`. Stage 12 still writes the static HTML to disk as a sealed byproduct but its path is never surfaced.
...
## Current status (v0.15.7, May 2026)

Core pipeline published to PyPI + Homebrew + Snap. v0.15.7 is a **release-pipeline fix** that unblocks v0.15.4–6 reaching PyPI + Homebrew (three CI failures had silently stacked up since 10 May 2026, leaving PyPI on 0.15.3 while source was on 0.15.6). No user-facing source change; on upgrade users pick up the entire v0.15.4–v0.15.6 changelog at once. Fixes: ffmpeg-preflight bypass in `tests/test_pipeline_abandon.py`; stub SPA bundle (`_VITE_INDEX_HTML` + `_STATIC_DIR` patch pattern) in `tests/test_serve_auth.py::test_spa_html_sets_cookie`; CI `node-version: "20"` → `"24"` in `.github/workflows/{ci,release}.yml` (the bundle-size "overage" of 0.75 kB was a Node 20 zlib 1.2.13 vs Node 24 zlib 1.3.1 measurement artefact, not a real regression). v0.15.6 lands the **server-rendered status page** for runs the SPA can't render — `bristlenose serve`'s `/report/*` catch-all now serves a server-rendered page (no React mount) when the project has no completed run, or the latest run failed or was cancelled. The SPA's invariant becomes: it only mounts when there's data to render. Three states surfaced — no-run-yet (CLI vs desktop copy via `BRISTLENOSE_PLATFORM`), failed (with cause + log tail in a `<details>` block sourced from `pipeline-events.jsonl`), cancelled. Reuses the five-kind `MessageKind` taxonomy — no sixth kind invented. Styling lives at `bristlenose/theme/templates/status-page.css` and uses the design system's tokens exclusively (sibling to `report.css` / `transcript.css` / `print.css` / `export.css`). Event-watcher startup seed widened to seed `failed` and `cancelled` termini, not just `completed`. English-only in v1; locale fills batched into the next translation pass. v0.15.5 landed the **first-run preflight block** — Whisper / ffmpeg / API-key-billing / spaCy lazy-fetch preflights all fire between ingest and audio extraction, ending with a "No more questions. ~X min to your report. Ctrl+C anytime." line after which no prompt fires for the rest of the run. New `--no-fetch` flag (on `run` / `transcribe` / `analyze`) plus `bristlenose doctor --fetch` for offline pre-warm; `BRISTLENOSE_SKIP_PREFLIGHT=1` defence-in-depth env escape; `state.json` (mode 0600) at `~/Library/Application Support/Bristlenose/` caching API-key validation for 24h. New `bristlenose/preflight/` package, `bristlenose/llm/billing_hints.py` registry, and a `preflight.*` i18n namespace in all six locales (English source; others mirror en pending native review). Documentation pass: design doc has a post-implementation Status table, SECURITY.md gained a preflight bullet + "What Bristlenose writes to your machine" appendix + HF Hub LFS-hash integrity caveat, and the man page (now `mandoc -Tlint` clean) documents the new flags + `state.json` under FILES + exit code 2. v0.15.4 lands **SPA auto-refetch on pipeline completion** (`GET /api/projects/{id}/last-run` + `LastRunStore` with 3 s polling; browser SPA refreshes within ~3 s of completion instead of going silent), **SPA trust-UX layer** (manual refresh button in NavBar — serve-mode only, hidden on desktop; refetch overlay on Quotes / Sessions / Dashboard; post-zero-quotes empty state copy), **sandbox-safe Export via `WKDownload` + `NSSavePanel`** (previous synthetic `<a download>` click was silently dropped under App Sandbox), **structured failure category surfaced in the desktop failure pill** (`EventLogReader` exposes the 10-category `Cause` from `pipeline-events.jsonl`; pill renders category-appropriate glyph + label), **pipeline structured per-stage failure summaries + abandon path** (`PipelineSummary` rollup on every terminus event; `PipelineAbandonedError` instead of silent fake-empty report; two new `Cause` categories `MISSING_INPUT` + `MISSING_BINARY`), three sidecar sandbox-compliance fixes (`mlx_whisper` bare-`ffmpeg` shellout, `/bin/ps`→`proc_pidinfo`, Starlette mmap→in-memory React bundle), CLI glyphs/colours sourced from `MessageKind`, and a desktop i18n sweep + locale delegated to System Settings. v0.15.3 landed **desktop provider-switching hardened end-to-end** (Change-provider flow activates across Keychain + sidecar env + UI; Ollama choice persisted before consent log; sidecar key injection scoped to active provider; LLM Settings lazy-loads Keychain per row), **prompt-injection sentinel-tag boundary (Phase A)** as defence-in-depth against injection hidden in transcripts, **sidecar bundle trim (S1+S2+S3)** — `mlx_whisper.torch_whisper` excluded, dedicated build venv, `torch` + `onnxruntime` evicted, mlx-whisper assets via `collect_all` for a significantly smaller sidecar, `doctor` recognising mlx-whisper as a complete backend on Apple Silicon, desktop log-tail trust on sidecar exit-1-after-success, and sandbox-on Debug locale-loading + resizable-window fix on macOS Tahoe. v0.15.2 landed **bundled FFmpeg/ffprobe discovery** (`bristlenose/utils/bundled_binary.py`) so the sandboxed desktop sidecar can find ship-with-app binaries, **`bristlenose/server/lifecycle.py`** factoring out serve-mode lifecycle (parent-death detection, sandbox-friendly bind(0)), **PipelineRunner `SidecarMode.resolve` migration** (PR #96), and **TLS→certifi redirect under sandbox** (PR #97). Frontend size budget re-baselined to live-SPA gzipped. v0.15.1 landed **Sprint 2 Track C C2–C5 alpha-readiness** (sandbox-safe Keychain, libproc zombie cleanup, privacy manifests, per-binary signing, supply-chain provenance) and **Track B Branch 1 first-run experience** (`BootView` + `WelcomeView` + Beat 3 API-key validation + Beat 3b Ollama setup sheet), plus `bristlenose serve` fall-back to bundled theme CSS, `doctor --self-test` for sidecar bundle integrity, fail-loud on missing React bundle, and Japanese locale completion. v0.15.0 landed **pipeline resilience Phase 1f / 4a-pre** — append-only `pipeline-events.jsonl` with structured `Cause` (10 categories), honest cost estimates, stranded-run reconciliation, desktop `EventLogReader`. Earlier product feature: **Video clip extraction** (FFmpeg stream-copy, async job, `ClipBackend` Protocol for future AVFoundation), 6 languages (en, es, fr, de, ko, ja). Inspector panel, codebook sidebar, finding flags, Settings + Help modals with shared ModalNav. Static render formally deprecated. ~2328 Python tests, ~1265 Vitest tests, 90 Swift tests. React migration complete (Steps 1–10). See `CHANGELOG.md` and git log for full history.
