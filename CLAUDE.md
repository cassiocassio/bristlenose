...
- **Pipeline resume (Phase 1c)**: `run()` loads an existing manifest on startup via `_prev_manifest = load_manifest(output_dir)`. Stages 8–11 check `_is_stage_cached(_prev_manifest, STAGE_*)` + intermediate JSON file existence — if both true, data is loaded from disk and `(cached)` is printed. Stages 1–7 always re-run (fast, no intermediate JSON). Stage 12 (render) always re-runs. Cache requires `write_intermediate=True` (the default). Only `COMPLETE` status triggers cache; `RUNNING`/`PARTIAL`/`FAILED` force re-computation
