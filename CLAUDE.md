...
Serve mode: `bristlenose serve <folder>` — FastAPI server + SQLite + React SPA. Auto-renders, serves a React Router app (pathname routes: `/report/`, `/report/quotes/`, `/report/sessions/:id`, etc.) over HTTP. `app.py` replaces `<!-- bn-app -->` markers in the static HTML with `<div id="bn-app-root">`; React Router handles all client-side navigation. Individual island markers (`<!-- bn-X -->`) exist in the baked HTML but are not processed — the outer `bn-app` substitution replaces them all. Vanilla JS modules are stripped from served HTML by `_strip_vanilla_js()` (Step 8); only `window.*` globals survive for React. 22-table SQLAlchemy domain schema, 6 data sync endpoints, sessions/quotes/dashboard/codebook APIs. `--dev` enables renderer overlay, live JS reload, SQLAdmin browser. See `bristlenose/server/CLAUDE.md` for architecture.
Footer feedback in serve mode is now React-native (no legacy `feedback.js`): `Footer` reads `/api/health` config (`links.github_issues_url`, `feedback.enabled`, `feedback.url`), `FeedbackModal` handles sentiment + message capture, submits to configured endpoint, and falls back to clipboard copy + toast on failure/non-HTTP.
...
Frontend: `frontend/` — Vite + React + TypeScript + React Router. 35 components in `frontend/src/components/`, 14 islands in `frontend/src/islands/`, 9 page wrappers in `frontend/src/pages/`, 1 layout in `frontend/src/layouts/`. In serve mode, a single `RouterProvider` root replaces the 11 separate `createRoot()` calls from the island era. Legacy island mode (individual mount points) remains as fallback for the static render path. 758 Vitest tests across 56 test files. `npm run dev` proxies to FastAPI; `npm run build` outputs to `frontend/dist/`. See `docs/design-react-component-library.md` for build sequence.

Export strategy: The served React app is the canonical experience. Standalone export will be implemented as a DOM snapshot from serve mode (embed state as JSON, inline CSS, download as self-contained HTML). The static render package (`bristlenose/stages/render/`) is not the export path — it's a deprecated legacy offline fallback.
...
Report JavaScript — 17 modules in `bristlenose/theme/js/`, concatenated in dependency order into a single `<script>` block by `bristlenose/stages/render/theme_assets.py` (`_JS_FILES`). Transcript pages and codebook page use separate JS lists. See `bristlenose/theme/js/MODULES.md` for per-module API docs.
...
- `load_transcripts_from_dir()` in `pipeline.py` is a public function (no underscore) — used both internally by the pipeline and by `bristlenose/stages/render/transcript_pages.py` for transcript pages
...
- **Homebrew formula uses `post_install` pip to avoid dylib relinking failures.** Pre-built wheels (`av`, `cryptography`) ship `.dylib` files with short Mach-O header padding that can't accommodate Homebrew's long Cellar paths — `install_name_tool` fails with `MachO::HeaderPadError`. The formula creates a venv + bash wrapper script during `install` (real file, not symlink — required for Homebrew's link phase), then runs `pip install` in `post_install` which executes *after* dylib relinking. This is a known Homebrew gap with no clean upstream fix (AWS SAM CLI, DVC, Selenium all hit it). The "official" alternative (`virtualenv_install_with_resources` with 100+ explicit resource blocks) is viable for homebrew-core but massively over-engineered for a third-party tap. See `docs/design-homebrew-packaging.md` for the full analysis and alternatives considered
...
- **`BRISTLENOSE_FAKE_THUMBNAILS=1`** env var — shows video thumbnail placeholders in the session table for all sessions (even VTT-only). Layout testing only. Defined as `_FAKE_THUMBNAILS` in `bristlenose/stages/render/dashboard.py`. The shipped version retains real `video_map` logic
...
- **Health payload shape changed (Mar 2026)** — `GET /api/health` now includes footer config: `links.github_issues_url` and `feedback.{enabled,url}` in addition to `status` and `version`. Reuse the shared `HealthResponse` type (`frontend/src/utils/health.ts`) instead of ad-hoc inline interfaces to avoid shape drift across `Footer`, `FeedbackModal`, About panel, and export embedded data
...
- **`<!-- bn-app -->` markers** — wrap nav bar + all tab panels in `bristlenose/stages/render/report.py`. The IIFE script block (including `BRISTLENOSE_ANALYSIS` JSON) is outside the markers and survives the serve-time substitution. `_REACT_APP_MOUNT` in `app.py` replaces these markers with `<div id="bn-app-root">`
- **IIFE-scoped globals need `window.` for React** — `bristlenose/stages/render/report.py` wraps all JS in `(function() { ... })();`. Variables declared with `var` inside the IIFE are invisible to React (which runs in a separate ES module context). Any global that React needs must be explicitly assigned to `window.` inside the IIFE. Currently exposed: `window.BRISTLENOSE_VIDEO_MAP`, `window.BRISTLENOSE_PLAYER_URL`, `window.BRISTLENOSE_ANALYSIS`. If adding new globals for React, add `window.X = X;` after the `var` declaration — the `var` must stay for vanilla JS code in the same IIFE
- **Dual-path CSS selectors for sticky scroll offsets** — any CSS rule that targets elements under a specific wrapper must have selectors for both render paths. Static render wraps quote content in `.bn-tab-panel`; the React SPA wraps it in `div.center` (from `SidebarLayout`). If a `scroll-margin-top` or similar rule only targets one wrapper, it silently fails in the other path. Pattern: always pair `.bn-tab-panel:has(.toolbar)` with `.center:has(.toolbar)` (or equivalent). See `toolbar.css` for the canonical example. When adding new sticky elements that occlude content, add `scroll-margin-top` rules for both paths
- **Session route priority in `app.py`** — `/report/sessions/{filename}` handles both transcript HTML files (`transcript_*.html`) and React Router SPA routes (e.g. `/report/sessions/s1`). Non-transcript filenames serve the SPA HTML instead of raising 404, so React Router can handle client-side routing. This applies to both dev and prod serve functions
- **Video player: two data paths** — video playback has two independent data flows: (1) **Static render path** — `html_helpers._build_video_map()` reads `InputSession.files` (full absolute paths) and writes `window.BRISTLENOSE_VIDEO_MAP` with correct `file://` URIs into the IIFE. `player.js` reads this via event delegation. (2) **Serve/SPA path** — the importer reads transcript headers (`# Source: interviews/show and tell 40.mov`) which store **relative paths including subdirectory** (written by `merge_transcript.py` using `fpath.relative_to(input_dir)`), stores them in the DB, and the video-map API (`/api/projects/{id}/video-map`) converts DB paths to `/media/` URIs via `_file_to_media_uri()`. `PlayerContext.tsx` fetches this API. **Encoding rule**: `_file_to_media_uri()` returns **unencoded** paths (e.g. `/media/interviews/show and tell 40.mov`). JavaScript's `encodeURIComponent()` in `PlayerContext.tsx` handles encoding once when building the player URL hash. Never add `urllib.parse.quote()` in Python — that causes double-encoding (`%20` → `%2520`). The importer's `_import_source_files()` still has a one-level subdirectory scan as fallback for old transcript files that stored only the filename. In SPA mode, `player.js` bails out (`if (document.getElementById('bn-app-root')) return`) — only `PlayerContext` handles playback. `SessionsTable.tsx` must have an explicit `onClick` calling `playerCtx.seekTo()` because vanilla event delegation is inert
- **Glow index keying: session ID vs speaker code** — transcript segments have `data-participant` set to the speaker code (`m1`, `p1`), but the popout player sends `pid=sessionId` (`s1`) in `bristlenose-timeupdate` messages (because `TimecodeLink` passes `data.session_id` to `seekTo`). `buildGlowIndex` in `PlayerContext.tsx` extracts the session ID from the URL pathname (`/report/sessions/:sessionId`) and keys all transcript segment entries under that ID. Without this, the glow index would be keyed by speaker code and no entries would match the player's session-ID-based lookup. The vanilla `player.js` has the same speaker-code keying but is inert in SPA mode (`bn-app-root` guard)
...
- **Word-level timing data** — Whisper captures per-word timestamps (`Word` model: `text`, `start_time`, `end_time`, `confidence`). Both MLX and faster-whisper backends populate these. The data flows through the pipeline and is persisted in `session_segments.json`. The serve-mode importer (`_enrich_words_from_intermediate()` in `importer.py`) reads this JSON and populates `TranscriptSegment.words_json` — a TEXT column storing compact JSON (`[{"t":"word","s":0.5,"e":0.8},...]`). The transcript API (`/api/projects/{id}/transcripts/{sid}`) exposes this as `words: [{text, start, end}, ...] | null` on each segment. VTT/SRT-only sessions have `words: null` (no Whisper run). Schema migration in `db.py` adds the column to existing databases. **Frontend consumption not yet implemented** — the API serves the data but `TranscriptPage.tsx` doesn't render word spans yet
- **Playwright E2E tests** (`e2e/`) — 3 test layers (console errors, link crawler, network assertions) running against the smoke-test fixture on Chromium + WebKit. `playwright.config.ts` auto-starts `bristlenose serve` via `webServer` — locally uses `.venv/bin/bristlenose` (resolved path), in CI uses bare `bristlenose` (on PATH). Port 8150, overridable via `BN_E2E_PORT`. The link crawler skips `<a role="button">` elements (Footer's "Feedback" and "? for Help" are intentionally href-less modal triggers). Known bugs (duplicate React keys, dead analysis links) are `test.fixme()` stubs that need a richer fixture to reproduce. The smoke-test fixture has only 1 session — enough for route navigation but not for multi-session edge cases. See `docs/design-playwright-testing.md` for output options, CI integration, and future layers
- **Sidebar layout & responsive playground** — 6-column CSS grid (`SidebarLayout.tsx`) with TOC overlay (hover intent + direction-aware close via `useTocOverlay.ts`), drag-to-resize (`useDragResize.ts`), and minimap slot. Dev-only playground (`ResponsivePlayground.tsx`, `PlaygroundStore.ts`) injects CSS token overrides via `<style id="bn-playground-overrides">`. `PlaygroundStore` uses `useSyncExternalStore` + `sessionStorage` (not localStorage — resets on tab close). Playground is code-split — only loaded when `--dev` flag is set (`AppLayout.tsx` dynamic import). The `PlaygroundFab` component renders a floating ◆ button via `createPortal` to `document.body`. `hoverDelay` and `leaveGrace` are JS-only values passed as props through `SidebarLayout` → `useTocOverlay`; the remaining 5 sidebar layout values are CSS custom properties. Module-level stores (`SidebarStore`, `PlaygroundStore`) persist across tests — always call `resetSidebarStore()` / `resetPlaygroundStore()` in `beforeEach`
...
- **Analysis module**: `bristlenose/analysis/` uses plain dataclasses (not Pydantic) — ephemeral computation, never persisted. `_compute_analysis()` in `pipeline.py` returns `object | None` (typed as `object` to avoid import at module level, lazy import inside the function). `_serialize_analysis()` in `bristlenose/stages/render/standalone_pages.py` uses `# type: ignore[attr-defined]` for the same reason. Cell key format `"label|sentiment"` — pipe in labels is a known limitation (documented in tests, not guarded)
...
## Current status (v0.13.2, Mar 2026)

Core pipeline complete and published to PyPI + Homebrew. Snap packaging implemented and tested locally (arm64); CI builds amd64 on every push. Latest: **Render refactor** — `render_html.py` (2,903 lines) broken into `bristlenose/stages/render/` package with 8 submodules (`theme_assets`, `html_helpers`, `quote_format`, `sentiment`, `dashboard`, `transcript_pages`, `standalone_pages`, `report`). Static render path formally deprecated (`DeprecationWarning`). ~99 doc references updated across 34 files. 1872 Python tests, 891 Vitest tests. Prior: **Codebook-aware tag autocomplete** — IDE-style grouped suggestions with colour pills and section headers. Sentiment tags unified into proper codebook framework (`sentiment.yaml`). Tag provenance (`"human"` / `"autocode"` source column), case-insensitive dedup guard, hidden-group UX (eye-closed icon, auto-unhide on accept). Quick-repeat tag shortcut (`r`). Context expansion crash fix. Doctor offers interactive MLX install on Apple Silicon. Sidebar accessibility (focus, ARIA, reduced-motion). Dashboard Cmd+click fix. 1886 Python tests, 891 Vitest tests. Prior: **Word-level transcript highlighting** — karaoke-style word sync during video playback. Prior: **Dual sidebar for Quotes tab.** Left sidebar: TOC with scroll-spy (sections + themes). Right sidebar: tag filter with codebook tree, eye toggles for badge hiding (localStorage persistence). 5-column CSS grid, drag-to-resize with snap-close, keyboard shortcuts (`[`, `]`, `\`, `⌘.`). New components: `SidebarLayout`, `TocSidebar`, `TagSidebar`, `TagGroupCard`, `TagRow`, `EyeToggle`. New hooks: `useDragResize`, `useScrollSpy`. Module-level `SidebarStore`. Backend: tag-group-with-quotes API, admin panel. Frontend CI added (ESLint, tsc, Vitest in GitHub Actions). Prior: **React migration complete (Steps 1–10).** Step 10: Export — self-contained HTML download from serve mode with optional anonymisation, blob-URL'd JS chunks, hash router for `file://`. About panel redesigned with sidebar layout (5 sections). Configuration reference panel in Settings. Morville Honeycomb codebook added. Prior: **Steps 5–9 merged** — serve mode is a full React SPA. Step 5: React Router with pathname-based routing (`AppLayout`, `NavBar`, 8 page wrappers, backward-compat shims). Step 6: `PlayerContext` (popout window lifecycle, seekTo, glow sync). Step 7: `FocusContext` + `useKeyboardShortcuts` (j/k navigation, multi-select, bulk actions, `?` help modal). Step 8: All 26 vanilla JS modules stripped from serve path (`_strip_vanilla_js()`), frozen for static render only. Step 9: Full React app shell — `Header`, `Footer`, `HelpModal`; serve mode serves Vite-built SPA directly, no more `_transform_report_html()` marker substitution. Also: video player links on sessions page + dashboard, importer source-path subdirectory fix, speaker name sizing, word-level timing plumbing. Prior: **Toolbar React island** (Step 4). Prior: **Split speaker badges** — two-tone pill (code left, name right) across all surfaces, settings toggle (code+name / code-only), em-dash removed from attribution. Always-on sticky transcript header with session selector. Serve-mode session routing: inline Jinja2 transcripts stripped, session links navigate to React transcript pages. Prior: **Names architecture** — YAML as canonical source of truth, DB as materialized view, write-through from serve mode browser edits to `people.yaml`. Prior: **Signal elaboration** — LLM-generated interpretive names, pattern classification, sparkbar chart. Prior: **Video thumbnails** — auto-extracted keyframes. **AutoCode frontend** — ✦ button, progress toast, threshold review dialog, proposed badges. **Analysis page React island** — per-codebook grids, signal badges, drill-down. **Pipeline crash recovery** (Phase 1a–1d-ext) — manifest-based resume, stage-level caching, per-session tracking. Prior: AutoCode backend, codebook import, desktop app, serve mode, interactive dashboard, sessions table, appearance toggle, user journeys, tab navigation, Gemini provider, transcript annotations, hidden quotes + codebook, Azure OpenAI, Ollama, transcript coverage, multi-participant sessions. See git log for full history.

**Next up:** Export polish (inline logo, footer spacing, hash-aware navigation links); help modal styling polish; responsive quote grid (CSS-only Phase 1); AutoCode threshold review QA on real data; pipeline resilience Phase 1e (status report); WKWebView integration; cross-session moderator linking; snap store publishing. See `TODO.md` for full task list.
