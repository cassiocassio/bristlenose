...
- **Stale `__pycache__` can serve old CSS after theme edits.** `bristlenose render` reads CSS files at runtime, but stale `.pyc` bytecode can interfere with the import chain. If theme CSS changes aren't appearing in the rendered output, run `find . -name __pycache__ -exec rm -rf {} +` before rendering. For daily dev, set `export PYTHONDONTWRITEBYTECACHE=1` in your shell profile to prevent `.pyc` creation entirely
