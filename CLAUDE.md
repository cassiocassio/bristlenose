...
**Internal TestFlight since 14 Jul 2026** (now the v0.25.x line — **0.25.3** is the shipping build; 0.25.0 reached the direct-download `.dmg` only and was superseded the same night by the export speaker-link fix, so the tag stays pinned at `94b13fb0` to match that image's manifest) — first build accepted by App Store Connect: **0.20.0 (2068)**, App-Sandbox + Hardened-Runtime + arm64-only, signed Apple Distribution.

**0.27.0 is the release in flight** (22 Aug 2026) — version files, CHANGELOG and README carry it, and it ships on Tier 1: TestFlight, `.dmg`, PyPI, GitHub Release, Homebrew, Snap edge. Nothing publishes until a maintainer approves the publish job — the `pypi` environment's required-reviewer hold stops it, and that approval is the act that lands the release. **`v0.26.0` was tagged and then abandoned, deliberately**: it pointed at `31932bc2` while 34 commits of real shipped code sat past it — the `Cause.reason` wire field, the transcribe-only silent-session accounting, the timecode unification, and eight refusal reasons across 21 locales — so the earlier note here that later commits were "docs-only, wheel byte-identical" was simply wrong. Rather than ship a version whose notes understated it, the tag was deleted and 0.27.0 cut from HEAD, carrying the whole fortnight plus that day. 0.26.0 reached no channel and is not in the changelog; **it is not a version that ever existed.** **It is a large release, not a locale release**, which is what this block said until 21 Aug: multiple windows (N studies at once, one sidecar each via `ServeFleet`; N windows on one study, each on its own lens), **cloud import** from Teams and Google Meet with a Settings ▸ Accounts pane (Zoom parked behind `BristlenoseFlags.cloudImportZoom`), the **MCP Agents projects register** with per-project exposure that survives a restart, **ingest refusals** (one bad file no longer aborts the batch; 27 formats, up from 16), **Re-analyse** working with a counted confirmation sheet, Welcome on launch, the two `.docx` transcript fixes (Teams turns, and Gemini's summary leaking into Meet transcripts — **affected studies need re-running**), and the **Catalan (`ca`) locale**, the first built glossary-first. `CHANGELOG.md` is the user-facing list; it was rewritten 21 Aug because the original six bullets described the first week of a fortnight's work.

**Owed before it ships:** the Catalan native pass (owned, a few weeks out — the release does **not** wait on it, since nine other locales already ship machine-seeded pending review); ~~the desktop language smoke~~ — **run mechanically and parked 22 Aug 2026** (i18n judged beta-shaped; `TODO.md` carries the findings and the re-runnable checks). It proved `choosePrompt` against AppKit's own corpus in all 22 locales, and surfaced two **never-enrolled-in-English** surfaces that ship on every channel and were green to every gate — the System Health window (`DoctorReportView.swift`, zero `i18n.t` sites) and Diagnostics ▸ Section 1 (`MenuCommands.swift`:134–149). **Adjudicated the same day and settled: English-only diagnostics is fine for TestFlight, and that whole surface is a trim/remove candidate for the Mac App Store build** — decide its fate before localising it. The one live residue is outside that call: `WelcomeHomeView.swift`:290 `Text("Setup →")`, hardcoded on the Welcome AI cell every locale sees. The Re-analyse sheet and the diagnostic popover seen on screen; and ~~the website privacy page~~ — **done and deployed 22 Aug.** The disclosure is live at **`bristlenose.app/privacy.html`**, which is now the single canonical privacy permalink: Apple, Microsoft Entra and Google Cloud all point at it (Entra's field had pointed into the docs tree and was corrected the same day), and `/terms.html` is its pair. `/docs/privacy.html` remains a *documentation* page — twelve sibling pages and the app's Welcome link it, so it must keep resolving — but it is no longer a second policy; change `content/privacy.html` in the website repo and nothing else. Rationale and the console-state table live in that repo's `README.md`. **It was never a publish blocker** — cloud import is macOS-app-only (`SECURITY.md` item 5), so the PyPI/Homebrew/Snap half the publish approval gates carries none of it, and Publisher Verification independently stops any cohort tester completing a sign-in (`docs/design-cloud-import.md`:608). **`AIConsentView` needs no change, decided 22 Aug** — cloud import is *ingress* (recordings come **to** the Mac; nothing is transmitted), while the consent dialog governs *egress* of transcript text to an LLM vendor. Orthogonal; the vendor-name overlap is coincidental. Apple agrees on both limbs — §4.8's "client for a specific third-party service" exception applies, and §5.1.2(i)'s explicit-permission requirement is about sharing data *out*. `currentVersion` stays at 2. Encoded in `.claude/agents/app-store-police.md`; the retracted claim is recorded at `docs/design-cloud-import.md`:1396. **Do not let the `associated-domains` entitlement ride this release** — committing it obliges regenerating the Mac App Store profile or the archive fails to sign, and it is for a parked feature. Sweep green 22 Aug at the tagged commit: **4057 pytest** (83 skipped, 22 xfailed), `ruff check .` clean, `check-locales.py` clean. The frontend number is deliberately not quoted here — the local run overlapped a concurrent session's uncommitted edits, so the tag's own CI run is the authoritative read.

**Recent releases, in one line each** — **0.25.0** the Sessions responsive grid and spatial arrow navigation; **0.24.0** Focus Mode and the single appearance seam (`AppAppearance.swift`); **0.23.0** the assistant surface end to end (`/mcp` on both channels, the `.mcpb` extension, Settings ▸ MCP Agents). Full entries in [CHANGELOG.md](CHANGELOG.md) — kept there rather than mirrored here, because a version history in two places is a version history wrong in one. React migration complete (Steps 1–10); bundled-sidecar desktop is the primary distribution path; CLI ships on PyPI + Homebrew + Snap. Static render is a sealed byproduct.
