...
- **QuotesStore is a module-level store, not React Context** — `frontend/src/contexts/QuotesContext.tsx` uses a plain module-level state object + `useSyncExternalStore` (React 18). This is intentional: `QuoteSections` and `QuoteThemes` mount as separate `createRoot()` calls (separate DOM locations), so React Context cannot be shared between them. The store is the single source of truth for quote mutations (star, hide, edit, tag, badge, proposed tags). `initFromQuotes()` populates from API response; `resetStore()` clears for re-fetch. Module-level state persists across tests — always call `resetStore()` in `beforeEach`. Step 4 will add `viewMode`/`searchQuery`/`tagFilter` (placeholders defined but not populated yet)
