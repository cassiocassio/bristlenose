...
**0.26.0 is bumped but NOT released** (14 Aug 2026) — version files, CHANGELOG and README carry it; no tag, no push, nothing approved. It adds the **Catalan (`ca`) locale**, the first built reviewer-led: the glossary was ratified by native reviewers *before* seeding, inverting the usual seed-then-correct order. 1,247 keys, all 10 registration sites, `check-locales.py` clean (the only locale with zero warnings). **Full sweep green** — 3925 pytest, 402 locale-key tests, ruff, Node-24 frontend build, `xcodebuild` build + `I18nTests` (18 tests, including the 22-locale set assertion), mandoc. Owed before it ships: the native pass over the ~1,220 machine-seeded strings, and the desktop language smoke (Settings → Language → Català; check a sidebar count plural, a run's activity status, a transcript empty state). The in-flight welcome-screen work can ride the same release. React migration complete (Steps 1–10); bundled-sidecar desktop is the primary distribution path; CLI ships on PyPI + Homebrew + Snap. Static render is a sealed byproduct, not a user-facing product.
