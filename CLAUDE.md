...
- **Three invocation modes**: (1) explicit `bristlenose doctor` — full report, always runs; (2) first-run auto-doctor — triggers when sentinel missing or version mismatch, writes sentinel on success; (3) pre-flight — full doctor table on every `run`/`transcribe-only`/`analyze` (skipped if auto-doctor already ran same invocation)
...
- **Header**: `_print_header()` in `cli.py` prints `Bristlenose [dim]v{version} · {provider} · {hw.label}[/dim]` as the first visible output, before doctor checks. "Bristlenose" in regular weight, rest dim. Session count is not in the header (not known until ingest); instead pipeline prints `{n} sessions in {dir}/` after ingest completes
- **Summary** (`_print_pipeline_summary()` in `cli.py`): dim stats line, dim LLM usage with cost estimate, `Report:` label (regular weight) with OSC 8 `file://` hyperlink, `[green]Done[/green]` + elapsed as the very last line
