...
Core pipeline complete and published to PyPI + Homebrew. Snap packaging implemented and tested locally (arm64); CI builds amd64 on every push. Latest: **API key onboarding + .dmg packaging** — first-run setup screen prompts for Claude API key (stored in macOS Keychain via `security` CLI), Settings panel (⌘,) for key management, `build-dmg.sh` produces distributable `.dmg` with ad-hoc signing. Prior: desktop app serve mode (auto-launches `bristlenose serve` after pipeline, opens report at `http://127.0.0.1:8150/report/` with React islands), 5-state launcher, self-contained sidecar with bundled FFmpeg/ffprobe/Whisper model, serve mode (FastAPI + SQLite + React islands), interactive dashboard, sessions table redesign, appearance toggle, user journeys, tab navigation, Gemini provider, transcript annotations, hidden quotes + codebook, Azure OpenAI, Ollama, transcript coverage, multi-participant sessions. See git log for full history.

**Next up:** WKWebView integration (report inside the app window instead of external browser); analysis page React island; cross-session moderator linking; snap store publishing. See `TODO.md` for full task list.
