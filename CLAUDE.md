...
**Next up (Feb 2026 session work):** LLM provider abstraction + Ollama as zero-friction entry point. See `docs/design-cli-improvements.md` for detailed design. Phase 1 (Ollama) removes the biggest adoption barrier — no signup, no API key required to try the tool. Then Azure for enterprise, Keychain for secure credential storage, Gemini for budget users. Also: Phase 2 cross-session moderator linking; snap store publishing. See `TODO.md` for full task list.
