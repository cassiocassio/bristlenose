...
- **New keys must go in every locale directory** — currently 19: en, es, ja, fr, de, ko, cs, it, pl, ru, uk, da, sv, nb, tr, pt-BR, pt-PT, zh-Hant, zh-Hant-HK (pl/ru/uk/da/sv/nb/tr are machine-seeded, pending native review). Every `t("key")` call needs an entry in each. If using `dt()`, the desktop override must also go in each `desktop.json`
