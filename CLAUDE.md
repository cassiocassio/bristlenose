...
**0.28.0 is the release in flight** (27 Aug 2026) — version files, `CHANGELOG.md` and `README.md` carry it. **The tag push is the publication**: the `pypi` required-reviewer hold was removed 23 Aug 2026, so nothing waits for a human and PyPI is immutable the moment it lands. `publish → build → ci (strict macOS)` is what stands between a tag and PyPI, asserted by `check-release-ready.sh`'s `publish gate` row. Tier 1 as usual: TestFlight, `.dmg`, PyPI, GitHub Release, Homebrew, Snap edge.
...
**Blocking the tag right now** (`check-release-ready.sh 0.28.0`): an unclean working tree, the `tap workflow` divergence (clears on push), and a stale dependency inventory that `build-all` will refuse. **Do not let the `associated-domains` entitlement ride this release** — it is `skip-worktree`, so `git status` cannot see it; committing it obliges regenerating the Mac App Store profile or the archive fails to sign, and it is for a parked feature.

**Fedora/Copr — built, proven end to end, deliberately NOT shipped.** `rpm/` + `.copr/` build a self-contained RPM in `mock` with no network; proven on real Intel Fedora 43 / Python 3.14 / rpm 6.0.2, and on Copr's own builders, where `dnf copr enable` + `dnf install` worked from a clean box. **The production project does not exist** (`api_3/project` → 404) and no public doc mentions Copr — that ordering is deliberate and was violated once already: `INSTALL.md`/`README.md` shipped `dnf copr enable` against the 404 and had to be reverted. `probe_copr` and a token-expiry gate are written and tested but **held out of `CHANNELS`**, because `rollup()` counts `unreachable` as not-ok and a channel listed before its first build reddens every release in between. The flip is one commit, specified in `docs/design-fedora-packaging.md` §7. Also settled there: `ffmpeg-free` is sufficient (26/26 formats, measured — HEVC costs a thumbnail and nothing else), and `sudo dnf install ffmpeg` is **not** a bug, having resolved to `ffmpeg-free` on both F41 and F42 when measured.

**Still owed from 0.27.0:** the Catalan native pass (owned, a few weeks out — no release waits on it, since nine other locales ship machine-seeded pending review), and `WelcomeHomeView.swift`:290's hardcoded `Text("Setup →")` on the Welcome AI cell, which every locale sees. **`v0.26.0` was tagged and abandoned** — it pointed 34 commits behind real work; it reached no channel and is in no changelog. Treat it as a version that never existed.
