...
## Current status (v0.10.0, Feb 2026)

Core pipeline complete and published to PyPI + Homebrew. Snap packaging implemented and tested locally (arm64); CI builds amd64 on every push. Latest: **Desktop app serve mode** — after pipeline completes, the app automatically launches `bristlenose serve` and opens the report at `http://127.0.0.1:8150/report/` with React islands (interactive dashboard, sessions table, codebook, quotes). 5-state launcher (ready → selected → running → serving → done), ProcessRunner manages both pipeline and serve processes, green "Serving report" indicator in done view. Prior: desktop app v0.1 (4-state, file:// only), self-contained sidecar with bundled FFmpeg/ffprobe/Whisper model, serve mode (FastAPI + SQLite + React islands), interactive dashboard, sessions table redesign, appearance toggle, user journeys, tab navigation, Gemini provider, transcript annotations, hidden quotes + codebook, Azure OpenAI, Ollama, transcript coverage, multi-participant sessions. See git log for full history.

**Next up:** WKWebView integration (report inside the app window instead of external browser); .dmg packaging; analysis page React island; cross-session moderator linking; snap store publishing. See `TODO.md` for full task list.
