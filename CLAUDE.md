...
**0.26.0 is bumped but NOT released** (14 Aug 2026) — version files, CHANGELOG and README carry it; no tag, no push, nothing approved. It adds the **Catalan (`ca`) locale**, the first built glossary-first: the ~30-row terminology table was settled *before* seeding, so the seed sits on top of it rather than guessing. **No native reviewer has seen any of it yet** — glossary or strings. 1,247 keys, all 10 registration sites, `check-locales.py` clean — as, since the 21 Aug sweep, is every other locale. **Full sweep green** — 3925 pytest, 402 locale-key tests, ruff, Node-24 frontend build, `xcodebuild` build + `I18nTests` (18 tests, including the 22-locale set assertion), mandoc. Owed before it ships: the native pass — the glossary table first, then the ~1,220 machine-seeded strings — and the desktop language smoke (Settings → Language → Català; check a sidebar count plural, a run's activity status, a transcript empty state). The in-flight welcome-screen work can ride the same release. React migration complete (Steps 1–10); bundled-sidecar desktop is the primary distribution path; CLI ships on PyPI + Homebrew + Snap. Static render is a sealed byproduct, not a user-facing product.

**0.25.0 (6 Aug 2026)** shipped the **Sessions lens responsive grid** (container queries + subgrid replacing a `table-layout:auto` table that overflowed 195px at 616px content; degradation ladder sheds surnames → journey → names, badge is the anchor — thresholds deliberately off the global breakpoint scale), **spatial arrow navigation** on the Quotes lens (arrows geometric via `spatialNav.ts`, `j`/`k` reading order, plus the route guard making quote keys inert off-lens), `.section-heading` promoted to an atom with a `<SectionHeading>` component across 19 call sites, and the dark-mode heatmap fix (`useIsDarkAppearance` — the `data-theme` read was permanently false on desktop). Focus Mode tuning (one-ring rule; cursor ring hoisted app-wide to `atoms/interactive.css`; starred bar tamed in dark), the appearance switch no longer restarting the serve, and the moderator pill + quote-context expansion parked behind `utils/featureFlags.ts`. Build side: `check-dmg-shippable.sh` + atomic `upload-dmg.sh` after the 14-hour unnotarised near-miss, and the sidecar fingerprint closing five holes (theme, LLM prompts, codebook, data, cohort-baselines) with coverage asserted by `test-ensure-sidecar.sh` case 9.
