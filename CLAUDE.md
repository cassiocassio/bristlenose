...
- **Header**: `[dim]Bristlenose v{version} · {n} sessions · {provider} · {hw.label}[/dim]` — printed after ingest (session count not known until then)
- **Summary** (`_print_pipeline_summary()` in `cli.py`): `[green]Done[/green]` + elapsed, dim stats line, dim LLM usage with cost estimate, dim Report label with OSC 8 `file://` hyperlink
...
- **Styling principle**: minimal colour. Green `✓` checkmarks, dim timing, dim header/stats/labels. No bold except the checkmark message text. The checkmarks and their messages are the only bright elements — everything else is muted
...
Core pipeline complete and published to PyPI + Homebrew. Snap packaging implemented and tested locally (arm64); CI builds amd64 on every push. Latest: CLI output overhaul — replaced garbled Rich spinner + logging output with clean Cargo/uv-style checkmark lines. Green `✓` per stage with right-aligned timing, dim header line (version · sessions · provider · hardware), LLM token usage + cost estimate in summary, OSC 8 terminal hyperlinks for report path. Output capped at 80 columns. All tqdm/HuggingFace progress bars suppressed. New files: `llm/pricing.py` (pricing table + `estimate_cost()`), `tests/test_cli_output.py` (13 tests), `tests/test_llm_usage.py` (10 tests). New `PipelineResult` fields: `elapsed_seconds`, `llm_input_tokens`, `llm_output_tokens`, `llm_calls`, `llm_model`, `llm_provider`, `total_quotes`. New `HardwareInfo.label` property, `LLMUsageTracker` class. Previous: man page (`man/bristlenose.1`) updated to v0.6.5, search-as-you-type filtering, platform-aware session grouping. v0.6.5 also adds page footer, timecode two-tone typography, hanging-indent quote layout, non-breaking quote attributions, transcript page name propagation. v0.6.4 adds concurrent per-participant LLM calls. v0.6.3 redesigns report header, adds view-switcher dropdown, Analysis ToC column, anonymisation boundary. v0.6.2 adds editable participant names, auto name/role extraction, editable headings. v0.6.1 adds snap recipe, CI workflow. v0.6.0 added `bristlenose doctor`. v0.5.0 added per-participant transcript pages. Next up: register snap name, request classic confinement approval, first edge channel publish. See `TODO.md` for full task list.
