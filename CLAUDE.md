...
- **New keys must go in all 7 locale files** — en, es, fr, de, ko, ja, cs. Every `t("key")` call needs an entry in each. If using `dt()`, the desktop override must also go in all 7 `desktop.json` files
...
- **QA-ing a *frontend/CSS* change in the bundled `.app` needs TWO refreshes, neither of which is `npm run build` alone.** The bundled `.app` (`sidecar=bundled` in the footer = prod serve, not `--dev`) gets the SPA from two baked sources, and a worktree edit reaches *neither* until you act: (1) **JS** — the React bundle lives inside the PyInstaller sidecar (`…/bristlenose-sidecar/_internal/bristlenose/server/static/`), so `npm run build` into `bristlenose/server/static/` is invisible to the `.app` until you re-run `desktop/scripts/build-sidecar.sh`. Worse, `/new-branch --from-cloud` `ditto`s **main's** sidecar into the worktree, so a fresh from-cloud worktree's `.app` runs *main's* frontend — any imported frontend feature looks like it "didn't work" until the sidecar is rebuilt. (2) **CSS** — `bristlenose/theme/organisms/sidebar.css` is **not** bundled into the Vite output; it's served as `/report/assets/bristlenose-theme.css`, which in prod (`serve_theme_css_with_fallback`, `app.py`) *prefers the per-project baked copy* `<output_dir>/assets/bristlenose-theme.css` and only falls back to the bundled source when the project has none. So an already-rendered project serves its **stale** baked CSS even after a correct sidecar rebuild. Fixes: rebuild the sidecar for the JS; for the CSS either test against a **freshly-imported** project (no baked copy → fallback serves the rebuilt source) or regenerate the baked file — `.venv/bin/python -c "from bristlenose.stages.s12_render.theme_assets import load_default_css; open('<proj>/bristlenose-output/assets/bristlenose-theme.css','w').write(load_default_css())"` run from the **worktree venv** (reads branch source). Dev mode (`serve --dev` + `?embedded` for embedded-only features) sidesteps both — live JS from Vite, live CSS from source — but it's a browser, not the WKWebView; for a feature whose whole point is the native context, the `.app` pass is the real acceptance test
