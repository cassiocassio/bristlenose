...
Frontend: `frontend/` — Vite + React + TypeScript + React Router. 31 components in `frontend/src/components/`, 14 islands in `frontend/src/islands/`, 8 page wrappers in `frontend/src/pages/`, 1 layout in `frontend/src/layouts/`. In serve mode, a single `RouterProvider` root replaces the 11 separate `createRoot()` calls from the island era. Legacy island mode (individual mount points) remains as fallback for the static render path. 635 Vitest tests across 121 test files. `npm run dev` proxies to FastAPI; `npm run build` outputs to `frontend/dist/`. See `docs/design-react-component-library.md` for build sequence.
...
- **Video player: two data paths, source filename truncation** — video playback has two independent data flows: (1) **Static render path** — `render_html._build_video_map()` reads `InputSession.files` (full absolute paths) and writes `window.BRISTLENOSE_VIDEO_MAP` with correct `file://` URIs into the IIFE. `player.js` reads this via event delegation. (2) **Serve/SPA path** — the importer reads transcript headers (`# Source: show and tell 40.mov`) which only store the **filename** (written by `merge_transcript.py` line 59 and `render_output.py` line 202 using `.path.name`), stores it in the DB, and the video-map API (`/api/projects/{id}/video-map`) converts DB paths to `/media/` URIs. `PlayerContext.tsx` fetches this API. **The gotcha**: transcript headers lose the subdirectory (e.g. `interviews/`) because `.path.name` strips it. The importer's `_import_source_files()` compensates by scanning one level of subdirectories under `project_dir` (mirroring `ingest.discover_files()`). If video playback 404s in serve mode, check: (a) is the server running from the correct worktree directory? (b) does `Session.source_file` in the DB include the subdirectory? If not, delete `bristlenose.db` and restart to force re-import. In SPA mode, `player.js` bails out (`if (document.getElementById('bn-app-root')) return`) — only `PlayerContext` handles playback. `SessionsTable.tsx` must have an explicit `onClick` calling `playerCtx.seekTo()` because vanilla event delegation is inert
