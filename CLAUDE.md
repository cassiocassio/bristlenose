...
- **The pathspec commit form guards other FILES, not the same file — and a failed edit must gate its own commit.** `git commit -F- -- TODO.md` commits that path **from the working tree**, so it takes a concurrent session's uncommitted edits to *that file* along with yours. The shared-index rule above does not cover this: nothing was staged, and the pathspec form is the documented fix for the other half of the problem. Bit on 12 Sep 2026 — a `python3 - <<'PY'` edit failed its own `assert` (the match string was from a commit-message draft, not the file), the `git commit` on the **next line** ran regardless, and a live session's in-flight `TODO.md` paragraph landed under an unrelated subject. Two rules: **chain the commit to the edit** (`edit && commit`, never consecutive statements — a heredoc that raises still exits non-zero, so `&&` is enough), and before a pathspec commit on a shared file, `git diff -- <path>` and read it. Recovery is *not* a reset: the commit is unpushed but the content is another session's, and rewriting under concurrency is the documented way to lose work — leave it, and say so in the next commit.
