...
- **Open two studies at once, in two windows.** The Mac app showed one study at a time; now every window holds its own. **File ▸ New Window** (⌥⌘N) gives you a second view of the study you're on — quotes in one window, the transcript they came from in the other. **Open in New Window** takes a second study; on a folder, **Open in New Windows** opens all of them. Each window keeps its own lens, sidebar and scroll position, and its title says which study it's showing. Bristlenose now opens on the Welcome screen rather than reopening whatever you had last.
...
- **Català — Bristlenose speaks Catalan, built glossary-first.** Catalan (`ca`) joins the desktop app, the report and the CLI, in Settings → Language. Rather than machine-seeding straight off, this one settles a ~30-row glossary first and seeds 1,247 keys on top of it, with the Apple terms checked on a Catalan-set Mac. It ships as one locale, not a Valencian pair. The native review is still to come; refinement happens on [Weblate](https://hosted.weblate.org/projects/bristlenose/).
...
- **Two privacy settings that were never implemented now stop the run instead of warning past it.** `BRISTLENOSE_PII_LLM_PASS` and `BRISTLENOSE_PII_CUSTOM_NAMES` had been declared and wired to nothing — so you could list the names you most wanted gone and get a run reporting that redaction succeeded with those exact names still in the transcripts. Either now stops the run and says which setting it can't honour.
