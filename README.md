...
- **Bring in recordings from Teams and Google Meet without downloading them by hand.** Connect a work account and **File ▸ Import** lists the meetings you recorded: filter by title or attendee, choose how far back to look, tick what you want. Bristlenose fetches the recordings, puts them in a project and starts the analysis — recognising recordings you already have on disk, and saying plainly when one has expired or your account can't reach it. Connect and disconnect in **Settings ▸ Accounts**. Zoom isn't offered yet.
- **If you have run a Teams or Meet Word transcript through Bristlenose, re-run that study.** Both were read wrong and neither said so. Teams packs a whole speaking turn into one paragraph, so turns arrived anonymous and stamped 0:00. Meet was worse: its download flattens Gemini's summary into the same file as the transcript, so an AI paraphrase *about* your participant could be shown in your report as their verbatim quote. Both now yield genuine speech only, with real timecodes and speaker names.
- **One unreadable file no longer costs you the whole batch.** A single empty or truncated recording used to stop the run and take every healthy session with it. It's now refused by name, with the reason, and everything else is analysed. Declined files are listed rather than silently skipped, and Bristlenose reads **27 file types**, up from 16.
- **You can see which studies an agent can read.** **Settings ▸ MCP Agents** lists every project: whether an agent can reach it now or only once you open it, how many sessions that exposes, and when it was last asked. Access survives quitting — it used to look permanent and quietly wasn't. The sidebar antenna lights while an agent is actually reading.
- **Re-analyse works, and tells you what it would cost.** It had been permanently dimmed. It now runs, and asks first — naming the edited quotes, tags, starred quotes, renamed speakers and named themes a fresh analysis would discard. Recordings you drop into a project folder in Finder are noticed and offered.
- **Controls that edit a report were live inside reports you had exported.** An exported report is meant to be a read-only leave-behind, and five things in it weren't — including the accept and deny buttons on suggested tags, clickable in exports since a redesign renamed the control out from under the rule that hid it. All five are gated again, and a test now fails the build if a rule stops matching what it hides.
- **A failed analysis stays failed until you deal with it.** A run that failed yesterday came back blank this morning, taking **Show Diagnostics** with it — the cause had been recorded correctly the whole time, and every surface that would have shown it to you switched off at once. The failure and its explanation now survive a relaunch.
- **`bristlenose transcribe` says which recordings were silent.** It reported 42 of 57 sessions transcribed and listed nothing as failed. A recording with no speech is now named, with its filename and the reason.
- **A session's length reads as a length, and a timecode is the same everywhere.** The Sessions grid showed `26:31` beside a date and a start time, where it reads as a time of day; it now says `26m`. And a quote's timecode said `05:30` on the card and `5:30` in the CSV of that same quote — they now agree.
- **The panel that explains a missing participant now speaks your language.** It translated its heading, counts and buttons, then gave the one sentence saying *why* a recording was skipped in English to every language but English. All eight reasons, all 21 languages.
- **Català — Bristlenose now speaks Catalan.** Catalan (`ca`) joins the desktop app, web report, and CLI (Settings → Language) — twenty-two locales in all, shipping as one locale rather than a Valencian pair. A machine-seeded community preview, open for native speakers to refine on [Weblate](https://hosted.weblate.org/projects/bristlenose/).
- **Switching session on the Mac is a titlebar popover, not a sidebar.** **Switch Session** (⌘⌥L on the Sessions lens) replaces both a permanent left column and a dropdown you had to be reading a transcript to find. It's a real source list, so type-select, arrow keys and VoiceOver are the system's. Each row carries the badge, the session number and *all* the names on a multi-participant session. In a browser nothing changes.
- **Fewer words in the report's sidebars.** The Codebook lens is **Codebooks**; "Your tags" is **Manual tags**, "Built-in" is **Default**, "Frameworks" is **Library**. The Quotes and Codebooks sidebars drop title bars that were an empty padded box in the Mac app. English only this release.
- **Two privacy settings that were never implemented now stop the run instead of warning past it.** Setting `BRISTLENOSE_PII_LLM_PASS` or `BRISTLENOSE_PII_CUSTOM_NAMES` used to warn and carry on — so you could list the names you most wanted gone and get a run reporting that redaction succeeded with those exact names still in the transcripts. Either now stops the run and says which setting it can't honour.
...
The Sessions lens learns to shrink, the arrow keys learn the layout, and two build gates that couldn't fail learn to. _(Superseded within the hour by 0.25.1 — use that one.)_
...
- **The arrow keys move by what you can see; `j`/`k` move by reading order.** On a multi-column grid, **down** used to move the cursor to the card on the **right**, and left/right did nothing. The arrows are now geometric and `j`/`k` are a list cursor. Quote shortcuts also stop firing once you leave the lens, where they'd been eating page scroll and starring quotes you'd navigated away from.
...
A large release — codebooks you can switch on and off, a shareable offline report, a more reliable AutoCode, and a pass over the report's visual system.

- **Codebooks you can install, switch off, and trust to stay put.** Every Codebook Library tile has one button — **Install** or **Uninstall** (retiring _Add_/_Remove_) — so an installed codebook is no longer look-only. Each carries an on/off switch and a matching sidebar status dot (blue on, grey off); switch one off and its section folds to a summary, its badges hide report-wide, and it leaves the tag sidebar and suggestions — but its tags and results are kept, so flipping it back on restores everything. Uninstalling forgets the on/off state (reinstall returns it on, not greyed out), and codebooks you have switched on now code newly-added sessions too.
- **Share an offline report — a self-contained file.** Export a report as a single HTML file that runs the full app from `file://` with the data inlined: no server, no internet, nothing to host. It opens read-only (controls that can't persist offline are removed, not dead), bakes in the researcher's language, and a build-time coverage gate makes it impossible to silently ship half a report. Security: the embedded data is now escaped against `<`/`>`/`&`, closing a stored-XSS hole from a literal `</script>` in transcript text, a name, or a tag.
- **AutoCode stopped losing proposals — and a stalled run is recoverable.** A run could report "AutoCoded in 50s ✓ — 0 of 0 proposals" while the model returned dozens: a cosmetic progress-counter write (read-then-write) hit SQLite's _"database is locked"_ under any concurrent write and, un-isolated, took the batch's proposals with it. The progress write is now write-only and non-fatal. Re-apply proposals de-dupe per quote, a cancelled/failed job can be re-run, start errors surface, and orphaned jobs reconcile on startup.
- **The report reads calmer.** One heading per content zone, section dividers unified to a single full-width rule, balanced gutters on minimap-less panes, and a consistent radius tier. The Analysis lens scrolls to the bottom again, transcript margins use a cleaner grid layout, and the quotes-page minimap keeps its proportion when quotes flow into columns.
- **CLI: configure a provider without a keyring, with the env vars the SDKs already use.** `configure` persists keys to a config file when no OS keyring is available, and Bristlenose reads the SDK-native, unprefixed env vars for Gemini and Azure. No-provider guidance now points straight at `configure`.
- **The Snap is now strictly confined** — converted from classic (home/network/network-bind/removable-media), with the shared libs `ffprobe` needs staged and the frontend built under Node 22. Snap users only.
- **Fixes:** media probing fails loud when `ffprobe`/`ffmpeg` errors instead of reporting a blank success; the Sessions sidebar detects moderators again; the thin-theme bucket is plain _"Uncategorised"_.
- **Desktop (next bundled build):** the welcome screen is illustrated (looping science-cell animations, a study-tools carousel, a self-scaling book shelf); the Settings window resizes with animated per-tab height; and the menus fill in (Hide/Show Projects, Help ▸ Welcome, a Sessions-contents toggle, a Diagnostics menu). A signed direct-download `.dmg` now works for the expiring alpha alongside TestFlight.
