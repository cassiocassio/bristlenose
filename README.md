...
- **Codebooks install and uninstall like browser extensions — and the sidebar shows what's on.** Every Codebook Library tile now carries one toggling button — **Install** when it's available, **Uninstall** when it's in your project — so an installed codebook is no longer a look-only dead-end (the old _Add_/_Remove_ wording retires for _Install_/_Uninstall_). Each codebook in the sidebar gets a status dot that echoes its on/off switch — **blue on, grey off**, nothing when merely available. Switch one off and its groups fold into a one-line summary (_"N tags · M coded"_); uninstalling also forgets its on/off state, so reinstalling a previously-off codebook comes back **on**, not greyed out.
- **AutoCode no longer loses a batch of proposals to a locked database.** A run could report _"0 of 0 proposals"_ — even "AutoCoded in 50s ✓" — while dozens of real proposals were thrown away (38 returned, every tag stuck at zero). A cosmetic progress-counter write, done as a read-then-write transaction, hit SQLite's _"database is locked"_ under any concurrent write and — not being isolated — took the batch's proposals down with it. The progress write is now a bare, write-only update that waits for the lock, and can never discard proposals if it does fail.
- **The quotes-page minimap keeps its proportion when quotes flow into columns.** A multi-column quotes page is a fraction as tall, but the single-strip minimap drew it full-length — viewport indicator too small, scrolling full of parallax. The minimap now reads the live grid's column count and lays each section out in the same columns, following the page's own source of truth rather than a second set of breakpoints.
