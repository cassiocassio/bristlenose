...
- **Switching session on the Mac is a titlebar popover now, not a sidebar.** The Sessions lens had two ways to change session — a left sidebar reached for that one thing and charging a whole column for it, and a dropdown you had to be reading a transcript to find. Both are replaced by a **Switch Session** popover on the toolbar (⌘⌥L on the Sessions lens). It's a real source list, so the highlight, arrow keys, type-select and VoiceOver are the system's; rows carry the badge, session number and **all** the names on a multi-participant session, with duration and date. **All Sessions** returns to the grid. The lens also restores the view you left rather than landing on the index. Embedded Mac build only — in a browser the sidebar, rails and dropdown are unchanged.
- **The report's sidebars lost some words they didn't need.** The Codebook lens is **Codebooks**; "Your tags" is **Manual tags**, "Built-in" is **Default**, "Frameworks" is **Library**. Quotes and Codebooks drop their sidebar title bars (an empty padded box in the Mac app, which has no close button), the duplicate Codebook Library button goes, and the left nav narrows 280px → 200px now it holds contents not tags. English only this release; the other twenty locales follow once the English settles.
- **Two PII settings that were never implemented now refuse the run instead of warning past it.** `BRISTLENOSE_PII_LLM_PASS` and `BRISTLENOSE_PII_CUSTOM_NAMES` have been declared and wired to nothing for a long time, warning once and letting the run continue — so listing the names you most wanted gone got you a run reporting redaction had succeeded with those exact names still in the transcripts. Setting either now stops the run and names the setting it can't honour, counting the names it was given without printing them.
- **Word-file transcripts from Teams and Google Meet were both being read wrong, and neither said so.** A `.docx` transcript isn't one line per paragraph — Teams packs a whole speaking turn, name and all, into a *single* paragraph with line breaks inside it, so Bristlenose never found the speaker or the timecode and each turn arrived as one anonymous block stamped 0:00. Both platforms now read correctly, with real timecodes and real speaker names. If you've run a Teams or Meet `.docx` through Bristlenose, the transcript, its timecode links and the coverage figures were wrong even though the report looked fine — re-run those studies.
- **Google Meet's `.docx` also carried Gemini's summary into your transcript, and an unreadable one is now refused rather than half-parsed.** Meet saves no transcript file — it saves a Google Doc with two tabs, "Notes" (Gemini's summary) and "Transcript", and downloading it flattens **both** into one stream that Bristlenose read entirely as speech. On a real interview that put pages of Gemini's prose *about* what your participant said into the transcript beside what they actually said, where quote extraction couldn't tell them apart and could present an AI paraphrase as a verbatim quote. The attendee's email went in too, and every line landed at 0:00 with no speaker, so timecode links, coverage and the player all pointed at nothing — and the report still rendered and looked clean. Meet transcripts now yield only real speech, with real timecodes and the speaker's name as attribution. A `.docx` with timecodes that can't be matched to a speaker is **refused and named in the run's failures** instead of being flattened into untimed paragraphs. A Meet recording and its notes Doc also group as one session again, so the video isn't re-transcribed alongside a transcript you already have.
- **Català — Bristlenose now speaks Catalan, built glossary-first.** Catalan (`ca`) joins across the desktop app, web report, and CLI (Settings → Language) — twenty-two locales in all. Every previous language was machine-seeded straight off; this one builds the ~30-row terminology table first and seeds all 1,247 keys on top of it. Apple's Catalan terms were verified **on a Catalan-set Mac**, not from the web record, which is wrong about Catalan on macOS in both directions. Ships as **one locale, not a Valencian pair** — unlike the deliberate Portuguese and Chinese splits, the standard is shared. Register follows Apple's shipped Catalan: imperative for commands (`Desa`, not the Spanish-borrowed _Guardar_ — the dominant failure mode for machine-translated Catalan), pronominal passive for status, nouns in titles. **The native pass is still to come** — glossary and seeded strings alike are unreviewed. Refinement happens on [Weblate](https://hosted.weblate.org/projects/bristlenose/); fill-empty-only, so contributed translations always win.
