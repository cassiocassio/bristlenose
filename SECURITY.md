...
**MCP endpoint (`/mcp/`):** present only when the `bristlenose[mcp]` extra is installed; the auth prefix is enforced even when it isn't (fail closed). Bearer-only — the browser auth cookie deliberately does not authenticate it. The MCP SDK's DNS-rebinding protection is active (loopback-only allowed hosts and origins). Tools are read-only, take no filesystem paths (pinned by an input-schema test), and never read participant names.
