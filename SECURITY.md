...
5. **(Desktop app only) iCloud Keychain syncs your API key.** If you have iCloud Keychain enabled, macOS replicates your stored provider API key to your other Macs as a synchronizable Keychain item — your own credential, under your iCloud account, revocable at the provider. No transcript or research data is involved. Disable iCloud Keychain, or use the CLI (file-based, non-synchronizing), to prevent this.
...
7. **You connect an MCP agent to a served project.** `bristlenose serve` can expose a read-only MCP endpoint (`/mcp/`, loopback only, bearer-token-gated). On the CLI it exists only when the separate `bristlenose[mcp]` extra is installed; the macOS app ships it (Connect Agent… on a project), inert until you connect an agent. When you connect an agent (Claude Code, Claude Desktop, ChatGPT desktop, Codex, or any MCP client) and ask it questions, the tools return quote text (**verbatim**), speaker codes, section/theme labels, signals, and codebook definitions to that agent — which sends them to its own model vendor under that vendor's terms. Participant names are never included (this path never reads the persons table — stricter than the export boundary), no tool accepts a filesystem path, and the connection is read-only in data and in cost (no writes, no LLM calls from Bristlenose's side). Nothing is sent until you connect an agent yourself. The credential the macOS app's Connect Agent sheet hands out is **MCP-scoped**: it opens the four read-only `/mcp` tools and nothing else — it cannot reach `/api/*` (participant names, curation writes). It is durable (stored in the macOS Keychain, per project); to invalidate a pasted config today, move the project folder or delete the "Bristlenose MCP Token" Keychain item — an in-app Revoke affordance is planned. Corollary: what the agent reads becomes part of that agent's conversation history — deleting the project later does not delete the agent's history.
