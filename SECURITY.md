...
Bristlenose itself has no sub-processors for its core operation — no cloud database, no analytics service, no error-tracking vendor, no auth provider, and no telemetry endpoint. The one exception is opt-in: using **Export → Send to Miro** makes **Miro Inc. a sub-processor** for the quote content you choose to upload, held under your own Miro account and access token and governed by Miro's terms rather than Bristlenose's.
