...
- [ ] **i18n: translation quality fixes** — es/fr use "codebook" loanword in ~15 browse/import keys instead of localised term; ko participant term inconsistency (참여자 vs 참가자); de/ko missing `nav.codebookShort`. Fixed: ko colDuration "길이"→"소요 시간", ko helpHint "도움말"→"도움말 열기". Confirmed correct: fr undo/cancel both "Annuler" (Apple convention), de "Teilnehmer" (Apple convention). Gotchas documented in `docs/design-i18n.md`
