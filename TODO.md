...
- **Pipeline failure trust-UX** (chipped 14 May 2026, observed during multi-project-switch QA) — small Swift-side branch (likely `pipeline-failure-trust-ux`). Four sub-symptoms, one fix family: (1) `PipelineFailureCategory.outputExists` case classified from CLI exit code or stderr marker — replaces `Failed (unknown)` for the most common re-run failure; (2) sidebar badge wording: "Last run failed" / "Re-analysis failed", not "Something went wrong" (the latter implies the project is broken when only the *attempt to re-run* failed); (3) detail pane must load the project's existing DB unconditionally — failure-of-last-run is a row badge / detail-pane banner, NOT a gate on viewing already-analysed content; (4) Re-analyse (replaces existing output) CTA that spawns `bristlenose run --clean <path>` — researcher has no `--clean` CLI flag available. Root principle from the resilience design: **the state of the last run should not gate viewing the project**. Likely touches `desktop/Bristlenose/Bristlenose/PipelineRunner.swift` (enum case + exit-code mapping), `ContentView.swift` detail pane (drop failure-state gating), `ProjectRow.swift` (badge copy), one or two locale files. ~30–60 min. Pre-existing — surfaced by cohort tester re-running a project, not caused by sidebar-switch branch.
