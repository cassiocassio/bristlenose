...
Last updated: 4 Jun 2026 (v0.15.13 release — pipeline-view v2 per-(provider, model) grain shipped to PyPI; desktop flow-B Ollama setup + consent-activation merged; CI hardened with job timeouts, SHA-pinned actions, and least-privilege tokens. 2978 tests passing, ruff clean. Also: **Cassandra** dependency pre-mortem agent + `/cassandra` skill shipped — `0bbb1ca`, not yet pushed; see Ideas below.)
