...
- **Website `docs-src/cli.md` edit is committed nowhere and undeployed.** 31 Jul 2026: the CLI-docs audit (`95ff4bd4`) found `bristlenose pipeline` missing from the published CLI reference — shipped 18 May (`640d4eb8`), never documented on the website. Added the command row + its `--stage` / `--json` options in the **website repo's** working tree (`docs-src/cli.md`); `python3 build.py` succeeds and the rows render in `build/site/docs/cli.html`. Still to do there: commit, then the manual rsync deploy (needs SSH agent — maintainer-only). Until deployed, `bristlenose.app/docs/cli.html` is missing one command. The README line pointing at `man bristlenose` + the website reference is also still **uncommitted** in this repo — it sits inside `README.md`, which carries another session's in-flight WIP (Miro bullet, notarisation copy, Gemini pricing), so it was deliberately left unstaged rather than swept into that work.
