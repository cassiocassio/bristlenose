...
Last updated: 3 Aug 2026. _This file is a capture inbox + session context, not a changelog — `git log` + `CHANGELOG.md` are the unabridged record._
...
- **The MCP extension owes three small things before the next TestFlight.** 1 Aug 2026, all recorded in `docs/design-mcp-extension.md`: (1) **`AIConsentView.currentVersion` is still 1** — connecting an agent is a new recipient class per the project's own bump rule, and the copy is near-legal so it wants the maintainer's own sentence, not a generated one; (2) the `.mcpb` has **no `icon.png`** (512×512), so Claude Desktop shows a generic tile; (3) the pane's Claude Desktop hint should **pre-announce the one-time macOS "access data from other apps" prompt** (§6.1's advice — a prompt you were told to expect reads as a boundary working, an unannounced one reads as a fault). Copy + 20 locales, batched with the next string pass.
