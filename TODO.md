...
Last updated: 16 May 2026 (`multi-project-folder-watcher` implemented Phase 2 #14 — NSFilePresenter folder watcher + SQLite snapshot reader + session-count sidebar row + Schema A bare-progressive-coarsen date format + uniform `.secondary` subtitle text + semantic state-colour glyphs + sidebar precedence chain. Two `/usual-suspects` review passes, 62 findings triaged. 10 commits unmerged pending human acceptance walk. Memory expanded by 8 design-principle entries. HIG corpus handoff staged for a separate session at `docs/private/handoffs/hig-corpus.md`.)
