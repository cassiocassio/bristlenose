...
Last updated: 5 Jun 2026 (v0.15.13 release — pipeline-view v2 per-(provider, model) grain shipped to PyPI; desktop flow-B Ollama setup + consent-activation merged; CI hardened with job timeouts, SHA-pinned actions, and least-privilege tokens. 2978 tests passing, ruff clean. Also: **Cassandra** dependency pre-mortem agent + `/cassandra` skill shipped — `0bbb1ca`, **now public on origin/main** (`917565b`, pushed 5 Jun — no version bump: all `.claude/`/`docs/`/`.github/`, nothing in the wheel); see Ideas below.)
