...
Last updated: 16 May 2026 (`multi-project-folder-watcher` acceptance walk partial — 4 of 8 steps verified ✅ (pre-flight, baseline, missing-file detection, volume-eject cantFind transition), 2 steps deferred for design reasons, 2 steps pending re-test next session. 14 commits on branch including three mid-walk bug fixes: PyInstaller + typeguard 4.x `module_collection_mode` for inflect; SQLite WAL under sandbox via `?immutable=1`; `SubtitleVariant.deltaOnly` for missing-files on legacy projects. AppleDouble bug surfaced in `bristlenose.cli` — pre-existing, chip filed, not gating merge. 6 chips spawned during walk; 14 design-principle memories captured.)
