...
- [x] Man page (`man/bristlenose.1`) — full groff man page covering all commands, options, config, examples; included in sdist, snap installs to `share/man/man1/`, attached as GitHub Release asset; CI enforces version match with `__version__`. Homebrew formula needs `man1.install "man/bristlenose.1"` (tap repo change)
