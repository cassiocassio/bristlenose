...
- **`--llm <provider>` does NOT switch the model to that provider's `default_model` on the CLI — you get a cross-provider 404.** `analyze --llm chatgpt` (no `--model`, no `BRISTLENOSE_LLM_MODEL`) keeps the Claude code-default `llm_model` and sends it to OpenAI → `404 model_not_found`, surfacing as `PipelineAbandonedError: All topic segmentation calls failed` at s08. The provider→`default_model` coherence snap in `config.py` (`load_settings`) is **gated `if not hosted_by_desktop()`** — desktop-only (Swift injects the model). Diagnose via the `llm_resolve` ledger in the run log (`provider=openai … model=claude-sonnet-… (source=code-default)`). Workaround until fixed: set `BRISTLENOSE_LLM_MODEL=gpt-4o`. Real fix is its own branch (orthogonal to chunking) — apply the provider default when the provider is overridden and no model was set explicitly, preserving the `--llm`-vs-env precedence history.
