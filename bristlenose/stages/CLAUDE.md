...
- **Moderator/observer codes are assigned per-session but COLLIDE in `people.yaml` — they are NOT independent entries.** `assign_speaker_codes()` resets `mod_counter`/`obs_counter` on every call, so every session's first moderator is `m1`. `compute_participant_stats()` keys its output dict by speaker code alone (`stats[code] = ...`), so in a multi-session study session 2's `m1` overwrites session 1's, session 3's overwrites that, and `merge_people()` carries the survivor into `people.yaml`. **Net effect: exactly ONE `m1` entry for the whole study, holding the last session's stats and `session_id`.** Same for `o1`. (This line previously claimed the opposite — corrected 22 Aug 2026 after a two-session repro; don't re-derive from the old claim.) Consequences worth knowing: the sessions table groups by `computed.session_id`, so earlier sessions show no moderator at all; and because `merge_people()` preserves *editable* fields by key while replacing *computed*, a name the researcher typed for session 1's moderator stays attached to an entry whose stats are now session 2's — mislabelling, not just loss. Each collision now logs a WARNING from `bristlenose.people` (grep the per-project log at `<output_dir>/.bristlenose/bristlenose.log`), so it is announced rather than silent — but it is still lossy. **The serve-mode DB does NOT have this collision**: `SessionSpeaker` (`server/models.py:265`) joins person↔session and represents per-session moderators correctly, so the two persistence layers genuinely disagree. One caveat on that — `_import_speakers()` populates `Person` name fields via a flat `people[code]` lookup, so the *names* it reads inherit the YAML collision even though the *rows* are per-session. Converging the two (giving `people.yaml` the person↔session split) is Layer 11 in `docs/design-transcript-speaker-editing-roadmap.md`; a bare key reshape without a migration would orphan researcher-typed names in every `people.yaml` already in the field. Pinned by `tests/test_people.py::test_multi_session_moderator_codes_collide_and_are_warned`
