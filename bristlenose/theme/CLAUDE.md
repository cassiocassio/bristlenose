...
- **`--bn-first-baseline` is NEGATIVE, and that's correct.** It aligns the webview content + inspector first-line *text baselines* to the native sidebar's first lens-row ("Project") baseline. It's a signed offset from the synced `--bn-toolbar-inset` line, and it's negative because the **detail pane's toolbar is taller** (two-line title) **than the sidebar's top strip** — so `--bn-toolbar-inset` (the detail chrome height, synced from native) sits *below* where the sidebar draws "Project". macOS is untouched; only the webview CSS moves. One knob (`--bn-first-baseline`), tuned by eye; each column subtracts its own header ascent-to-baseline as a literal (`.tag-sidebar` 24px, `.center` first-heading 19px) so a big heading and a small label share the line. Web↔web is exact (shared token); web↔native is a tuned approximation (a couple of px, forgivable). Don't try to compute font baselines across the AppKit/CSS seam — position from the shared datum with a tuned offset. Tuning happens in the `.app` only (rules are `[data-embedded]`-scoped, so a browser can't show them); rebake the project's baked theme CSS per the root-CLAUDE frontend/CSS-QA gotcha.
