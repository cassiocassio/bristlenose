...
The right tags sidebar is styled to read as a native-feeling **inspector** continuous with the AppKit sidebar. Three coupled gotchas, all in `organisms/sidebar.css` + `tokens.css` (added Jul 2026, commit `a96c7c6f`):

- **`--bn-colour-inspector-bg` is a themeable chrome tint, not a content surface.** It's in the colour *contract* (`colors/_contract.css`) — every palette must define it (default `#f9f9fa`/`#1c1c1e`, Xcode's flat inspector; edo warms it to washi `#f7f2e6`). `.tag-sidebar` background uses it instead of `--bn-colour-bg`. The principle: Apple *cools* the neutral inspector off white, edo *warms* the paper — same subtle desaturated-sibling relationship, mirrored hue.
