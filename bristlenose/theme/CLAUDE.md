...
- **Modifier-clicks must open new tabs** — any clickable element that behaves like navigation (tabs, session links, stat cards, speaker links, dashboard links) must check `e.metaKey || e.ctrlKey || e.shiftKey` and call `window.open(url, '_blank')` instead of in-page navigation. Users don't know or care that this is a single-page app — cmd+click should always open a new tab. Pattern: `if (e.metaKey || e.ctrlKey || e.shiftKey) { window.open(window.location.pathname + '#' + target, '_blank'); return; }`. Already applied to: tab bar buttons (`initGlobalNav`), speaker links (`_initSpeakerLinks`), featured quote cards, dashboard session links, dashboard list links, TOC links, analysis page, and dashboard React island. When adding new click handlers for navigation-like actions, include the modifier-key guard
