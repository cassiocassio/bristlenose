...
- **`--bn-colour-danger` / `-success` / `-warning` are now real tokens (fixed 13 Jul 2026, branch `claude/css-system-audit-xy6hm7`)** — defined per-palette in `colors/palette-*.css` as aliases to existing adaptive tokens: `danger → var(--bn-colour-negative)`, `success → var(--bn-sentiment-satisfaction)`, `warning → var(--bn-sentiment-surprise)`, so they dark-adapt and follow the palette. They're in the colour contract (both palettes declare them; `test_color_contract.py` enforces it). Use `var(--bn-colour-danger)` **without a fallback** — the old `var(--bn-colour-danger, #dc2626)` fallbacks were removed. Note the badge dark-mode pill hover still hand-writes rgba (a separate hardcoded-colour item, not this fix). Don't reintroduce `--bn-colour-text-muted` — it never existed; the token is `--bn-colour-muted`
- **Z-index uses the `--bn-z-*` scale, not magic numbers (added 13 Jul 2026)** — `tokens.css` defines `base(1) · sticky(100) · dropdown(200) · overlay(300) · toast(400) · modal(1000) · popover(1100)`. Pick the tier by the element's ROLE; use `calc(var(--bn-z-base) + 1)` for a second local level. Never add a bare `z-index: <number>` — the old bare `200` collided across ~9 components
- **Design-system compliance tooling exists — use it (added 13 Jul 2026)** — `scripts/audit-css.py` computes token-Coverage/Health from source (`--by-lens`/`--json`/`--fail-under`); `docs/design-system/compliance-catalogue.html` is the visual X-ray (all 5 lenses); `docs/design-system/colour-adjudicator.html` maps the hardcoded-colour backlog to tokens by eye. **These are ADVISORY only — no gate is wired yet, by deliberate decision** (raise quality before enforcing, so current mediocre values don't get baked in as "official"). When you add/edit theme CSS, prefer existing tokens; when no token fits, surface it (don't hardcode silently). See TODO.md "Ideas" top for the parked governance roadmap
