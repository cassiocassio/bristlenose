...
Shared editing and committed states for inline contenteditable fields. Extracted from `quote-actions.css` and the since-deleted `name-edit.css` (Round 2 CSS refactoring). Groups all editing visual patterns together — including the crop bracket handles for trim editing.

- **Editing state** — the house inline-edit field, driven entirely by the `--bn-field-edit-*` set. **Read the inline-edit block in `tokens.css` before styling a new one**: there are two treatments and the choice is mechanical, decided by the ground the field sits on. On page background it tints up (`--bn-field-edit-fill-on-page`); on a tinted surface it blanks to neutral (`--bn-field-edit-fill-on-tint`). Both carry `--bn-field-edit-edge` at 1px. Applied here to `.editable-text[contenteditable="true"]` and `.bn-speaker-editable-name[contenteditable="true"]`; the tinted-ground half lives on `.group-title-text` / `.group-subtitle` (`organisms/codebook-panel.css`) and `.tag-input-box` (`atoms/input.css`)
- **Keyed on the attribute, not a class** — `.editable-text.editing` is retained in the same selector for the frozen vanilla renderer, which is the *only* thing that writes `.editing` (`theme/js/editing.js`, `names.js`). `EditableText` never has. A rule keyed on `.editing` alone therefore could not fire in the SPA at all, which is how theme headings shipped with no editing treatment. Don't reintroduce a class for this state
- **Yellow is crop, and only crop** — `--bn-colour-editing-bg` survives on `.crop-editable` and `.crop-included-region`. That is a different verb ("move this boundary", not "retype this string") and it was designed as one signal with the burnt-amber brackets. Don't borrow it, and don't restyle crop to match the field set. `--bn-colour-editing-border` now has no consumers at all
...
- **Committed state** — deliberately NOT shared, unlike the editing state. `.editable-text.edited` is a dashed `--bn-colour-muted` underline; the speaker name marks the same fact in italic (`person-badge.css`); the codebook marks it not at all, because a codebook name was never the model's to override. One editing signal, three committed ones, on purpose
- **Side-by-side comparison** — `docs/mockups/edit-field-idioms.html` renders every scenario in both treatments with live CSS, plus the two exemptions (crop; real `<input>` controls, which have a box at rest and so only promote their resting border)
...
- **`.bn-session-speakers`** — `display: flex; flex-direction: column; gap: 0.35rem`. Contains one `.bn-session-speaker-entry` per speaker (vertically stacked)
- **`.bn-session-speaker-entry`** — `inline-flex, align-items: center, gap: 0.35rem`. Decomposed layout for serve mode: code-only `PersonBadge` + editable name text + pencil icon. Replaces the unified two-tone pill in the sessions grid only (other surfaces still use the full `PersonBadge` pill)
- **`.bn-speaker-editable-name`** — editable name text beside the code badge. Matches badge name typography (`0.72rem`, emphasis weight, body font). Hover: subtle `quote-bg` background. Active: the shared field rule in `molecules/editable-text.css` names this class alongside `.editable-text`, so the sessions grid and the quotes lens cannot drift apart — only the metrics and the committed italic are local here. Committed (`.edited`): italic. Uses `EditableText` component with `trigger="external"`. Full name shown as `title` tooltip when it differs from short_name
