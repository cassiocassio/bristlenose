...
///
/// Static plaintext/overflow formatters currently live on `PipelineActivityItem`
/// (still its canonical home + the unit-test call site); they relocate here when
/// the pill is removed.
struct ProjectDiagnosticPopover: View {
    ...
    private func degradedBody(message: String, category: PipelineFailureCategory) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("Category: \(PipelineActivityItem.humanCategoryLabel(category))")
                .font(.caption)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
        }
                   ...
                   !overflowMessage.isEmpty {
                    GridRow {
                        Image(systemName: MessageKind.warning.symbolName)
                            ...
                            .font(.footnote)
                        Text("")
                        Text(PipelineActivityItem.localisedOverflowText(
    ...
    private func copyDiagnosticForCurrentState() {
        ...
        case .completedPartial(let summary):
            text = PipelineActivityItem.formatDiagnosticPlaintext(
        ...
        case .failed(let message, let category):
            ...
            text = PipelineActivityItem.formatDiagnosticPlaintextDegraded(
