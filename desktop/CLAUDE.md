...
- **`.onDrop` on individual List rows breaks selection** — per-row `.onDrop(of:)` causes the same selection interference as tap gestures. _Historical note: workaround used to be `SidebarDropDelegate` + `RowFramePreferenceKey`/`FolderFramePreferenceKey` hit-testing on the List itself. Replaced May 2026 by per-row `.dropDestination(for: SidebarDrop.self)` — the newer `Transferable`-based API works on List rows without breaking selection. See next gotcha for the wrapper-Transferable pattern._
