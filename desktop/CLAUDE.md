...
- **Debug signing uses Apple Development + `DEVELOPMENT_TEAM = Z56GZVA2QB`** (Martin's personal team). Stable per-machine signature so Keychain "Always Allow" ACLs persist across rebuilds — without this, every Cmd+R re-prompts for each provider API key because ad-hoc (`Sign to Run Locally`) signatures change every build. Contributors without that team configured will see "No signing certificate found for team Z56GZVA2QB" on Debug build — fallback is Xcode → Bristlenose target → Signing & Capabilities → Debug → set Team to your own (or None + Sign to Run Locally). **Do not commit that local change.** Release config is independent (Apple Distribution + Mac App Store provisioning profile, unaffected by Debug signing choice). **First Cmd+R after switching identity fires three one-time prompts** — (1) `codesign wants to access key "Apple Development: …"` (signing key Always Allow), (2) `"Bristlenose" differs from previously opened versions` (LaunchServices, Open Anyway), (3) one Keychain ACL-extension prompt per stored provider key (Always Allow each). Verified 16 May 2026: macOS extends the existing item's ACL rather than orphaning it, so previously-saved keys survive the migration. Second Cmd+R is silent — that's the test.
