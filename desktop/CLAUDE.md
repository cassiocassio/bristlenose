...
- **When adding a new `PipelineState` case, audit `applyScanResult` for the early-return list.** `PipelineRunner.applyScanResult` guards the runner-owned states (`.running` / `.queued` / `.failed` / `.completedPartial` / `.failedWithDiagnostic`) from being clobbered by passive manifest scans. The two new diagnostic states added in `pipeline-diagnostic-popover-swift` weren't initially in this list; harness-injected state would render once then get reverted within seconds when the next scan ran. Class-of-bug rule: every new `PipelineState` case is presumed runner-owned (= early-return) unless you can prove otherwise. The compiler doesn't help here — `switch` over `PipelineState` in `applyScanResult` has a `default` arm. Audit it manually on every state addition.
