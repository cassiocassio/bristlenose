...
- **If a SwiftUI view is making a decision, the decision belongs in a testable helper, not in the view.** Drop hit-testing, menu enable state, error categorisation, availability mapping — pull each into a plain `struct` or free function that takes inputs and returns an outcome (e.g. `DropDecision.evaluate(items:target:) -> DropOutcome`, `RevealAvailability.from(project:) -> Bool`, `LocateValidation.classify(folder:) -> LocateResult`). The view becomes a thin caller; the helper gets a unit test in `BristlenoseTests/`. This is the minimum-step convention before reaching for an XCUITest target — covers the bulk of Swift-layer testability without new infrastructure. See `docs/design-test-philosophy.md` for the philosophy
