...
**Read process metadata via libproc, never via subprocess.** Three swaps now in the family — Swift `proc_pidpath` (zombie-cleanup PID verification, commit `38808fe`), Swift `proc_listpids`+`proc_pidfdinfo` (TCP-LISTEN socket walk, commit `5471b35`), and Python `proc_pidinfo(PROC_PIDTBSDINFO)` for process start time in `bristlenose/run_lifecycle.py:_ps_start_time` (May 2026). All `/bin/ps` and `/usr/sbin/lsof` execs are sandbox-blocked at startup, not at first call — adding a fourth subprocess-shaped helper will crash the bundled sidecar before it serves a byte. If a fourth need arises, that's the trigger to extract `bristlenose/utils/libproc.py` (Rule of Three, met).
