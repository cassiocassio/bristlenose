...
# latest_json <version> <dmg-filename> — the body of /dmg/latest.json.
#
# The homepage shows the current version beside its download button. Typed
# into the page, that number goes stale at every release; read from here, it
# moves in the same step as the redirect. Both values come from __init__.py and
# a fixed file-name pattern, so nothing needs JSON escaping —
# test-upload-dmg.sh parses the output to keep it that way.
latest_json() {
...
LJ="$(mktemp "${TMPDIR:-/tmp}/bn-latest.XXXXXX")"
trap 'rc=$?; rm -f "$HT" "$LJ"; [ "$rc" -ne 0 ] && cleanup_staging; exit $rc' EXIT
cat > "$HT" <<EOF
...
EOF
# The homepage reads the version beside its download button from latest.json.
# Both files go up first and are renamed into place by ONE ssh call, redirect
# first, so the page can name a version the button no longer serves for one
# rename at most.
latest_json "$VERSION" "$DMG_NAME" > "$LJ"
rsync -e "ssh ${SSH_OPTS[*]}" "$HT" "$REMOTE_HOST:$REMOTE_DIR/.htaccess.part" || die "redirect upload failed"
rsync -e "ssh ${SSH_OPTS[*]}" "$LJ" "$REMOTE_HOST:$REMOTE_DIR/.latest.json.part" \
    || die "latest.json upload failed — the live download is unchanged"
ssh "${SSH_OPTS[@]}" "$REMOTE_HOST" \
    "chmod 644 '$REMOTE_DIR/.htaccess.part' '$REMOTE_DIR/.latest.json.part' && mv -f '$REMOTE_DIR/.htaccess.part' '$REMOTE_DIR/.htaccess' && mv -f '$REMOTE_DIR/.latest.json.part' '$REMOTE_DIR/latest.json'" \
    || die "redirect swap failed"
ok "/dmg/Bristlenose.dmg → /dmg/$DMG_NAME"
ok "/dmg/Bristlenose.dmg.sha256 → /dmg/$SHA_NAME"
ok "/dmg/latest.json → $VERSION"
...
✓ published  ·  $DMG_NAME  ·  $(du -h "$DMG_PATH" | cut -f1)
  ...
  Undo:  point the Redirect line in $REMOTE_DIR/.htaccess at a previous file,
         and set the same version in $REMOTE_DIR/latest.json.
──────────────────────────────────────────────────────────────
EOF
