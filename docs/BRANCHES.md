...
**Why it drifted, and the cheap guard.** Worktrees stopped being the default when the project moved to solo trunk (`CLAUDE.md` § Branch workflow — *a branch is free, a worktree costs an env*), so nothing routinely opens or closes one, and nothing routinely reads this table either: a register only stays true if something consults it. **Run `git worktree list` before trusting these rows.**
...
**Worktree:** _removed 4 Sep 2026_ (1.6 GB reclaimed)
**Remote:** never pushed
**Retired:** 4 Sep 2026 — history at tag `archive/drag-push` (`7105fb47`)
...
Feature — Finnish (`fi`) locale, the 21st language — 9 namespace files + registration at all enrolment sites + 21 glossary rows; machine-seeded, native review by a Finnish contact (TAUCHI / Aalto / UXHel / FSD orbit) pending. Terminology grounded in Finnish primaries (FSD Tietoarkisto/KvaliMOTV, Mozilla `fi` style guide, TSK, TENK). Landed via `3e193fa8` (`--no-ff` merge; a concurrent merge had dropped the enrolment-list resolutions) + `92033192` (re-registered fi at all sites). Worktree detached and kept on disk, local branch deleted; remote was never pushed. Verified on main: check-locales (no fi warnings), pytest 3425, ruff, build+size 196.93 kB.

---

### `nl` — merged 3 Jul 2026

Feature — Dutch (`nl`) locale — 9 namespace files + 9 registration sites; machine-seeded, native review pending. Merged to main on 3 Jul 2026 (`88961b7a`, `--no-ff`). Worktree detached and tagged orange on disk, local branch deleted; remote was never pushed.
...
DevTools-style collapsible bottom inspector panel in the Signals lens. Heatmap matrices (section × sentiment, theme × sentiment, codebook group matrices) in a resizable bottom pane. Collapsed by default with grid-icon handle bar; opens via click, drag, or `m` shortcut. Signal card selection syncs with inspector source tabs. Tooltip flips above when near viewport bottom. New files: `InspectorPanel.tsx`, `InspectorStore.ts`, `useVerticalDragResize.ts`, `inspector.css`. 4 commits.
...
- **Parked, in the register above:** ~~`drag-push`~~ (retired 4 Sep 2026 — see
  below), `living-fish`, `symbology`, `tower-of-hanoi`,
...
reading, not counting. **17 local branches → 11; across the day 15 remote branches and 7 tags deleted.**
...
| `security-txt` | ~~Open PR #85~~ **closed 4 Sep, remote deleted** — `website/` no longer exists here. The security.txt itself is still owed (the live URL 404s); carried to `TODO.md` for the deploy repo. The local branch remains and carries the private tree — never push it; the guard now makes "never" mechanical |
...
**Third pass, same day — the orphans, then what the triage had walked into.**
`origin/claude/daily-code-lesson-j3z7l7` and `origin/claude/review-dependabot-updates-CF7in`
deleted. Checking them showed the second still carried the **private docs
tree** — and so did four more remotes and **twenty-three tags, sixteen of them
release tags (v0.14.3–v0.15.12)**: the 2 Jun scrub untracked the tree, but
nothing had ever gated *pushing* a pre-scrub ref. Also deleted from origin the
same day: the three tree-identical-to-local branches
(`claude/opus-4-8-transcript-eval-7QpEY`, `claude/shell-portability-macos-audit-dxwjD`,
`claude/restore-translation-project-30AZ9`), `security-txt` (PR #85 closed),
and the seven checkpoint/snapshot tags `pre-a1c-rebase`, `pre-c4`, `snapshot/*`
(remote SHAs 89ce4e2e, 6509a7dc, a3bc4ae9, 2caa3ed5, b2ce911b, ea629638,
059a9af0 — local copies remain at different SHAs). **Not deleted, by decision:**
the release tags and `main`'s own history. That is the private history-scrub
runbook's call; it stays unexecuted — no credentials, no third-party PII,
specifics already effectively disclosed — and reopens at the desktop-repo
split. Guard: `scripts/git-hooks/pre-push` (CLAUDE.md § Gotchas has the
pre-commit reasoning; `scripts/test-pre-push.sh` proves it). Locals still
carrying the tree, never to be pushed: `tower-of-hanoi` and the four branches
above.
