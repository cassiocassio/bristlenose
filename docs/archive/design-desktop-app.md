...
- _2026-07-10_ — scoped one-row true-up (topic: sessions folder-proxy reveal-in-Finder, commit `e5347cde`): corrected the §"Unresolved questions" custom-URL-scheme row — its "the React sessions table currently copies `file://` URIs to clipboard … a custom URL scheme solves this cleanly" premise is now false for the embedded app, which reveals the interviews folder in Finder via the new `reveal-in-finder` `project-action` bridge message (`NSWorkspace.activateFileViewerSelecting`, `BridgeHandler.swift`); clipboard-copy is now the browser-only fallback. The superseded §"Bridge protocol (expanded)" message-type table (`:979`, points to `desktop/CLAUDE.md` as canonical) was left as-is — `project-action` is a top-level type, `reveal-in-finder` a sub-value. No other sections assessed this pass.
- _2026-06-21_ — trued desktop slice against the `warm-sidecar-pool` merge (`78b2d40`): added a warm-pool supersession banner to §"Serve instance lifecycle" (kernel-assigned `bind(0)` ports + park-on-switch replace the `8150 + hash` / SIGINT-on-switch model); **re-superseded** the 2026-04-23 §"Loading and transition states" banner, whose `ContentView.swift:754-761` / `PipelineActivityItem` anchor is now dead — the pill was deleted and per-project progress/copy/failure moved to the sidebar row. Body preserved. Anchors: `ServeManager.swift`, `ParkedSidecar.swift`, `ProjectSubtitle.swift`, `ProjectDiagnosticPopover.swift`; commits `8ffa470` (pill deleted), `4313bff` (copy pill removed), `beaac38` (warm pool). Out-of-scope sections (Vision/PRD/Stack/menus/security/settings) not assessed this pass.
...
| Custom URL scheme (`bristlenose://`) for deep linking? | Future (folder-reveal case already solved) | Would enable serve-mode UI to open folders/files natively. **Partly resolved (2026-07-10, `e5347cde`):** the embedded macOS app reveals the interviews folder in Finder via the `reveal-in-finder` `project-action` bridge message (`NSWorkspace.activateFileViewerSelecting`) — no URL scheme needed, Finder does the reveal out-of-process (sandbox-safe). Clipboard-copy of the `file://` URI is now the *browser-only* fallback (browsers block `file://` navigation from `http://`). A custom scheme remains a candidate for richer file deep-linking, but the WKWebView bridge already covers folder-reveal without it |
...
> **Re-diverged 2026-07-15.** The 2026-05-01 divergence below is now moot — the rail is **cut** (it read as primary navigation). But decision 16 has diverged *further*, not returned: `WelcomeHomeView` is five rotating teaching cells (one study tool, one piece of science, one tip per visit) plus the drop card. The "no tutorial" decision survives in spirit — there is still no wizard, tour, or wall — but "drop zone + one sentence" is not what ships. The bet: professionals will read one ignorable factoid per visit if it never blocks them. See [design-welcome-screen.md](design-welcome-screen.md).
>
> _**Diverged 2026-05-01** (preserved — records why the rail was accepted): Shipped `WelcomeView.firstRun` includes a 3-step rail (ingest → process → export) and an "AI & privacy" link below the action cards — slightly more than the "no tutorial" decision recorded here. Trade-off accepted because the rail is a calm hairline-divider strip, not procedural prose, and answers the "what happens after I drop a folder?" question that researchers asked in early QA. `.noSelection` variant remains close to the original spirit (icon + sentence + button)._
...
> **Superseded 2026-06-21 (warm-sidecar pool, Phase A2).** The port model and switch behaviour described below predate the A2 rewrite. Shipped reality: ports are **kernel-assigned via `bind(0)`** (Swift scrapes the `Report:` URL from the sidecar's stdout), not `8150 + hash(project_id)`. On switch, the outgoing sidecar is **parked** (a single warm slot; no signal sent), and switching *back* re-points to it synchronously instead of teardown+restart — killing the boot wait. Canonical: `desktop/CLAUDE.md` → "Warm-sidecar pool (Phase A2)", `design-desktop-switch-performance.md`, `design-workspace.md`; code: `ServeManager.swift` (`switchProject`, `parked`), `ParkedSidecar.swift`. Body retained as planning history.
...
> **Re-superseded 2026-06-21** — the 2026-04-23 banner immediately below is itself now reversed, and its `ContentView.swift:754-761` / `PipelineActivityItem` anchor is **dead** (the per-project toolbar pill was deleted, commit `8ffa470`). Shipped reality has moved *back* onto the sidebar row, so the two table rows above are directionally closer to truth than the 04-23 correction claimed: per-project run progress is a determinate ETA ring + live progress-ladder subtitle (`ProjectRowActivityIndicator.swift`, `ProjectSubtitle.swift`, `ProjectRow.swift`); failure is a clickable glyph → `ProjectDiagnosticPopover.swift`; copy progress also rides the row (`"Copying · N%"` — the separate `CopyProgressPill` was deleted, commit `4313bff`). `[View Partial Report]` is still not shipped. App-global concerns (Ollama/Whisper download) stay in the toolbar. Canonical: `design-desktop-project-status.md` §4 (the per-project-row vs app-global-pill placement axis), `design-sidebar-activity-indicators.md`.
...
| **No project selected** (empty state) | Sidebar shows `+ Add Project` and project list. Main area shows **`WelcomeHomeView`** (2026-07-15, `WelcomeHomeView.swift`) — one unparameterised view for both first-run and no-selection: a golden-ratio spiral of five cells (Study tools · Scientific background · Tip · AI · Delight) that rotate their content per visit, with a dashed drop card as the sole action. Canonical spec: [design-welcome-screen.md](design-welcome-screen.md). _Supersedes `WelcomeView` (2026-05-01, commit `816ab65`): two variants (`.firstRun` / `.noSelection`), a New-Project card, a 3-step ingest→process→export rail, and the `chrome.noProjectSelected` / `chrome.selectProject` locale keys — all gone. Before that: a single drop zone + one sentence._ | — |
