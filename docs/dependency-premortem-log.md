...
| **fastapi** (major) | FastAPI / starlette | **The weakest row here, recorded as such.** There is no fastapi 1.0 — latest is 0.141.1, which is installed — and unlike pydantic there is **no external cap**: nothing in either venv requires fastapi except `huggingface_hub`'s optional extras, which we do not install. So the reason is not a resolver fact, it is blast radius: the entire serve layer (`bristlenose/server/`), and starlette moves underneath it. That is a judgement, and it has never been exercised, because Dependabot has never had a major to offer | fastapi ships 1.0 → **pre-mortem it then**, do not auto-take. The honest alternative is to drop this ignore and let the 1.0 PR arrive for hand-review, which is what `/cassandra` is for. **Open decision, see § the asymmetry (2026-09-05)** | 2026-09-05 | held — reason is blast radius, not a cap |
