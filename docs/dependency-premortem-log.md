...
| **anthropic** 1.x | LLM SDKs · anthropic→httpx2 | _(graduated 2026-09-05 — ceiling lifted, floor raised to `>=1.0` because the 0.x `temperature`-kwarg path no longer exists in the code. Verified on the day: `.venv` on **anthropic 1.4.0**, full free suite 4399 passed, and one live call per shipped model through the installed 1.4.0 SDK with sampling in `extra_body` — sonnet-4-6, haiku-4-5, opus-5 all PASS via `scripts/check-providers-live.py`. `versions: [">=1"]` retired from `dependabot.yml`. The "httpx2 wave" turned out to be one member: `openai` and `google-genai` were never ceilinged and had already resolved to 3.x / 2.x in the sidecar venv)_ | n/a — graduated | 2026-09-05 | **graduated** |
| **mcp** 2.1+ | assistant surface · mcp→mcp-types (pinned `==`) | _(graduated 2026-09-05 — ceiling lifted in both extras, floor stays `>=2.0`. The fix is the base class (`e6a161c3`: `ToolInputError` inherits the SDK's `ToolError`, guarded import). Verified on the day: `.venv` on **mcp 2.1.1 + mcp-types 2.1.1**, `test_mcp_server.py` green inside a full free suite of 4399. `versions: [">=2.1"]` retired for both names in `dependabot.yml`. MCP is local, so the suite against the real SDK is the live evidence)_ | n/a — graduated | 2026-09-05 | **graduated** |
...
| **@eslint/js 10 / eslint 10 / eslint-plugin-jsx-a11y** | **npm peer-cap chain** (new cluster) · `@eslint/js → eslint → eslint-plugin-jsx-a11y` | The hold was being enforced on **one of three** members, so the others could walk it past the pin. `@eslint/js@10.0.1` declares `peerDependencies: {eslint: "^10.0.0"}` while eslint is major-ignored at 9.39.4, and `eslint-plugin-jsx-a11y@6.10.2` (latest) peer-caps eslint at `^9` with no eslint-10 release in existence. Unlike a pip cap this fails as **ERESOLVE at `npm ci`**, not a warning. Paid for once already: landed 2 Jul 2026 (`7cc28c44`), reverted 3 Jul (`e449aad3`), recorded at `frontend/CLAUDE.md:23`. Both missing members added to the ignore list in `a6f8cc66` | `eslint-plugin-jsx-a11y` ships eslint-10 support; then bump `eslint` + `@eslint/js` + `eslint-plugin-jsx-a11y` in **one commit** | 2026-09-03 | held |
| **snapcore/action-build** `v1.3.0` (node20) — **we are pinned to a FORK** | GitHub Actions runtime · snap build path | **The recorded predicate could never fire, because snapcore is not the publisher.** Measured 4 Sep 2026: `snapcore/action-build` is `fork=true`, `parent=canonical/action-build`. It was created 23 Sep 2024 and last pushed the *same day* — 5 stars against upstream's 45; the entire `snapcore` org has 3 public repos and its newest activity anywhere is `snapd-ci` in Jan 2025. The "chore: update to node24" PR sitting there (#1, opened 5 Apr 2026) is a PR against a fork nobody watches. Upstream `canonical/action-build` is quiet but alive — #96, #94 and #98 all open on node24, touched Apr–Jun 2026 — yet `master` is **still** `runs.using: node20` and its newest tag is the same `v1.3.0` we pin, so repointing fixes provenance, not the runtime. **Node24 risk closed 4 Sep 2026 by a real run**: after the repoint (`30483d17`, identical SHA), run 33907334405 built the snap on `canonical/action-build` under the runner's forced-node24 and produced revision 14 | **DECIDED 2026-09-05 — hold on `canonical/*`.** Option 1 of the three below, and it is already in the tree: `snap.yml` uses `canonical/action-build` and `canonical/action-publish`, SHA-pinned, and revision 14 published to edge under the runner's forced node24. Nothing upstream says stop using them, so there is no forcing function; 16 Sep passes without a red build. Re-open when **arm64** is wanted (option 3 is the one that buys it), not on a date | 2026-09-05 | **decided — hold on `canonical/*`** |
| **snapcore/action-publish** `v1.2.0` (node20) — **also a fork** | GitHub Actions runtime · snap publish path | Same shape: `fork=true`, `parent=canonical/action-publish`, 1 star against upstream's 40, created and abandoned 23–24 Sep 2024. Its node24 PR (#1, 5 Apr 2026) has **zero comments**. Upstream has #52 and #54 open on node24 (May–Jun 2026) and `master` is still node20 at the same `v1.2.0` we pin. **Note this half has no craft-actions replacement** — `canonical/craft-actions` ships `pack` and `setup` only, no publish — so the publish path is either upstream `action-publish` or the snapcraft CLI with `SNAPCRAFT_STORE_CREDENTIALS`. **The one path skipped 59/59 is now exercised**: on 4 Sep 2026 `canonical/action-publish`, forced onto Node 24 (the runner's own warning names it), published revision 14 to `latest/edge` — confirmed in the store's channel-map, not just the run's green | **DECIDED 2026-09-05 — hold, with action-build.** | 2026-09-05 | **decided — hold on `canonical/*`** |
| **pydantic** (major) | **presidio ceiling** — the same cluster as cryptography and numpy | **There is no pydantic 3**; latest is 2.13.5 and 2.13.4 is installed, so the ignore blocks nothing *today*. What would block it is not ours: `presidio-analyzer 2.2.364` requires `pydantic>=2.12.5,<3.0.0` — the sole reverse-dep in either venv. So this is a third-party cap we cannot lift, which is precisely what makes it **unlike** the anthropic/mcp ceilings retired the same week: those were **our own** ceilings in `pyproject.toml`, verified against and removed. Same word, different basis | `presidio-analyzer` floats its `pydantic<3` cap; then move presidio-analyzer + pydantic **atomically**, as the cryptography row prescribes. Until pydantic 3 exists this cannot fire | 2026-09-05 | held |
| **fastapi** (major) | FastAPI / starlette | _(RETIRED 2026-09-05 — decided, ignore dropped. **The weakest row here, and that is why.** There is no fastapi 1.0 — latest is 0.141.1, which is installed — and unlike pydantic there is **no external cap**: nothing in either venv requires fastapi except `huggingface_hub`'s optional extras, which we do not install. So the reason is not a resolver fact, it is blast radius: the entire serve layer (`bristlenose/server/`), and starlette moves underneath it. That is a judgement, and it has never been exercised, because Dependabot has never had a major to offer The alternative taken: let the 1.0 PR arrive and hand-review it, which is what `/cassandra` is for and what the allow-list already does for react / react-router / i18next)_ | n/a — retired. The 1.0 PR is the trigger; pre-mortem it, do not auto-take | 2026-09-05 | **retired** |
| **typescript** 7 | **npm peer-cap chain** — the SECOND chain through typescript-eslint | Live and real, unlike most of this block: **typescript 7.0.2 is out and we are on 6.0.3**. `typescript-eslint@8.69.0` (latest) peer-caps `typescript: >=4.8.4 <6.1.0`, so taking 7 is an **ERESOLVE at `npm ci`**, not a warning — the same failure mode the eslint chain already cost us. `~6.0.3` in `package.json` is a tilde, so the resolver never proposes it anyway | `typescript-eslint` floats its `<6.1.0` cap to admit 7.x (one `npm view typescript-eslint peerDependencies` field); then bump **typescript + typescript-eslint in one commit**. As of 2026-09-05 the newest publish is `8.69.1-alpha.0` and the cap has not moved | 2026-09-05 | held |
| **typescript-eslint** (major) | **npm peer-cap chain** — the hinge of BOTH chains | Had no row while being the package the other two chains hang from: it peers on `eslint ^8.57 \|\| ^9 \|\| ^10` **and** on `typescript >=4.8.4 <6.1.0`. So it is simultaneously the thing that must move for eslint 10 and the thing that must move for typescript 7. **There is no typescript-eslint 9** (latest 8.69.0), so like pydantic this ignore blocks nothing today | typescript-eslint ships 9.x → take it **with** whichever chain it un-gates, never alone. It is the first mover in both | 2026-09-05 | held |
| **eslint-plugin-react-hooks** (major) | **npm peer-cap chain** | Installed 7.1.1 **is** latest 7.1.1, so nothing is held. It is in this cluster because it peers on eslint and would join the eslint-10 wave; on its own the ignore is inert | ships 8.x → take it with the eslint + `@eslint/js` + jsx-a11y commit | 2026-09-05 | held — inert today |
| **jsdom** (major) | frontend test env · jsdom → Node | **Three majors behind: 27.4.0 installed, 30.0.1 latest** — the largest gap in the tree, and the recorded reason ("Tied to Node major… see CHANGELOG v0.14.5 for jsdom + Node 25") has already been found dead once, in the 3–5 Sep re-grounding. Measured 2026-09-05: jsdom 30 declares `engines: node ^22.22.2 \|\| ^24.15.0 \|\| >=26.0.0`, which the pinned Node **satisfies** — so the Node predicate is met and is not what blocks. What actually blocks is untested: jsdom 27 ships **no** `localStorage` and no `matchMedia`, both shimmed in `frontend/src/test-setup.ts`, and three majors of jsdom may have added, renamed, or changed either — a shim that silently stops being needed, or silently stops working, is not visible to a green suite | **Not a version wait.** Read jsdom 28/29/30 release notes for `localStorage` / `matchMedia` / `structuredClone`, then take all three majors in one commit and delete whichever shims became redundant. Nothing external gates this | 2026-09-05 | held — Node predicate MET; the block is an unread shim question |
| **vitest** (major) | frontend test env · vitest → vite | 4.1.5 installed, **5.0.0 latest**. The recorded reason ("Tied to Node major") is not the binding one: vitest 5 peers `vite: ^6.4.0 \|\| ^7.0.0 \|\| ^8.0.0`, which our vite 8 **satisfies**, and `package.json` carries **no `@vitest/*` companions** (no coverage-v8, no ui), so there is no lockstep set to move with it. Its `@types/node` peer is `^22 \|\| >=24`, also satisfied | Nothing external gates this either — it is a read of vitest 5's breaking-changes list against ~40 spec files. Pre-mortem and take, or drop the ignore | 2026-09-05 | held — no external gate found |
...
### Note — the asymmetry, and what it turned out to be (5 Sep 2026)

The week's question, once `anthropic` and `mcp` came off their ceilings: **why
do `pydantic` and `fastapi` keep major-ignores "on the same floor-only basis"?**

They do not. Writing their rows is what showed it, which is the argument for the
register having no tombstones — the reason only gets tested when someone has to
type it.

- **anthropic / mcp** were **our own ceilings**, in `pyproject.toml`, put there
  by us and removable by us. We verified against 1.4.0 / 2.1.1 and removed them;
  the `versions:` rules in `dependabot.yml` were mirrors of those ceilings and
  retiring them followed mechanically. Nothing was decided about blast radius.
- **pydantic** is a **third-party cap**: `presidio-analyzer 2.2.364` requires
  `pydantic>=2.12.5,<3.0.0`, and it is the sole reverse-dep in either venv. We
  cannot lift it. That is the shape of the `cryptography` and `numpy` rows, both
  already held, both already presidio — pydantic belongs in that cluster and was
  simply filed under the wrong reason.
- **fastapi** is neither. There is **no cap** — nothing in either venv requires
  it except `huggingface_hub` extras we do not install — and **no fastapi 1.0
  exists**, so the ignore has never suppressed a single PR. Its reason is blast
  radius: `bristlenose/server/` entire, with starlette moving underneath.

So one of the two is justified on a stronger basis than the one recorded, and
the other is the only real question. **It is open, and it is the user's call.**

The argument for **dropping** fastapi's ignore is that the file's own opening
paragraph already states the policy — *"only deps that empirically caused rework
when auto-bumped"* — and fastapi never has, because it never had a major. The
config applies exactly that reasoning to react / react-router / i18next in its
"Notable allow-list" paragraph: majors flow, each hand-reviewed. An ignore is
also **silent**: it does not defer the 1.0 PR, it means nobody is told 1.0
shipped.

The argument for **keeping** it is that the serve layer is the largest single
surface in the tree and a Monday-morning PR is the worst moment to meet it.

Either answer is one line. What should not survive is the recorded reason,
which grouped it with pydantic on a basis neither of them has.


### Note — the snap build path (4 Sep 2026; decided 5 Sep 2026)

**Decision: option 1, hold on `canonical/*`.** The repoint is done and the
node24 risk is closed by a real run, so nothing forces a move — and neither
alternative is free: craft-actions has no publish half at all, and remote-build
is a rework of `snap.yml` rather than a repoint. **What re-opens this is arm64,
not a date.** The snap is amd64-only, which is why amd64 VMs exist to test it
from an arm64 dev machine; option 3 retires that dance. Take it when the arm64
snap is worth a day, not because 16 Sep is coming.

The three options are kept below as the record of what was weighed.
