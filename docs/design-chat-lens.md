...
- _30 Jul 2026_ — **§6 prototype built, to the §5a corrections.**
  ...
  truncation — with a **server-constructed integer citation space** (§5a
  Correction 2: quotes carry `[1]`…`[n]` markers, ids never reach the model,
  `resolve_quote_indices` rejects fabrication by arithmetic; `resolve_quote_ids`
  stays as the stable-id seam for the MCP workstream). `server/chat_lens.py`
  (service, `elaboration.py` template) adds the §5a Correction 1 **support
  check**: a second batched prompted-judge call (`chat-lens-support` prompt)
  marks each cited claim supported/unsupported — flag, never gate; a dead judge
  degrades to "unchecked". Answer schema (`chat-lens` prompt v0.2.0, v0.1.0
  archived): `claims[].quote_indices` + `citation_exempt` for connective text,
  three-way `abstain_reason` (`out_of_scope` / `no_evidence` / `ungroundable`),
  core rules restated after the corpus per the instruction-placement finding.
  Lab page (`/chat-lens`, `experimental_chat_lens` flag, default on): renders
  cited quotes beside claims in **both contested layouts** — inline and aligned
  sidebar — behind a toggle, plus curated example questions and
  reason-specific abstention messages that coach toward a better request.
  Tests (`tests/test_chat_lens.py`) centre on citation honesty: fabricated
  (out-of-range), hidden-quote (not expressible), and unsupported-but-resolvable
  citations are all flagged, never rendered as clean evidence. Native Claude
  citations (§5a option B) not taken — provider-agnosticism kept; worth the
  spike later.
