...
**On Google, the list UX and the affordable scope are mutually exclusive.** `drive.meet.readonly` is classified **restricted**, same tier as `drive.readonly`: CASA assessment, $500–4,500/year, re-verified annually. The affordable alternative, `drive.file`, is non-sensitive but *means the user picks each file in Google's own Picker* — not a filterable list, not one-click. Note the asymmetry's real cause: Microsoft's blanket scope is merely user-consentable while Google's is restricted. (Microsoft's nearest picker analogue, `Files.Read.Selected`, is *not* the counterpart it looks like — it is Preview, admin-consent, and its own docs say it "should not be used for directly calling Microsoft Graph APIs". Verified 15 Aug 2026.) We take `Files.Read` on Microsoft for exactly the reason we reject the Picker on Google — the list *is* the feature.
