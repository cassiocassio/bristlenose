...
- ~~**i18n — now realised debt, not a future cost.** The shipped window is hardcoded English throughout.~~ **Paid, 16 Aug 2026 (`49ec8a50`)** — 120 `desktop.cloudImport.*` keys across the 21 full locale directories, CLDR plurals on the count-bearing strings, `zh-Hant-HK` inheriting rather than pinned. Two vendor-mandated button strings remain, and §9a says to look those up rather than machine-translate them. _**Reopened 18 Aug 2026:** Settings ▸ Accounts shipped hardcoded English throughout — ~15 strings, plus the pane title, which is the only one of six Settings tabs not reading `i18n.t`. No `desktop.accounts.*` key exists in any of the 21 locales. Tracked in [i18n-defects.md](i18n-defects.md) rows 1–2._ The warning underneath still stands and is why this cannot be booked as closed: **`scripts/check-locales.py` only warns — nothing fails if it is skipped**, so the gap that reappears will do so silently.
