...
**A signal row in the analysis sidebar gives a long name a second line rather than an ellipsis.** Measured over the 58 named rows that survive de-duplication, a row runs 34 characters at the median once the group chip is beside it, 42 at p90 and 50 at its longest — so against a 28-character line, 90% of rows truncate. The shipped ellipsis is textbook-correct CSS (`flex:1; min-width:0; overflow:hidden`), so the mechanism was never broken; the outcome was. The chip is `flex-shrink:0`, so what a reader got was the words cut and the label kept — "Goal-driven Category Naviga… `Motivation`". An ellipsis that fires on nine rows in ten is a haircut, not an affordance. The cost is height, and de-duplication has already paid it: the busiest trial project drops from 29 rows to 19, and 85% of locations carry a single row. (13 Sep 2026; `docs/mockups/signals-sidebar-row-layouts.html` §V1 for the fitting, §T6b for the content.)

### A nameless signal row shows its group, and nothing else

**A card with no elaborated name appears in the navigation as its group chip alone — clickable, landing on a real card.** No fallback to the location name, no placeholder. Measured after de-duplication across 60 locations in the trial corpus, 38 carry a single named row and 13 a single bare chip, so the bare case is 22% and reads as what it is: a place with one group under it. Two readings died on the data — the shipped fallback repeats the location under a heading that already says it, and the "title plus subordinate tags" shape does not occur at all, because where a place has several rows they are usually all named and the named row is first in only 77% of locations. (13 Sep 2026; elaboration ranks across the whole project while the navigation ranks within a place, so naming is not monotonic down a location's list.)

### A signal's valence is computed but not shown

**Every elaborated card is classified `success`, `gap`, `tension` or `recovery` against the tag's own definition — and none of it reaches the screen.** The construct is sound: it asks whether the quotes satisfy the ideal a tag definition describes or fall short, which is the sense a researcher wants — is this place working or not. The presentation failed. An all-caps coloured chip reading TENSION shouts, reads as a category label, and competes with the score for the top right of the card. The data keeps flowing into `elaboration_caches`, so nothing needs regenerating when a better treatment arrives. (13 Sep 2026; a second, undisplayed valence vocabulary also exists — `classify_flag`'s Win/Problem/Niggle/Success/Surprising, which is sentiment-only and gates nothing. Settle both together.)
