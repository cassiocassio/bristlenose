...
> Architecture decision. **Status: Plan A queued for alpha; Plan B parked for post-alpha re-evaluation.** Sibling to `docs/design-modularity.md` and `docs/design-wkwebview-messaging.md`.
...
Likely root cause (unconfirmed pending Python traceback): macOS App Sandbox blocks the syscall Starlette uses for file streaming (probably `sendfile()` or an `aiofiles` async path). HTML routes work because they use a different code path (string read, not file stream).
...
2. Reads the file with plain blocking `path.read_bytes()` (or async equivalent that doesn't shell out to sendfile)
3. Returns a `Response(content=bytes, media_type=guess_type(path), headers={...cache-control...})`

This bypasses Starlette's sendfile-based streaming entirely. Works on any platform. Loses the streaming optimisation for very large files (irrelevant for ~80 small JS chunks). Adds an exception logger so any future failure surfaces a real Python traceback instead of generic 500.
...
Doesn't address the broader architectural friction (localhost server in a sandbox vs. native scheme handler). Just gets the sandbox StaticFiles failure out of the alpha path.
