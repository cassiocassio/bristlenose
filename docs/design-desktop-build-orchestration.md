...
The desktop app runs a **bundled** PyInstaller sidecar (`desktop/Bristlenose/Resources/bristlenose-sidecar/`) plus the React SPA baked into it — not live source. Today the only safety net is **detection**: the `Copy Sidecar Resources` build phase runs `check-sidecar-freshness.sh`, which **fails the build** when any hashed runtime input has moved past the bundle's `.source-stamp` (the input set is tabulated in §Core principle — deliberately not re-listed here, because keeping a second copy honest is what failed twice: this sentence has been wrong about it in two different ways, first naming three of six sets, then six of nine). Remediation is manual and three-step (`build-sidecar.sh && sign-sidecar.sh` + sometimes a clean build). Two things are wrong: the gate detects but doesn't remediate, and `build-sidecar.sh` recreates `.venv-sidecar` from scratch every run (multi-minute), so it can't naively fire on every Cmd+R.
...
**Point 1 has a precondition this section didn't state: the input set must be COMPLETE.** "Stamps attest inputs" silently assumes the recipe hashes *every* runtime input. Miss one and the gate doesn't merely fail to answer — it answers **confidently and wrongly**, which is worse than having no gate at all. Proven 5 Aug 2026: `bristlenose/theme/**` is bundled via the spec's `datas` and is real runtime input, but the recipe's `find bristlenose -name '*.py'` sweep never saw it (CSS/HTML, not `.py`) and no other clause covered it. So a theme-only edit didn't move the hash, `check-sidecar-freshness.sh` reported **"✓ matches source"**, the Ensure phase skipped the rebuild, and the `.app` went on serving the previous stylesheet — costing a rebuild-and-recheck cycle chasing a Focus Mode keyline that was already fixed in source. Fixed by hashing `bristlenose/theme` (commit "sidecar freshness: hash the theme too — the gate was answering wrongly, not just failing to answer"). **Rule for anyone adding a bundled data directory: add it to `sidecar-source-hash.sh` in the same commit as the spec `datas` entry.** The two are one decision; splitting them is how this recurs.

**The theme was one instance of a live class, not the whole of it** — worth stating, because the first pass of this section read as though naming the rule closed it. An audit the same day found four more repo-rooted `datas` entries outside the recipe, and the worst was verified empirically rather than reasoned about: appending a comment to `bristlenose/llm/prompts/autocode.md` left the hash **unmoved**, so editing any LLM prompt and hitting Cmd+R shipped the old prompt under a green gate — and every LLM-using stage loads those. Closed by the same commit that added the coverage assertion below. Current input sets, complete as of 5 Aug 2026:

| Input | Why it's in the recipe |
|---|---|
| `bristlenose/**/*.py` (minus generated `_build_info.py`) | the Python the sidecar runs |
| `bristlenose/locales/**` | i18n JSON — bundled into the SPA *and* read at runtime |
| `bristlenose/theme/**` | CSS/HTML served by the report |
| `bristlenose/llm/prompts/**` | every LLM-using stage loads these at runtime |
| `bristlenose/server/codebook/**`, `bristlenose/data/**` | bundled `datas`, read at runtime |
| `bristlenose/llm/cohort-baselines.json` | single-file `datas` entry |
| frontend build inputs (`frontend/src` minus tests, `index.html`, Vite/TS config, `package{,-lock}.json`) | build into `bristlenose/server/static/` |
| `desktop/bristlenose-sidecar.spec` | the recipe for *what* gets packaged |
| `desktop/mcpb/**` | the agent extension shipped in Resources |

`bristlenose/server/static*` is deliberately **out**: it is build output, not source — the frontend slice already fingerprints its inputs, and hashing the output would move the hash on every build.
...
**Not to be confused with the fourth class** (§Risks 5): an input the recipe never hashes at all. This one is a *timing* problem within an agreed input set, and it fails **loudly** — you see two hashes and a window. That one is a *set* problem, and it fails **silently and affirmatively**, printing "✓ matches source" about a file it isn't looking at. Same symptom on the surface ("my edit didn't take"), opposite diagnostic value, so read the gate's output before assuming which you're in: a mismatch means the machinery is working.
...
5. **The fingerprint's input set can be incomplete — and an incomplete input set inverts the gate.** Distinct from risk 3, which is writer/checker *drift* over one agreed recipe; this is the recipe agreeing with itself about the wrong set. The failure is silent and confident: the missed input never moves the hash, so `check-sidecar-freshness.sh` prints "✓ bundled sidecar matches source", the Ensure phase skips, and the `.app` serves the old file while `git diff` says you changed it. Worse than an absent gate, because you stop suspecting the bundle and start debugging source that was already correct. **Materialised twice, both on 5 Aug 2026** — `bristlenose/theme/**` (found by a QA cycle chasing a stylesheet fix that "wouldn't take"), then `llm/prompts` + `server/codebook` + `data` + `cohort-baselines.json` (found by auditing the class rather than the instance). Both closed, and unlike risks 2–3 this one now has a real mitigation: case 9 asserts spec→fingerprint coverage mechanically, so a new `datas` entry without a recipe entry fails the test rather than shipping a green lie. Two residuals, both known and neither live today: the parser matches `os.path.join(PROJECT_ROOT, "bristlenose", …)` only, so a differently-spelled `datas` entry would slip past it (all nine current entries match — checked); and the probe edits **one file per directory**, so a directory whose data files are uncovered while its `.py` files are swept could pass on the wrong probe. The probe therefore prefers a non-`.py` file, since `bristlenose/**/*.py` is already swept wholesale and a `.py` probe proves nothing about the directory's data. That matters concretely for `bristlenose/server/alembic`, which is pure `.py` + `__pycache__` and would otherwise have passed by ordering luck rather than by coverage.
