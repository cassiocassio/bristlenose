...
The desktop app runs a **bundled** PyInstaller sidecar (`desktop/Bristlenose/Resources/bristlenose-sidecar/`) plus the React SPA baked into it — not live source. Today the only safety net is **detection**: the `Copy Sidecar Resources` build phase runs `check-sidecar-freshness.sh`, which **fails the build** when any hashed runtime input has moved past the bundle's `.source-stamp` — `bristlenose/**/*.py`, `bristlenose/locales/**`, `bristlenose/theme/**`, the frontend build inputs, the PyInstaller spec, or `desktop/mcpb/**`. _(This sentence originally named only the first two and `frontend/`; the theme was missing from both the prose and the recipe, which is what made the gate false-green — see §Core principle.)_ Remediation is manual and three-step (`build-sidecar.sh && sign-sidecar.sh` + sometimes a clean build). Two things are wrong: the gate detects but doesn't remediate, and `build-sidecar.sh` recreates `.venv-sidecar` from scratch every run (multi-minute), so it can't naively fire on every Cmd+R.
