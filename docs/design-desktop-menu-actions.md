...
| `toggleInspectorPanel` (heatmap) | — | **Shipped** (bridge, tab-gated) | Disabled outside Signals lens (`MenuCommands.swift:267-270`) |
...
| Action | Status | Notes |
|---|---|---|
| `browseCodebooks` | **Shipped (bridge → `CodebookV2`)** _(12 Sep 2026, second edit)_ | Handled in `islands/CodebookV2.tsx`; switches the lens to `?view=library`. _(Read **Shipped (bridge → CodebookPanel)** until 12 Sep, then **Orphaned** for part of that day — `baa1aa0e` deleted the panel and took the listener, so the row named its own missing consumer. The dead `bn:codebook-browse` dispatch in `AppLayout.tsx` is now deleted.)_ |
...
| ~~`importFramework`~~ | ~~`{ templateId: string }`~~ | **Retired 12 Sep 2026** — no payload; `installCodebook` acts on the open page |
| ~~`removeFramework`~~ | ~~`{ frameworkId: string }`~~ | **Retired 12 Sep 2026** — `uninstallCodebook` acts on the open page |
