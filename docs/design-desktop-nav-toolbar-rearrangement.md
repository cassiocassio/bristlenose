...
### 3.1 Selection model — one List, one mode rail
The sidebar `List` is **already** `@State Set<SidebarSelection>` (`ContentView.swift:164`, `:1390`) — a discriminated-union multi-selection (`.project` / `.folder`). The earlier "stock `List` is single-selection" worry was wrong, and it dissolves the old "two stacked Lists" question:

- **Add a `.lens(Tab)` case** to `SidebarSelection`. One `List` carries both axes; a lens click rides the native `List(selection:)` path (no `.onTapGesture` — important, given macOS-26's tap-gesture-breaks-selection class), and arrow-key traversal stays continuous across the column.
- **The lens band is a *mode rail*, not a peer selection.** Render it with a deliberately *different, lighter* weight than a project row (accent-tinted symbol / medium-weight label — toolbar-toggle language), so a lit lens + a lit project never read as a stuck multi-select. The honest framing is *one selection (project) + one persistent mode (lens)*, not "two equal selections."
- **Empty state:** the lens rail is **dimmed/disabled until a project is selected** — the same affordance today's tab `Picker` gives via `.disabled(selectedProject == nil || !isReady)` (`:1322`). The dimming *is* the "pick a project first" teaching. With zero projects, `WelcomeView.firstRun` owns the detail pane.
...
- `.contextMenu(forSelectionType:menu:primaryAction:)`: the `primaryAction` closure **is** the double-click handler — this **unblocks the parked slow-double-click rename** *and* gives selection-aware right-click in one modifier. Right-click selects-under-cursor first; mirror every item into the menu bar. [SerialCoder]
- Drag-to-reorder: hover-revealed drag handle + `.moveDisabled(!isHovering)` so `onMove` doesn't steal focus from the rename field. **Folder-drag is the likely AppKit-surgery site you flagged.** [Nil Coalescing]
...
**TF triage — what of §10 is actually TestFlight build work:** just the focus model — §10.1's no-trap ⌘ "focus Projects" command + the standard keyboard/menu wiring — plus keeping the §3.2 row animation reduce-motion-safe. **Direction-recorded, post-TF / beta:** background alerting (§10.4); drag-to-reorder (§10.5 — hover-handle + folder-AppKit when built); Quick Look (§10.6); and the `performAccessibilityAudit` gate + the native→web Dynamic-Type curve (§10.7 — with the beta a11y pass, which genuinely needs the running app to tune). The slow-double-click rename (§10.5) rides in **for free** with `.contextMenu(forSelectionType:primaryAction:)` whenever the sidebar's built — adopt the modifier, no extra cost.
