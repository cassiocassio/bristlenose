...
- **Empty state:** the rail is **dimmed/disabled until a project is selected** — the same affordance the old tab `Picker` gave via `.disabled(...)` — shipped as `LensRail(... isEnabled: selectedProject != nil && bridgeHandler.isReady)` (`ContentView.swift:1472`). The dimming *is* the "pick a project first" teaching. With zero projects, `WelcomeView.firstRun` owns the detail pane.
