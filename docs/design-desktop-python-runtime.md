...
| `com.apple.security.cs.allow-jit` | ⚠️ **Required for the RUN path** (added 7 Jun 2026) | mlx-whisper → numba → llvmlite JIT-compiles word-timestamp kernels at transcription time; under Hardened Runtime the sidecar SIGKILLs (`EXC_BAD_ACCESS` / "Code Signature Invalid") without it. The C0 "not needed" finding was **serve-only** (no transcription). Apple DTS documents `allow-jit` as unrestricted + sandbox-compatible + MAS-permitted (forums thread 805941). | App Review justification in the `desktop/bristlenose-sidecar.entitlements` header comment. Post-external-submission experiment: try `allow-jit` alone (drop the row below) — modern llvmlite may use `MAP_JIT`. |
| `com.apple.security.cs.allow-unsigned-executable-memory` | ⚠️ **Required for the RUN path** (added 7 Jun 2026) | Paired with `allow-jit` for numba/llvmlite's legacy MCJIT (W+X pages rather than the hardened `MAP_JIT` dance). Apple calls this the "less secure legacy" JIT key — preferred is `allow-jit` alone, but numba currently needs both on arm64. | Same note as `allow-jit`. Candidate to drop if the `allow-jit`-only experiment succeeds pre-external-submission. |
...
| `com.apple.security.app-sandbox` (host) | ✅ **Required** — now BOTH configs | Top-level App Sandbox enable. `ENABLE_APP_SANDBOX = YES` in Debug **and Release** (Release flipped 2 Jul 2026, mirroring Debug's network/file settings + `ENABLE_HARDENED_RUNTIME = YES`). A non-sandboxed MAS upload is an `ITMS-90296` rejection. | "Standard App Sandbox enable for the host UI process. Sidecar inherits this scope via `com.apple.security.inherit`." |
...
**Summary.** Track C C0 established that **serve-only** needs just one Hardened Runtime exception (`disable-library-validation`). But the **run path** (transcription) re-adds two: `allow-jit` + `allow-unsigned-executable-memory` for numba/llvmlite (7 Jun 2026 crash fix). So the shipping sidecar carries **three** HR exceptions, not one — the C0 four-run table below is correct for its serve-only scope but does NOT reflect the run-path reality. All three are documented + App-Review-justified in the `desktop/bristlenose-sidecar.entitlements` header. See the next subsection for the C0 table, and §"Why DLV survives unified-identity signing" below for the framework-nested-seal finding.
