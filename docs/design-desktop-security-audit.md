...
| 2 | App Sandbox disabled in Release (`ENABLE_APP_SANDBOX = NO`) | 🟡 **Track A in progress** — Debug-only sandbox-on landed across A1 (29 Apr), A1c (30 Apr), A2 (1 May `49cb600`). Empirical Debug entitlement floor: `app-sandbox` + `network.client` + `network.server` + `files.user-selected.read-write` (host) + `inherit` + `cs.disable-library-validation` (sidecar). Beats 1 + 3 reachable end-to-end; beats 6+ gated by an unrelated stale `PipelineRunner.findBristlenoseBinary()` code path (not migrated to `SidecarMode.resolve()` when C1 landed). Release flip pending A4/A6 (file-access + doctor probes) and the PipelineRunner migration. See `design-desktop-python-runtime.md` §"Entitlement table" for the empirical justification text. | §12 Must |
...
3. Keychain — see the "Keychain credential storage" row in §What's Already Strong (one statement, not two; the pair drifted apart once)
