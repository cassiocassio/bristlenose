...
- _2026-09-03_ — **Truing pass (`--doc`): three claims in the body were describing code that had moved, and one of them had survived four previous passes.** (1) **§Preferences → serve process said `ServeManager` subscribes to `bristlenosePrefsChanged` and calls `restartIfRunning()`.** It has had no subscription of its own since `4ec46822` (19 Aug) — *two days before the 21 Aug pass, inside its own window.* `BristlenoseApp` registers the observer and hands it to `ServeFleet.applyEnvChange()`, which fans out per serve on `ServeEnvStaleness.action`. The load-bearing arm was absent entirely: a serve **exposed to agents** restarts eagerly even unfronted, because deferring it would let an external agent keep receiving real participant names after Anonymise was switched on — the reader who must not be stale is not the human. (2) **§Validation invariants printed the Ollama consent sequence backwards** — doc said `recordConsent` → `activeProvider` → post; shipped is `activeProvider` → `ollamaModel` → `syncGlobalModel` → `recordConsent` → post, and has been since `da612872` (4 May 2026). The headline invariant (consent before the post) was right, which is exactly why four passes read straight past the body: **a correct summary is camouflage for a wrong detail underneath it.** The *reason* for the real order is now recorded, because it is the thing that stops someone "fixing" it back — `recordConsent` reads `activeProvider` to write the audit trail. (3) **`overlayAPIKeys` was attributed to `ServeManager`**; it is `BristlenoseShared`, and the cited threat-model comment line was dead. That sentence is the immediate sibling of the one `fe145d32` corrected — the fix stopped one sentence short. Also: the refocus-recheck for credit top-ups was listed ⬜ unshipped when it ships on the out-of-credit pill and only the Settings board lacks it (now 🟡); `scripts/llm-weather.py` was named as a call site and does not exist; the out-of-credit pill no longer goes through the responder chain; the register cross-ref pointed at §3.7 rather than §5a-bis/ter/quater. **Same-day self-correction:** the 3 Sep entry below and its body bullet both said "growth is never deadbanded", which overstates the code — the growth arm is `fitting > current + refitNoise`, and this doc's own test `layoutNoiseNeverMovesTheWindow` pins a 0.2pt growth returning `nil`. Corrected in the doc, in both code comments, and in the test name. That is the 21 Aug lesson recurring inside its own file: a claim that reads as a guarantee it does not make. **Method note:** body `file.swift:NNN` anchors were converted to symbol names. Nine of them had rotted, most by this session's own `+66` lines in `LLMSettingsView.swift`, which makes line anchors a recurring cost rather than a one-off error. Changelog-entry anchors were left alone — they are dated records of what was true when written. Anchors: `BristlenoseApp.swift` (the observer), `ServeFleet.swift` (`applyEnvChange`), `AIConsentView.swift` (`installAndUse`), `BristlenoseShared.swift` (`overlayAPIKeys`), `OutOfCreditModel.swift`, `SettingsRefitTests.swift`.

- _2026-09-03_ — **The re-fit grew a shrink deadband, Reduce Motion, and its first master-detail caller.** `LLMSettingsView` pinned itself at `minHeight: 660` under a comment saying the floor let _"the tallest provider config (Azure…) open without a scrollbar"_. Measured through the same `fittingSize` the window reads (`SettingsRefitTests`, 3 Sep 2026): ChatGPT 484, Claude 499, Gemini 499, Ollama 501, **Azure 681**. The floor was wrong in both directions — 21pt too short for the one provider it was set for, and ~160pt too tall for the other four, which is the dead space under the Concurrency slider on the default provider. The 21pt is the part that bit: `fittingSize` reports `max(660, 681)`, so opening Settings **onto** Azure sized correctly while **switching** to Azure left it in a 660 window and compressed it — the same pane rendering differently depending on how you arrived, silently, exactly as the 21 Aug entry below predicts. Floor deleted; the pane now calls `refitToContent(shrinkThreshold:)` on a `heightSignature` covering provider, status, error text and the custom-model row. **The deadband is asymmetric, and the asymmetry is the whole design:** growth is never held back by the *threshold* — only by the half-point noise floor both directions share — because a pane wanting more room is compressed, not scrolled; shrinking has to be worth about two grouped-Form rows — so browsing Claude ↔ ChatGPT ↔ Gemini ↔ Ollama, a 17pt spread, never moves the window and Azure always does. Two movements per visit, both on a genuine change of shape. The reasoning is that switching tabs is *navigation* and a resize reads as arrival, whereas picking a row in a master-detail list is *browsing* — and a window that resizes on every click makes the side-by-side comparison the pane exists to support feel unstable. The decision is a pure function (`SettingsWindow.refitTarget`) precisely because both its arms return `fitting`, so a mis-edit would typecheck and ship; `growthIsNeverDeadbanded` fails against the inverted version (verified by mutation, not assumed). Also, **Reduce Motion is now honoured inside `refitToContent` rather than at its call sites, because both call sites had forgotten it** — MCP Agents gets the fix without touching it. Anchors: `SettingsView.swift` (`refitTarget`, `refitNoise`), `LLMSettingsView.swift` (`shrinkThreshold`, `heightSignature`, `refit()`), `SettingsRefitTests.swift`.
...
This never bit while every pane was fixed-height by construction. It bites the moment a pane's height depends on its own state — a picker that swaps content of different lengths, a table whose row count is data. Three consequences for pane authors:

- **A pane whose height varies with its own state must ask for a re-fit.** `SettingsWindow.refitToContent()` (`SettingsView.swift`) runs the same arithmetic on demand: `layoutSubtreeIfNeeded`, read `fittingSize`, set the frame from the top-left so the title bar stays put, with a half-point deadband so layout noise cannot animate the window. It is deliberately the package's own question asked again, not a reimplementation of layout — though the no-arg form is where that holds exactly; a caller passing a `shrinkThreshold` is asking a deliberately different question (see the next bullet). **Reduce Motion is applied inside it**, so a pane cannot forget it; both original call sites had.
- **A master-detail pane also needs a shrink deadband — `refitToContent(shrinkThreshold:)`.** Between tabs, exact fit in both directions is right: you went somewhere else, and the resize reads as arrival. Inside one pane, selecting a row is browsing, and a window that resizes by a row's worth on every click destabilises the comparison the list exists for. So shrinking is deadbanded to what the caller thinks is worth moving for (`LLMSettingsView.shrinkThreshold`, 60pt) while **growth is deadbanded only by layout noise, never by the threshold** — a pane that wants more room than the window has does not get a scrollbar, it gets compressed by the required constraints above. (Growth past `refitNoise`, not growth past zero: a sub-point increase is still refused, or the window would animate on every render pass.) A caller passing `0` (the default, and what MCP Agents wants) keeps exact fit both ways. **A grown window is deliberately not constrained to the screen** — `refitToContent` pins the top-left and moves the bottom edge, so growing near the foot of a display can push the frame partly off it. Left unguarded on 3 Sep 2026 as a considered call, not an oversight: it is already true of every tab switch, it has never been reported, and the guard would be code that earns its keep only in a case the user fixes by moving the window. Don't add it back without a report.
- **A pane's "visit" is the WINDOW being open, not the pane being on screen — `SettingsWindow.window`.** The package's tab transition swaps the pane underneath anything scoped to it, so per-visit state (MCP Agents' revocation receipts) keyed on the pane's lifetime silently resets when the user glances at another tab and comes back. Read the window instead. `SettingsView.swift` exposes it for exactly this; it is a Settings-window invariant, not an MCP one.
...
`SettingsWindow` (`SettingsView.swift`) owns the controller and wraps the six SwiftUI pane views (each with `.environmentObject(i18n)` + a `SettingsPaneChrome` modifier for live palette tint + appearance). `AppDelegate.showSettingsWindow(_:)` answers the one remaining responder-chain caller, the web bridge's "open-settings" (`BridgeHandler.swift`, via `NSApp.sendAction`); `CommandGroup(replacing: .appSettings)` provides the App-menu item + Cmd+,. Everything else deep-links straight to a pane through `SettingsWindow.shared.show(pane:)` — the welcome "Setup →" and the out-of-credit pill to `.llm` (`OutOfCreditPill.swift`), Bristlenose ▸ Connect an Agent… and the welcome screen to `.mcpAgents` (`MenuCommands.swift`, `WelcomeHomeView.swift`). **Namespace gotcha:** the package's `enum Settings` collides with SwiftUI's `struct Settings` (scene), so the pane types are reached through `PkgSettings` — a typealias defined in `SettingsPackageAlias.swift`, a deliberately SwiftUI-free file where `Settings` resolves unambiguously to the package.
...
- **Language** — `@AppStorage("language")`, 22 autonyms (never translated). `.onChange` calls `i18n.setLocale` and posts `.bristlenosePrefsChanged` to restart serve (`BRISTLENOSE_LANG`, set unconditionally by `childEnvironment` from `I18n.resolvedLocale`; it is the language the pipeline **generates** section and theme names in). It no longer drives `BRISTLENOSE_WHISPER_LANGUAGE` — the spoken language of a recording is a property of the recording, not of the reader, and deriving one from the other pinned Whisper to the interface language (`cca68462`, 22 Sep 2026).
...
## Tab 2: LLM Provider (brain) — Mail Accounts pattern
...
(the model), `AccountsSettingsView.swift` (the view), `SettingsView.swift` (the `.accounts` pane registration).
...
`docs/design-mcp-extension.md` — §3.7 for the entry point, and §5a-bis / §5a-ter / §5a-quater for the
projects register (badge, roll-up, receipt lifetime). Accounts, by contrast, had no home at all; its behavioural spec
is here and its per-platform sign-in flows are in `docs/design-cloud-import.md` §9.

## Preferences → serve process

`BristlenoseShared.overlayPreferences()` reads `UserDefaults` and injects values as environment variables into the `Process.environment` dictionary before launching `bristlenose serve`. **Don't-override-default guard**: `overlayPreferences` only emits an env var when the user has explicitly set the value (concurrency, for instance, only when the user has touched the slider). **`BRISTLENOSE_WHISPER_LANGUAGE` used to be the worked example here and is now the counter-example**: it was emitted when `lang != "en"`, which meant the guard was working exactly as described and still produced the wrong answer, because the value it was guarding came from the wrong axis. It is no longer emitted at all — `whisper_language` defaults to `"auto"` in `config.py`, so injecting nothing means per-file detection. This lets Python-side defaults stay authoritative when the user hasn't expressed a preference. See `BristlenoseShared.swift:221-287`.
...
**The eager arm is load-bearing and is why this is a fan-out rather than a loop.** A serve that is *exposed to agents* restarts immediately even when it is not fronted, because otherwise it would go on answering an external agent with real participant names after Anonymise had been switched on. Deferring that restart to the next time a human looks at the window is exactly the wrong trade — the reader in question is not the human.
...
| Language | `language` | `BRISTLENOSE_LANG` (via `childEnvironment`, not `overlayPreferences`) |
...
use the cache. The comment block above `applyPresenceAndCache` (`LLMSettingsView.swift`) is the
load-bearing record.
...
(`revalidateAzure`, `LLMSettingsView.swift`) is wired through `@FocusState` for the
...
**Consent recorded BEFORE the prefs notification on Ollama setup.** The
invariant that matters is `recordConsent` → post, so a prefs-change-driven serve
start cannot fire pre-consent. The consent gate downstream still catches it, but
the ordering is deliberate, not incidental.

The *writes* come first, though, and this doc had the sequence backwards from
`da612872` (4 May 2026) until 3 Sep — through four truing passes, because the
headline was right and only the body was wrong. Shipped order, when "Use Ollama
Instead" completes (`AIConsentView.swift`, `installAndUse`): `activeProvider` →
`ollamaModel` → `syncGlobalModel` → `recordConsent` → post → pull. The provider
write leads because **`recordConsent` reads `activeProvider` for the audit
trail** — record first and the trail names whatever was selected before.
...
>   `revalidateAllStale`, `LLMSettingsView.swift`); activation-acts-on-truth
>   via `canActivate` (`LLMSettingsView.swift`, `LLMProvider.canActivate`); the 402-masking split
>   into a sticky `.outOfCredit` (`LLMProvider.swift:238`).
> - ⬜ **Still design-forward:** the shared rung-3 `probe(provider, model)` unit,
>   and the offline three-bucket grey ("worked before" vs "never configured" vs
>   "not set up").
> - 🟡 **Half-shipped:** the `NSApplication.didBecomeActive` refocus-recheck for
>   credit top-ups exists — on the **out-of-credit pill** (`OutOfCreditModel.swift`),
>   which re-reads on return because a top-up happens in the browser and the fix
>   lands out-of-band. The **Settings board** does not do it; opening this pane
>   after topping up still waits on the 60s cache TTL.
>
...
(a provider × every-known-model sweep — neither the tool nor
`docs/design-llm-weather.md` is written; `scripts/llm-weather.py` is the proposed
name, not a path that exists) are **the same operation**:
