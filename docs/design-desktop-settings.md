...
- **A pane whose height varies with its own state must ask for a re-fit.** `SettingsWindow.refitToContent()` (`SettingsView.swift`) runs the same arithmetic on demand: `layoutSubtreeIfNeeded`, read `fittingSize`, set the frame from the top-left so the title bar stays put, with a half-point deadband so layout noise cannot animate the window. It is deliberately the package's own question asked again, not a reimplementation of layout — if the package ever grows a public re-fit, it becomes a one-line forward. **Reduce Motion is applied inside it**, so a pane cannot forget it; both original call sites had.
- **A master-detail pane also needs a shrink deadband — `refitToContent(shrinkThreshold:)`.** Between tabs, exact fit in both directions is right: you went somewhere else, and the resize reads as arrival. Inside one pane, selecting a row is browsing, and a window that resizes by a row's worth on every click destabilises the comparison the list exists for. So shrinking is deadbanded to what the caller thinks is worth moving for (`LLMSettingsView.shrinkThreshold`, 60pt) while **growth never is** — a pane that wants more room than the window has does not get a scrollbar, it gets compressed by the required constraints above. A caller passing `0` (the default, and what MCP Agents wants) keeps exact fit both ways.
