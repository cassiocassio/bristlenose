...
- **Appearance** — `@AppStorage("appearance")` drives `.preferredColorScheme` on the main window (a SwiftUI scene); the Settings window is AppKit-hosted, so it follows `window.appearance` instead — set in `SettingsWindow.show()` and re-applied live by `SettingsPaneChrome.onChange` when the pref changes (see 31 Jul changelog entry). Synced to the web layer via `BridgeHandler.syncAppearance()` on `ready` (native wins; the web Settings modal hides its own appearance picker in embedded mode).
