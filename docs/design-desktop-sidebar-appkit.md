...
- **Phase B — drag, working.** The unified `NSOutlineViewDataSource` insertion-point model so drag **between folders and to the top level** actually works (the currently-broken "apocalypse" cases — a *required* outcome, not a bonus). The data source written for A *is* the drag model, so it's built once. Gated by the `DropRouting.resolve(...)` table test (§5).
  - **Within-scope manual reorder (`toIndex`) — SHIPPED** (`bbda9a6a`, 1 Aug 2026), exactly as the deferral below described it: `position` renumbered on drop, rows animating into place via `moveItem(at:inParent:to:inParent:)`. Root ordering stayed **interleaved** — a folder is another row in the root sequence, sharing one `position` space with root projects — and folders became draggable (`OutlineNode.dragItem`), so the payload is kind-tagged (`"project:<uuid>"` / `"folder:<uuid>"`) and a mixed multi-select drag needs one reader, not two. One `DropPlan` replaced `[ProjectMove]`: the items land as a contiguous run, so a per-item index was redundant state. `DropRouting.reordered(scope:inserting:at:)` is the shared arithmetic — the model renumbers from it and the animation reads each row's final index from it, so they agree by construction rather than as two copies. `moveProject` (the Move-To menu) routes through the same `ProjectIndex.apply(_:)`, which closed a real defect: it used to set `folderId` and leave `position`, so a project leaving a folder carried its in-folder index into root and collided there — and `sidebarItems`' sort has no tiebreak, so tied rows could swap places between renders.
    - _Superseded, kept for the delta —_ **the 22 Jun deferral:** *"Dragging a project/folder to a new position within its scope, with the other rows animating into place, Finder-style. Today sort order is the `position` Int … effectively **fixed at creation** — `DropRouting` computes `toIndex` (unit-tested) but `acceptDrop` wires only `toFolder`, `moveProject` leaves `position` untouched, and the SwiftUI `.onMove` reorder was not ported. To deliver: renumber `position` on drop + animated `moveItem(at:to:)` on the outline. Not in the parity push."*
    - **Ordering is manual, and stays manual.** A curated list gets manual order (Finder Favourites, Xcode's navigator); a derived one gets sorted order (Notes folders are alphabetical). This sidebar is curated — projects arrive by dropping a folder — so a later "sort by last run" default would fight the user's arrangement every time a run finished. New projects still land at `position` 0 and push the rest down; that composes with manual order, an auto-sort would not.
    - **Undo is not wired** — see [`design-undo-debt.md`](design-undo-debt.md). The gesture is live, so this is live debt now, not a pre-condition.
