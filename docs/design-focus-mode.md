...
**Extended to the Signals lens, 21 Sep 2026.** Focus is entered only on Quotes, but the class lives on `<html>` and leaks onto Signals as an undesigned half-state. [`design-focus-mode-signals.md`](design-focus-mode-signals.md) applies the two axes below to the signal card and ungates `z` and the View-menu item there; mockup `docs/mockups/signals-focus-mode.html`. **Built 21 Sep 2026** — Focus is live on Quotes and Signals; the dimmed-elsewhere rule for the menu item and the key's route gate name the same two lenses.
...
So: **web** registers bare `z` in `useKeyboardShortcuts.ts` (behind the existing `isEditing()` guard), passing through identically in browser and WKWebView; **native** binds `⌘⌥F` on the View ▸ Focus Mode item, dispatching `menuAction("focusMode")` over the bridge (~~mirrors `⌘F` → `menuAction("find")`~~ — **that precedent is gone as of 12 Sep 2026:** ⌘F is now native end-to-end and crosses no bridge, because `menuAction("find")` reached a handler that queried an element embedded mode never renders. The reasoning below still holds on its own terms; do not copy the ⌘F wiring). They need not match. The webview holds the state and reports back so the checkmark stays honest. The CLI gets **no `run` flag** — a flag mutating a downstream viewer's state is the `--static`/`--no-serve` conflation again; its only honest contribution is the boot default, a shared Settings key.
...
**Lens scoping — built quotes-only, provisionally.** Focus's entire signal/noise vocabulary is quote-card vocabulary, so all three affordances are scoped to the Quotes lens and agree with each other: the menu item is `.disabled(activeTab != .quotes)` matching its neighbours, `z` is route-guarded to `/report/quotes` the way `m` is to `/report/signals`, and the toolbar mounts only in `QuotesTab` so the moon button can't appear elsewhere. All three had to move together — a menu that says "unavailable" while the key still works is worse than either behaviour alone. **Widened to Quotes + Signals on 21 Sep 2026** once the Signals lens had a transform of its own ([`design-focus-mode-signals.md`](design-focus-mode-signals.md)): the menu's lens set is `ViewMenuContent.focusModeTabs`, the key's route gate lists the same two, and the toolbar is unchanged — it is still Quotes-only because everything else in it is.
