...
input for *how much a translation is read*; it was never a survey of which
exist.
...
1. **The private key first, then macOS.** `I18n.configure` (`I18n.swift:62-88`) reads
   ...
   forPreferences: Locale.preferredLanguages)`. So a **fresh install follows the OS**
   (measured: `fr`→`fr`, `nn-NO`→`nb`, `zh-Hans`→`en`), and since 22 Sep 2026 that is
   true of the *report* too — `BridgeHandler`, `ContentView`'s `?locale=` param and the
   sidecar env all read the one resolver, where two of them previously read the raw key
   with their own `?? "en"` and gave a French Mac French menus around an English report.
   `forPreferences:` is passed explicitly because `nil` latches the language list on the
   first call in the process. An install that has used the picker follows the
...
1. **Incomplete string audit upfront** — missed SettingsModal (separate component from SettingsPanel), CodebookSidebar headings, SignalsSidebar headings, SidebarLayout "Contents" title, "Browse codebooks" button. Each required a QA cycle to discover. A `grep -r '"[A-Z][a-z]' --include='*.tsx' frontend/src/` upfront would have caught them
