...
| D7 | **Formalise the "section header + divider" pattern** | The section-divider rule is hand-rolled ~15 times (every `*-header` class) with no shared atom/molecule — only `--bn-colour-border` is common. Two divergent constructs: (a) border welded onto the base `h2`, which sits under the 52rem `max-width` prose cap → **rule truncates mid-pane** (was the case in the Quotes + Analysis lenses); (b) border on a full-width container with the title as a child, un-capped → spans the pane (the codebook lens's `.framework-section-header`). Weight is also undecided: base `h2` = **2px**, codebook = **1px**. **Low-risk slice done (17 Jul 2026):** added `.section-heading { max-width: none; border-bottom-width: 1px }` in `report.css` and tagged the Quotes-lens h2s (`QuoteSections.tsx` / `QuoteThemes.tsx`) with it, so both lenses now match codebook (full-width, 1px). **Deferred big refactor:** a real `.bn-section-header` molecule that codebook + lenses + the other `*-header` sites all consume, plus a canonical divider token (`--bn-divider-width` / hairline atom) to settle 1px-vs-2px once. Scoped, so the base-`h2` 2px capped treatment still exists elsewhere (About, etc.) — deliberate transitional inconsistency until the molecule lands. Sibling of D2. | S (done) / L (refactor) | Medium |
...
- ~~A1: Break up `render_html.py`~~ DONE (Mar 2026)
