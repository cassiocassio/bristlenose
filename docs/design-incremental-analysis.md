...
**Sibling docs:** `docs/methodology/tag-rejections-are-great.md`, `docs/methodology/consent-gradient.md`, `docs/design-research-methodology.md`, `docs/design-signals-future.md`.
...
> **Empirical note (Phase 2 experiment, 4 Jul 2026).** Holding a fixed quote set and re-running only the grouping stages, with and without a deliberate topic-outlier interview, confirms the two-level distinction is real, not just a design intuition. *(A statistical summary only; the experiment harness, corpora, and real labels are kept out of the public tree.)*
> - **Sections behave as durable sub-clusters.** Run-to-run noise floor at fixed quotes is ARI 0.956; adding the outlier barely perturbs the existing sections (disturbance ARI 0.917, gap +0.04). The outlier *appends* its own section; the existing ones hold. So a re-run should treat section membership as stable and only surface genuine new-material sections.
> - **Themes behave as editorial labels that drift.** The outlier's arrival silently re-labelled the existing navigation theme (same underlying quotes, new wording) and added a theme of its own. This is the silent-rename footgun made concrete from adding a *single* interview — direct evidence for the rule that theme labels must be human-committed and never overwritten on re-run.
> - **Section churn is mostly inherited from quote drift, not generated by grouping.** At fixed quotes the *section* layer is near-deterministic (section floor 0.956 on the e-commerce corpus), so section instability a researcher sees comes mostly from quote-extraction drift propagating upward — stabilise the quote layer (union/split-crediting merge) and sections follow. **Themes are a different story — that corpus's theme layer was too thin (8 quotes) to test, so its floor of 1.00 was an artefact. See FOSSDA below.**

> **FOSSDA cross-check (4 Jul 2026 — resolves the 8-quote caveat, and inverts the reassuring reading).** FOSSDA is 10 open-source oral-history interviews: theme-heavy, section-light (273 general-context quotes → a reportable 12+ theme layer; only 11 screen-specific). Re-theming the *same* fixed 273 quotes 5× (`cluster_repeat.py`, quotes held constant):
> - **Theme grouping is substantially NON-deterministic at reportable scale.** Theme counts came out 13 / 20 / 20 / 15 / 14 across five runs on identical input, with a run-to-run floor **ARI 0.425** (vs the e-commerce corpus's artefactual 1.00 on 8 quotes). Same quotes, materially different theme structure every run — not just labels, *membership*.
> - **Two tangled forces:** (1) genuine membership churn; (2) unconstrained granularity — neither the clustering nor the theming prompt carries a count target (the known count-target gap noted earlier in this doc), so the model returns 13 themes one run and 24 the next. Enforcing the reportable 7–12 band would likely tame force (2); worth testing directly.
> - **The incremental signal is swamped.** Dropping the two smallest interviews reshuffled the remaining themes no more than a plain re-run on identical data did (disturbance ARI 0.384 ≈ floor 0.425). At the theme layer the run-to-run noise is *larger* than any "did new data change the structure" signal — so you cannot detect incremental theme drift by re-running and diffing.
>
> **What FOSSDA changes in the design:**
> - Hardens the editorial-theme rule from "don't overwrite labels" to **"never regenerate theme membership wholesale on a re-run."** Regeneration is chaotic at scale, so the researcher's committed theme structure must be persisted and *extended*, never recomputed. This is the strongest evidence yet for the analytical/presentation split.
> - **Moves the saturation-proxy governor off the theme layer.** The [ratio-rule demotion](#researchers-mental-model-of-fluidity) proposed gating lock/unlock on "new themes / orphan-candidates per added session" — but theme count is ±40% noise on *identical* data, so that signal is unusable at the theme layer. The proxy must live at the stable quote/code layer, not the theme layer.
> - Sections stay durable in both corpora (e-commerce corpus 0.956, FOSSDA 1.0 on its thin section layer) — the section side of the two-layer model holds.
> - **Local-model footnote (for completeness):** the default Local path (`llama3.2:3b`) can't even produce a usable quote set on this data — 3 of 4 interviews in the e-commerce corpus failed the structured-quote schema, ~6.5 min/run. The stability questions above are all *downstream* of a quote-quality floor small local models don't clear; they presuppose a capable extractor (cloud, or a much larger local model — untested).
...
> **Refinement forced by the 3–4 Jul 2026 experiment (below): the key must credit SPLITS, not demand a single match.** "A single re-extracted quote covers ≥70% of the starred span" recovers only ~81% of stars — below the ≥90% target. "The starred span is ≥70% covered by the *union* of the session's new quotes, star carried to the best-overlapping one(s)" recovers 94.6%. The primitive is right; the matching rule must be union/split-crediting, because the dominant failure mode is a quote splitting into two shorter ones, each individually under threshold.
...
**Validation — RUN 3–4 Jul 2026 on a private e-commerce usability study (4 interviews, 10 independent re-extractions, `claude-sonnet-4` temp 0.1). Result: the naive heuristic FAILS the ≥90% target; the union refinement MEETS it.** Findings (statistical summary; harness kept out of the public tree):

- **Single best-overlap match ≥70% → 80.9–83.5% recovery** (worst run 76.7%). Below target; ~1 in 5 stars at risk. This is the empirical refutation of the naive reading.
- **Union / split-crediting ≥70% → 94.6%** (worst run 88.4%, **zero fragile quotes**). Meets the target. Hence the merge-rule refinement flagged above.
- **The typical quote is bit-identical across re-runs** — median boundary drift 0.0s at both ends, median text-similarity 1.00, 34 of 43 quotes recovered in *all* 9 re-runs. Instability is a thin tail, not uniform jitter — so "quotes are stable enough to carry stars" is broadly true; the risk is concentrated.
- **~4 of 43 quotes (~9%) are genuinely fragile** — the model omits them entirely in some runs, so *no* boundary rule recovers a star pinned to them. These need the honest caveat below, or a sturdier anchor (pin the star to a transcript `segment_index` / char-span, not the extracted quote).
- **3.1% of timecode matches land on different words** → the embedding/text tiebreaker (below) is confirmed as a precision guard, at ~3 points of recall cost.
- **Per-session extraction independence confirmed** — the deliberately off-topic outlier interview's own quotes re-extract as stably as the on-topic interviews'; a quote's boundaries depend only on its own transcript, so batch composition / processing order don't perturb them.

Caveat on generality: one project, one model, a single reference run; the *direction* (typical quote bit-stable, loss concentrated in a tail, union clears 90% where single-match doesn't) transfers; the exact 94.6% is not a universal constant. Re-run on a second corpus before hard-coding thresholds.
