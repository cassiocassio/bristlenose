...
**Why shipped baselines are safe here.** Prompts and shipped baselines move together — a release that bumps `prompt_version` also re-runs the corpus and ships an updated `cohort-baselines.json`. The user always receives the prompt version and its calibration atomically; the "theme → themes" trap can't bite a user, only a maintainer who forgot to re-baseline before tagging.

**That safety is asserted, not enforced — read the previous paragraph as a design intent, not a description.** The CI gate named in §Implementation does not exist, and the coupling it describes has never held: the baselines were empty for five months across many prompt versions, and nothing said so. Until the gate is built, "prompts and baselines move together" is a sentence in a document.
...
- **Shipped baselines.** `bristlenose/llm/cohort-baselines.json` covering cloud cohorts only (Anthropic/OpenAI/Azure/Gemini at families known at release time). On-device cohorts get `basis="none"` until local data accumulates. **Shipped empty and stayed empty 2026-04-28 → 2026-09-21**, which disabled the forecast for every user rather than degrading it — a new user has no local rows by definition, so the shipped baseline is the only step of the resolution order that can answer them. Populated 2026-09-21 with 15 rows across `claude-sonnet/4`, `gpt-4o/4` and a pooled `("*", "*")` fallback.
- **Release ritual.** `scripts/regenerate-cohort-baselines.py` reads a maintainer's JSONL corpus and writes the JSON — **exists as of 2026-09-21**; run it as `.venv/bin/python scripts/regenerate-cohort-baselines.py trial-runs/`. It emits aggregates only and asserts before writing that no session id, run id, timestamp or prompt sha reached the output, because its inputs are re-identification keys and its output is committed. **The CI gate is still unbuilt**: nothing fails when `prompts.lock` and `cohort-baselines.json` drift, so re-running the script after a prompt reword remains a habit rather than a rule. That is the same shape as the gap this section's own baselines line describes — an obligation recorded in prose and enforced by nothing.
