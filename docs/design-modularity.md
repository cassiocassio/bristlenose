...
- _2026-09-20_ — struck four packaging lines that still told the reader presidio and spaCy are excluded from the sidecar. They have been bundled since 12 Sep 2026; only the `en_core_web_lg` weights are excluded, as data. The §PII-removal row and the Open-questions entry had already been trued in "pii: stage 7 abandons on any failure…"; the packaging decision bullet and the three Order-of-work items had not, so item 4 contradicted the Open question directly above it. Anchors: `desktop/bristlenose-sidecar.spec:248-258`; `scripts/project.conf:117`; `docs/design-redact-pii.md`.
...
- ~~Exclude `presidio-*` + `spacy` from Mac sidecar PyInstaller spec for alpha (bundle them), and for public beta move to Non-Essential Background Asset~~ **SETTLED 12 Sep 2026 — the code is bundled on every channel, permanently.** The bullet was self-contradictory as written ("exclude … (bundle them)") and its public-beta half is retired: what moves to Background Assets is the **model**, not the code. presidio + spaCy measured 33 MB in the built sidecar against the ~100 MB this planning assumed, so bundling is cheaper than the startup hook it would have needed. `bristlenose-sidecar.spec:248`
...
**Order of work:**
1. (Track C C1) Write PyInstaller spec that includes only `[apple]`-style transcription, ~~excludes presidio/spaCy~~ (**presidio and spaCy are bundled as of 12 Sep 2026** — only `en_core_web_lg` is excluded) — targeted bundle ≤ 200 MB before Whisper. **Reality check (C0, 18 Apr 2026):** the shipped spec landed at **644 MB**, not 200 MB — torch (288 MB), llvmlite (110 MB), onnxruntime (58 MB), and scipy (37 MB) came in as transitive pulls via `tiktoken`/`transformers`/`numba`/`librosa`. The gap is absorbed by the trickle-to-full-capability strategy below (Background Assets for Whisper models + future optional deps) rather than by build-time trimming. Revisit if TestFlight reports cite install size as friction; for alpha, the bundle ships as-is. See `design-desktop-python-runtime.md` §"Bundle-size findings" for the transitive-pull table.
2. ~~(Track C C1) Write `bristlenose-sidecar[pii]` asset pack for Background Assets — deferred to public beta.~~ **Superseded 12 Sep 2026.** There is no code asset pack; the pack is the **model** (`en_core_web_lg`), and its delivery is specified in [design-redact-pii.md](design-redact-pii.md) §"Delivery architecture". As of 20 Sep 2026 the pack is designed and **not yet hosted** (`PII_PACK_URL=""`, `scripts/project.conf:117`).
...
4. (post-100-days) ~~Implement runtime `sys.path` extension for downloaded PII asset pack on Mac.~~ **RETIRED 12 Sep 2026** — the same retirement recorded under Open questions below, which this line went on contradicting. Bundled code needs no `sys.path` hook.
