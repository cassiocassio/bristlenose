...
- _2026-05-07_ — **`pipeline-summary-events` branch — failure-taxonomy Phase A.** Adds structured per-stage failure summaries to terminus events, an explicit abandon path so empty-data runs no longer render a fake-empty report, and two new `Cause.category` values (`MISSING_INPUT`, `MISSING_BINARY`) plus a routing rule for bare-name `FileNotFoundError`. Trigger: 7 May 2026 desktop sandbox walk where every `s05_transcribe` session failed (`[Errno 2] 'ffmpeg'`), the pipeline silently proceeded through s06–s12, and the user's seat showed "Done. 0 sessions / 0 quotes" with no diagnostic except 1346 lines of `bristlenose.log`.
    - **New event-payload field.** `_TerminusEvent.summary: PipelineSummary | None` — added to all three terminus events. `PipelineSummary` carries three optional `StageOutcome` slots (`transcripts`, `quotes`, `themes`); each `StageOutcome` is `(attempted, succeeded, failed: list[StageFailure])`. Backward-compatible decode: legacy lines without the field deserialise with `summary=None`.
    - **`PipelineAbandonedError`** — new exception in `bristlenose/events.py`, carries its own `Cause` + partial `PipelineSummary`. The `run_lifecycle` context manager catches it ahead of the generic `BaseException` branch, writes a `RunFailedEvent` whose `cause` is the exception's (not categorised from `str(exc)`) and whose `summary` is the exception's payload. Crucially, the abandon path **does not** call `s12_render_output` — no report file appears on disk for the empty-data case.
    - **Three abandon checkpoints in `pipeline.py`** (run path):
        1. After s05_transcribe — every session failed → abandon with dominant `MISSING_BINARY` / `WHISPER` cause.
        2. After s09_quote_extraction — every LLM call failed → abandon with dominant `AUTH` / `QUOTA` / `NETWORK` / `API_SERVER` cause.
        3. (Soft) After s11_thematic_grouping — failure here records into the summary but never abandons; quotes alone are useful research output.
    - **`run_transcription_only` also abandons** on both `s01_ingest` returning empty (now `MISSING_INPUT`, was `_empty_result`) and on every-session-fails-transcription (mirror of the run-path check). `run_analysis_only` only has the s09 check (no transcription).
    - **`Pipeline._summary` accumulator.** Per-stage outcomes attach as stages finish; `PipelineResult.summary` carries it back to the CLI; `RunHandle.set_summary(...)` mirrors the existing `set_cost(...)` pattern (rejected the handoff's `summary_provider` callable as more plumbing for the same outcome).
    - **`_dominant_cause` helper.** Picks the most common `Cause.category` across failures; on tie, prefers non-retryable in priority order `AUTH > MISSING_BINARY > MISSING_DEP > MISSING_INPUT > DISK`. Single chosen `Cause` retains its provenance (stage / session_id) but the message is overridden with the abandon-level summary.
    - **Categoriser extension.** `categorise_exception` now recognises bare-name `FileNotFoundError` (filename without a path separator) as `MISSING_BINARY` — the today's-ffmpeg-under-sandbox bug pattern. Slash-containing filenames fall through (genuine missing data file). Round-3's "FileNotFoundError → UNKNOWN" rule is preserved for the slash case; the bare-name shortcut is additive.
    - **Stage-side refactor.** `s05_transcribe.transcribe_sessions`, `s09_quote_extraction.extract_quotes`, and `s11_thematic_grouping.group_by_theme` now return `(results, StageOutcome)` tuples. Per-session failures are categorised and accumulated; existing `errors: list[str]` parameter on s09 is preserved for legacy callers but mirrored into the structured outcome.
    - **Schema-coupled with the Swift `pipeline-diagnostic-pill` branch but NOT runtime-coupled.** Until the desktop consumer ships, the richer payload is silently ignored by `EventLogReader` (which uses `decodeIfPresent` for the new fields). Real E2E user-visible verification needs both branches on main.
    - **Tests:** new `tests/test_pipeline_abandon.py` (6 cases — `_dominant_cause` × 3, `run_lifecycle` abandon × 2, s05 outcome surfacing × 1); extends `tests/test_run_lifecycle.py` with `MISSING_BINARY` rules (bare / slash / no-filename); extends `tests/test_events.py` with `PipelineSummary` round-trip + legacy-decode-without-summary backward-compat.
    - **Won't do in this branch:** Swift side (Branch 2 `pipeline-diagnostic-pill`); per-session retry CLI (`--only-sessions s2,s3` — parent plan tracks as future); cost-estimation for retries.
...
| `out_of_credit` | `failed` | **no — top up first** | Billing exhausted. Terminal until the user acts out-of-band. Anthropic serves it as a **400** + "credit balance is too low" (not a 402); OpenAI as a **429** with `code=insufficient_quota`. |
| `quota` | `failed` | yes — transient, wait and retry | **Rate limit / throttling only.** Azure quota walls and Gemini `RESOURCE_EXHAUSTED` land here too — their wire doesn't isolate billing from rate, so we don't claim a top-up story they didn't tell us. |
| `api_request` | `failed` | sometimes (depends on `code`) | Provider rejected request (4xx other than auth/credit/quota) |
| `api_server` | `failed` | yes — usually transient | Provider 5xx / overloaded |
...
**Cross-boundary naming.** Python [`CauseCategoryEnum`](bristlenose/events.py:62) and Swift [`PipelineFailureCategory`](desktop/Bristlenose/Bristlenose/PipelineRunner.swift:9) use the **same snake_case raw values** so JSON round-trips losslessly through the events log. Python now carries **14** cases: original 6 (`auth | network | quota | disk | whisper | unknown`), the Phase 1f-added 4 (`user_signal | api_request | api_server | missing_dep`), 3 shipped since (`missing_input | missing_binary | output_truncated`), and `out_of_credit` (Jul 2026, splitting billing exhaustion out of `quota`). The table above is now complete against `events.py` — but see the two-mirror warning below before trusting any Swift-side parity claim.

> **⚠ There are TWO Swift mirrors, not one — and they drift independently.**
> `PipelineSummary.swift::CauseCategory` **decodes `cause.category` off the
> wire**; `PipelineRunner.swift::PipelineFailureCategory` drives the row summary
> and popover label. Adding a category to Python + only one mirror compiles
> clean (the exhaustive switches live on the *other* enum) while leaving the
> decoder unable to parse the new raw value — and `CauseCategory` is a plain
> `String, Codable` with no custom `init(from:)`, so one unknown value fails the
> **whole** `PipelineSummary` decode rather than degrading to `.unknown`. That
> is exactly what happened when `out_of_credit` was added (caught in the Jul
> 2026 truing pass, before it shipped). Update **both**, plus `pillPrecedence`
> and the pill locale keys — the five-site checklist lives in
> [design-pipeline-diagnostic-popover.md](design-pipeline-diagnostic-popover.md)
> § "read this before adding a new error". The forward-compat question parked
> below ("decode unknown category as `.unknown`") would have made this a
> non-event; it's now the obvious next hardening. Swift uses `camelCase` Swift names with explicit `String` rawValues (`case userSignal = "user_signal"`) so the language conventions hold on both sides while the wire format stays canonical. No existing case is renamed; pre-pivot drafts proposed renames based on a stale snapshot of the Swift enum and were rejected. Adding a new category requires coordinated changes on both sides (Python first, Swift in the next desktop release); the design doc currently treats this contract as stable, but a "decode unknown category as `.unknown` for forward-compat" question is parked (see round-3 changelog).
...
- **Durability guarantee.** macOS `os.fsync()` is *not* `F_FULLFSYNC` — survives process crashes, not power loss. Documented contract: Phase 1f / 4a-pre survive process crashes only. Full power-loss durability is not promised. Revisit if a user reports an unrecoverable crash on a UPS-less laptop.
  - **Update (Jun 2026): a power-cut-truncated *intermediate cache* no longer crashes the resume.** The fsync contract above is unchanged, but the failure mode it left open — a half-written `extracted_quotes.json` / `session_segments.json` / `speaker-info/{sid}.json` / `topic_boundaries.json` killing the *next* run with `JSONDecodeError` — is now closed on the per-session resume path. Phase 2b's SHA-256 verification only guards the full-cache (`COMPLETE`-stage) read; the per-session reads execute while the stage is `RUNNING`, so `_is_stage_verified` returns at the status check before hashing. Those reads now go through `pipeline._load_cached_json()`, which treats an unreadable/truncated file as a cache miss (returns `None`) and the call site recomputes the affected sessions rather than crashing or silently dropping them. So a hard power-off mid-write degrades to "re-run the in-flight stage," matching the user's expectation that the appliance picks up where it left off. Residuals (accepted): the `content_hash=None` legacy-manifest full-cache read and the `run_analysis_only`/render read path are still raw `json.loads`.
...
- `bristlenose/run_lifecycle.py` — `run_lifecycle(output_dir, kind)` context manager wraps each CLI command body. On entry: writes Python-side PID file `<output_dir>/.bristlenose/run.pid` (mode 0o600, `O_NOFOLLOW`) with `(pid, start_time, run_id)`; appends `RunStartedEvent` to `pipeline-events.jsonl` (also 0o600, `O_NOFOLLOW`); installs SIGINT/SIGTERM handler that records the signal number and re-raises `KeyboardInterrupt`. On clean exit: appends `RunCompletedEvent` and removes PID file via `try/finally`. On `KeyboardInterrupt`: appends `RunCancelledEvent` with `Cause(category=user_signal, signal=..., signal_name=...)`. On other exceptions: `categorise_exception(exc, *, provider=None)` produces a `Cause` and appends `RunFailedEvent`. Order: `isinstance` special-cases first (`ImportError`/`ModuleNotFoundError` → `MISSING_DEP`; `OSError(errno=ENOSPC)` → `DISK`; bare-name `FileNotFoundError` → `MISSING_BINARY`); then — **only when the provider is known** (an LLM call site via `_build_cause`) — the shared classifier `bristlenose/llm/failure_classifier.py::classify_exception`, which reads the provider's structured error code + HTTP status and is the thing that splits `out_of_credit` from `quota`; then word-boundary regex text fallbacks for LLM errors that bubbled up as bare strings; everything else falls through to `UNKNOWN`. The provider guard is load-bearing: without it a non-LLM exception carrying an HTTP-status attribute (a huggingface 404 during a Whisper download) gets mislabelled `api_request` and bypasses the s05 whisper fallback. Stale-running recovery: `_reconcile_stranded_run` checks the events log tail; if it's a `run_started` with no terminus, appends a synthesised `RunFailedEvent(cause.category=unknown, cause.message="Analysis stopped unexpectedly.")` before starting the new run. Concurrent-run refusal: `ConcurrentRunError` raised when an existing PID file matches a live `(pid, start_time)` pair (verified via `kill(pid, 0)` + `/bin/ps -o lstart=`); cli.py converts to `typer.Exit(1)` with a clear message. The PID file is **always** removed via the context manager's `try/finally`; the spec's `atexit` hook was removed in round-3 review (unbounded list growth on re-entry, redundant with `finally`).
...
- **`TQDM_DISABLE` env var must be set before any tqdm import**: tqdm's `@envwrap("TQDM_", …)` decorator reads the var at each `tqdm()` instantiation, but only catches calls that don't pass an explicit `disable=` kwarg. Set in `bristlenose/__init__.py` (was previously in `pipeline.py`, which was too late — see below).
- **`HF_HUB_DISABLE_PROGRESS_BARS` env var is read at `huggingface_hub` import time** (in `constants.py`): if `huggingface_hub` was already imported before the env var is set, `HF_HUB_DISABLE_PROGRESS_BARS` freezes as `None` and the env var has no effect. The doctor preflight (`bristlenose.doctor._check_whisper_model`) imports `huggingface_hub` via `scan_cache_dir`, which runs **before** `pipeline.py` loads. Original placement in `pipeline.py:14-15` therefore leaked `Fetching N files:` and `Download complete: : 0.00B` lines from `snapshot_download` during the cached-files path (BUG hit 12 May 2026 on an A4 happy-path run). Fix: env-var setdefaults live at the top of `bristlenose/__init__.py` — before any other import the package performs — so the var is in `os.environ` before any caller (including the doctor preflight) can transitively import `huggingface_hub`.
- **Belt-and-suspenders programmatic disable**: any code path that calls a `huggingface_hub` API which prints progress (currently `snapshot_download` in `_init_mlx_backend` and `preflight_whisper`) also calls `disable_progress_bars()` from `huggingface_hub.utils` right before the call. This protects against (a) future dep updates that introduce a new env-var bypass, and (b) edge cases where something imports HF before bristlenose. Either suppression alone is sufficient on the happy path; both protect the unhappy paths.
...
- **Regression coverage**: `tests/test_preflight_whisper.py::TestProgressBarSuppression` — three assertions: env vars set at `import bristlenose`, HF constant resolves to `True` after the full preflight import chain, and an instantiated `hf_tqdm` has `disable=True`. Catches re-introduction by dep update or re-ordering of imports in `bristlenose/__init__.py`
