...
> **Status**: Phase 0–1e implemented (crash recovery, per-session caching, `bristlenose status` command, pre-run resume summary); Phase 2a implemented (SHA-256 content hashes on stage outputs stored in manifest); Phase 2b implemented (verify hashes on load — corrupted/tampered files trigger re-run instead of silent use); Phase 2c implemented (input change detection — source file metadata hashing via size+mtime, upstream content_hash propagation, cascade invalidation); **Phase 1f + 4a-pre drafted 2026-04-25 — design only, not shipped** (run-level outcome + structured cause + estimated cost in a single-source append-only `pipeline-events.jsonl` event log; manifest unchanged); Phase 3 (reset command) next
...
> **Status:** design draft (2026-04-25). Not shipped. Replaces the implicit `parseManifest` inference (Swift desktop) with an explicit on-disk record of how each run ended. Closes the desktop-consumer drift documented in the status header above.
...
**Message size cap.** `message` is capped at 4 KB at the writer. Single event lines must fit in Swift's existing `readLogTail` 64 KB read window ([PipelineRunner.swift:570](desktop/Bristlenose/Bristlenose/PipelineRunner.swift:570)) so the line isn't truncated mid-string and silently discarded as malformed JSON. 4 KB is also under the macOS POSIX `PIPE_BUF` (512 B) × 8 safety margin and well within `O_APPEND` atomicity bounds for regular files on Linux ext4 / macOS APFS (where the kernel-level vnode lock guarantees append-write integrity at any reasonable size, but explicit cap protects the more-stringent macOS interleave guarantee under concurrent writers).
...
**Cross-boundary naming.** The Python `CauseCategoryEnum` and the shipped Swift `PipelineFailureCategory` ([PipelineRunner.swift:9-16](desktop/Bristlenose/Bristlenose/PipelineRunner.swift:9)) use the **same case names** so there's no rename across the boundary. The shipped Swift enum is `auth | network | quota | disk | whisper | unknown` — Phase 1f extends both sides additively to add `user_signal`, `api_request`, `api_server`, `missing_dep`. No existing case is renamed; `disk` does not become `disk_full`, `auth` does not become `api_auth`. Pre-pivot drafts of this doc proposed renames; that proposal was based on a stale snapshot of the Swift enum and is rejected here.
...
**Concurrency.** Two CLI processes against the same project race on the event log. Append-only JSONL handles concurrent appends safely on regular files under POSIX `O_APPEND`: the kernel-level vnode lock (Linux ext4 `i_rwsem`, macOS APFS) guarantees seek-to-end-and-write as one atomic unit per `write()` call. Single-event lines are well under any practical buffer size with the 4 KB `cause.message` cap. (PIPE_BUF is sometimes cited here — that's the *pipe / FIFO* guarantee, not the regular-file guarantee, and on macOS it's only 512 B. Regular-file `O_APPEND` is stronger than PIPE_BUF on both platforms.) Both processes can append without corrupting the file. But that does **not** make running two pipelines simultaneously a good idea (they'd race on the same per-stage manifest, the same SQLite DB, the same output directory).
...
> **Status:** design draft (2026-04-25). Replaces the inference in Swift `PipelineRunner.parseManifest` (`PipelineRunner.swift:534-583`) with a tail-read of the events log once Phase 1f ships.
...
**Status: not shipped — design draft 2026-04-25.**

**What it is**: Add an append-only `pipeline-events.jsonl` file recording how each run ended (kind / outcome / structured cause / token counts / estimated cost). Plus a `SessionRecord` extension for per-session token + estimated cost. Replaces the inference path the Swift desktop currently uses (which mis-classifies interrupted runs as `.ready`). The event log is the single source of truth for run-level state; the manifest stays focused on per-stage resume. Schema and lifecycle defined in §"Run outcomes and intent" above; envelope and event-line shape in §"Phase 4a-pre" below.
...
- `bristlenose/pipeline.py` — call `events.append_event(RunStartedEvent(...))` at the top of each command's body, write Python-side PID file `<output_dir>/.bristlenose/run.pid` with `(pid, start_time)`. Call `RunCompletedEvent` on the success path (clean up PID file). Top-level try/except wrapping the body builds a structured `Cause` and appends `RunFailedEvent`. SIGINT/SIGTERM handler sets a module-level cancel flag (captures signal number); main loop checks the flag at safe break points (between stages, sessions, LLM calls) and from there appends `RunCancelledEvent` with `Cause(category=user_signal, signal=…, signal_name=…)`. Stale-running recovery on next `start_run` synthesises a `RunFailedEvent(cause.category=unknown, cause.message="Process exited without writing a terminus event")` for the stranded prior run before appending the new `RunStartedEvent`. `atexit` hook removes the PID file on clean exit; an existing PID file with `(pid, start_time)` not matching any live process is overwritten silently on next `run_started` (after stale-running recovery has appended its synthesised event).
...
- Stale-running recovery: write an events log ending with `run_started` and no PID file (or PID file pointing to a dead process). Run `start_run`, assert a synthesised `run_failed` (`cause.category: unknown`, `cause.message: "Process exited without writing a terminus event"`) was appended before the new `run_started`.
...
**Status: not shipped — design draft 2026-04-25.** Subset of Phase 4a's full event log brought forward because the desktop needs run-level outcome data now, AND (per 2026-04-25 single-source pivot) this file is the canonical home for that data — it's not duplicated to the manifest.
