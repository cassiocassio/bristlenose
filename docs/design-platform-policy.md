...
- _2026-09-25_ — the **test** floor came down from 26.1 to 15.0, closing the known gap in
  Pillar 3. Proven before committing: the Swift suite builds and runs on a macOS 15.7.3 guest
  (Xcode 26.3) and a 26.6.2 guest (Xcode 27.0), and its first run on 15 found a sidebar
  defect no 26/27 machine could show. `check-deployment-floors.sh` now pins both floors at
  15.0. Guests: `design-test-environments.md` §3.6.
...
**Stay inside the band, too.** "Stay on stable" is the don't-get-ahead half; this is its counterweight, because the floor is bounded on *both* sides. The boundary is measurable rather than a matter of taste: what gates a CPython (or a Node) is the set of dependencies carrying **compiled** extensions — pure-Python packages install anywhere and never constrain us. Below the band, current releases of those packages have no wheels for us and we sit on old versions indefinitely; above it, they haven't caught up yet. [`scripts/check-python-band.py`](../scripts/check-python-band.py) computes it from the venv that ships; run it at the quarterly review rather than arguing it. Measured 3 Sep 2026: band **3.12–3.14**, ship target 3.12, inside. Don't carry reputations instead of measurements — "numba and llvmlite lag the newest CPython" was repeated for months after it stopped being true.

**The floor is an engineering number, not a reach number.** `requires-python` is not a decision about which users can install Bristlenose. Homebrew pins `python@3.12`, the Snap stages `python3.12-minimal`, Copr builds on 3.14, and the macOS sidecar bundles its own CPython in `Python.framework` — the packaging exists precisely so users never have to care which Python they have. Set the floor to keep the dependency set coherent, current and secure; don't set it to chase the oldest interpreter someone might still have. The caveat that keeps this honest: **Windows has no bundling channel at all**, so `pip` / `pipx` / `uv` is its only route.

**Being current is not the same as being patchable.** Sitting inside the band buys nothing if one upstream cap pins a vulnerable transitive. On 3 Sep 2026 `presidio-anonymizer` capped `cryptography<49.0.0` **on its own latest release**, holding three CVEs open in a library that ships inside the Mac bundle — and no amount of interpreter movement touched it. A cap that blocks a security fix outranks any version-currency argument; record it as a `(reason, release-predicate)` row in the Held register so the next review checks a predicate instead of re-deriving the problem.
...
Current deployment target — **measured 3 Sep 2026 via `xcodebuild`, correcting a long-standing error in this line.** The app ships **15.0 (Sequoia)** on *all four* schemes (Bristlenose, Dev Sidecar, External Server, default), both configurations. `BristlenoseTests` is also at **15.0** since 25 Sep 2026. It sat at 26.1 before that, not for "debug/feature schemes" and not for Apple Intelligence: the value arrived incidentally in `cce34d2a` when the test target was created and Xcode defaulted it to the then-current SDK, and **no test used a macOS 26 API**. There is no Foundation Models code in the app at all. Pinned by `desktop/scripts/check-deployment-floors.sh`, which `build-all.sh` runs as step 1c on every real build (wired 12 Sep 2026 — until then the script existed and nothing invoked it). Rationale for the Sequoia floor: [`docs/design-decisions.md`](design-decisions.md) — Sequoia is n-1 by launch, avoids SwiftUI contortion for older APIs.
...
- ~~**Known gap, pinned rather than fixed:** the test target's floor (26.1) sits *above* the app's (15.0).~~ **Closed 25 Sep 2026** — lowered to 15.0 after the suite was proven on macOS 15 and 26 guests (`design-test-environments.md` §3.6). The gate now asserts the two floors are equal.
...
3. **macOS deployment target review.** The test floor matches the app's (15.0,
   since 25 Sep 2026); has the suite been run on a 15 guest this quarter?
