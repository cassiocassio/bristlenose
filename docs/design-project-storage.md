...
trued-against: HEAD@main on 2026-09-12 (§1 row 1, §3 anchors and copy, §5 — the session pass, after the §3 brctl correction stamped by 895fe8a6)
---
...
- _2026-09-12 (session pass)_ — trued up: §1's first YES row still carried the retracted `EDEADLK`
  rationale and the `NSFileCoordinator` remedy §3 struck on 4 Sep — rewritten; three sets of §3
  line anchors (`audio.py`, `ProjectFolderWatcher.swift`, `ProjectIndex.swift`) had rotted, the
  last for the second time in 25 days, and are now symbols with a dated line hint; the "is
  Dropbox running?" copy the mockup's §G rules out replaced by the settled "Still fetching";
  `percentDownloaded` clarified against the SDK headers; the 29 Jul reproduction gained a
  post-script (the pre-check landed the same day, `b9325004`); scope wording harmonised to
  "OneDrive"; §5 gained Box. Anchors: `fs.py`, `docs/mockups/cloud-fetch-states.html` §G,
  Foundation `NSURL.h`.
...
| | Item | Why |
|---|---|---|
| **YES** | Read cloud-backed files correctly | A dataless read blocks until the bytes arrive under this app's policy (ON); the fix is detect-then-bounded-materialise (§3, shipped in `fs.py`), not `NSFileCoordinator` — that remedy, and the `EDEADLK` rationale this row carried until 12 Sep 2026, were retracted in §3 on 4 Sep |
...
Row shows a red ✗ and `ffprobe timed out after 30s probing <file>` — which a researcher reads as **"my video is broken"** when it means **"my file was still downloading."** Opposite remedies. Two sites in `bristlenose/utils/audio.py`: the probe's `except (TimeoutExpired, FileNotFoundError, OSError)` (~:187 on 12 Sep 2026) catches and warns; the extractor's `except subprocess.TimeoutExpired` (~:327) raises `AudioToolError` and kills the run. _(Anchored by symbol from 12 Sep 2026 — the `:48`/`:141` numbers had rotted.)_
...
**Progress is not observable, and that is permanent.** Dropbox stages the download and swaps the file in atomically: measured at ~50% on Finder's pie, `st_blocks` was still **0** and the file still flagged `dataless`. A POSIX-derived ring would sit at zero then snap to 100. iCloud's `percentDownloaded` is deprecated (the URL resource key `NSURLUbiquitousItemPercentDownloadedKey`, since 10.8; the `NSMetadataItem` key `NSMetadataUbiquitousItemPercentDownloadedKey` is not deprecated and is `design-cloud-wait-label.md` §5's one open probe — SDK headers, 12 Sep 2026); `NSFileProviderManager.globalProgress` is per-provider aggregate (and would attribute the researcher's unrelated syncing to BN's fetch). So the wait must be **indeterminate** — that is the honest shape, not a compromise. Corollary: BN also cannot distinguish a healthy fetch from a stalled one, so the only legitimate wall-clock number is a soft *hint* after several minutes — the settled copy is "Still fetching from Dropbox" (`cloud-fetch-states.html` §G rules out "is Dropbox running?" as sending the researcher to check the thing most likely fine) — and it is a hint, not a failure, until the sidecar's 30-minute bound (`MATERIALISE_TIMEOUT_SECONDS`, `fs.py`) does raise one.

**`.inCloud` is unreachable in the normal case.** [`ProjectAvailability.swift:141`](../desktop/Bristlenose/Bristlenose/ProjectAvailability.swift:141) short-circuits on `fileExists(atPath: path)` — the project *folder*. With every file in the folder evicted, `os.path.exists()` is still true and `listdir` still returns entries, so availability resolves to `.ready` and step 5 never runs. The cloud glyph therefore cannot appear for evicted media in an existing folder, at any free-space level. Meanwhile `ProjectFolderWatcher.swift` (`isCloudEvicted`, defined ~:474 and called ~:420 on 12 Sep 2026; the `:228` anchor had rotted) *already* calls `isCloudEvicted` on every non-resident file and discards the result with `continue` — the count is computed and thrown away. Turning that into `evictedFiles` gives whole-vs-partial for free.

**~~The provider classifier is an allowlist of three.~~ Fixed — it is structural now.** _Trued 18 Aug 2026._ The finding was right and the prescription was taken verbatim: `ProjectIndex.cloudProviderLabel` (~:984, the `/Library/CloudStorage/` match ~:1000 on 12 Sep 2026) matches `/Library/CloudStorage/`, takes the first path component and splits it on `-`, so **Box and Google Drive work without being enumerated** — the code comment says so in this doc's own words. `providerNames` (~:1017) now maps only the three whose on-disk container name differs from the product name a researcher would recognise (`GoogleDrive`, `ProtonDrive`, `pCloudDrive`); an unknown provider keeps its own name rather than being flattened to a generic "Cloud".

_(The original anchor in this bullet, `ProjectIndex.swift:650`, had rotted — it now lands on `setFolderCollapsed`. Worth noting as a mechanism rather than a typo: a line-number anchor into a file under active work is a citation with a short half-life, and it fails **silently** because the link still resolves. It rotted again by 12 Sep 2026 — every `ProjectIndex.swift` number in this bullet — so the anchors are now symbols with the line as a dated hint.)_

**The fix carried a win this bullet never predicted, and it is the more serious of the two.** The old three-prefix allowlist had no way to catch a *home* folder that Desktop & Documents sync had taken over — `~/Documents` has no distinguishing path shape at all — so it reported **Bristlenose's own default project location as "On this Mac" on any Mac with iCloud Drive turned on** (the `isUbiquitous` fall-through, ~:1012 and ~:1029 on 12 Sep 2026). The stated symptom was two named third-party providers going unclassified; the unstated one was the default case being wrong for a large fraction of users. Resolved by falling through to the File Provider flag (`isUbiquitous`) once the path-shape tests miss.
...
**Scope of any such messaging — decided by the maintainer, 4 Sep 2026: iCloud Drive, Google Drive and OneDrive only, and nothing clever.** (SharePoint document libraries mount through the OneDrive client and resolve as OneDrive in `cloud_provider_for`.) Those are the stores that pair with the researcher's meeting platforms (iCloud for the personal Mac; Google Drive with Meet; OneDrive with Teams). Dropbox, Box, pCloud, Proton, a NAS: no custom materialisation messaging. Two things this does *not* change. The Dropbox examples in this section were the **measurement test bed**, not a scope statement — the findings hold, the messaging scope is narrower. And §5's rule that Bristlenose must *read* any File Provider correctly stays **unconditional**; this scopes the label, never the correctness. The full write-up — scope, policy, what the OS exposes per store, and what a pick-up builds — is `design-cloud-wait-label.md`; propose the label there, not here.
...
**What already works and should not be disturbed:** a run against a cloud folder with resident files is completely normal; the failure surfaced on the row *and* in the detail pane; the context-menu retry recovered cleanly with no corrupted state. The defect is a missing pre-check and a wrong label, not architecture. _Post-script, 12 Sep 2026: the pre-check landed the day this was written (`b9325004`, 29 Jul 2026 — `ensure_materialised` in `fs.py`, wired at the probe); the label did not. The reproduction above is kept as the measurement._
