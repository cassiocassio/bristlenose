...
**Drag *out of* and *between* folders — NOT modelled by this table, and currently broken (18 Jun 2026).** This table is keyed on the *kind of thing dropped* and the *target*; it does not model where a dragged project *starts*. A project already *inside* a folder can't be dragged *out* to top level or *into another folder* by drag — only the Move-To menu does those (`moveProject(toFolder:)`, `nil` = root). It's structural, not flakiness: the sidebar is per-container `.onMove` reorder islands + one per-folder drop target, so a drag can't cross a folder boundary, and there's no internal-move drop target at root. This is the gap we keep re-deriving — full forensic inventory + fix: the **Sidebar Apocalypse** doc, [`private/handoffs/sidebar-apocalypse.md`](private/handoffs/sidebar-apocalypse.md) § FORENSIC. POST-TF, not a blocker.
