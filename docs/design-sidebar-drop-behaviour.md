...
**Tracked ≠ analysed (refinement, 7 Jun 2026).** "Tracked" is detected by a `projects.json` path match alone — note the "possibly cached pipeline output" above. A tracked entry can have *no* `bristlenose-output/` marker: the first run was interrupted before output was written (e.g. the app was quit to install a new build), or the project was added and never run. Such a **tracked-but-unanalysed** folder is substantively a Source folder, and a re-drop must honour drag-to-analyse — kick off the run, don't dead-end on the duplicate gesture. Only a tracked *and analysed* folder (`LocateFlow.folderLooksAnalysed` true) is the navigation gesture in the TL;DR table below. The action that splits these two lives in `DroppedFolderState.classify(isTracked:folderLooksAnalysed:)` and is wired into `processDroppedURLs`; a full V1 `DropEvaluator` should preserve the split. This is why the "Tracked project → empty sidebar" row assumes analysed output exists — when it doesn't, the folder takes the Source-folder action (analyse) instead.
...
| Tracked project | sidebarFolder | green plus | Move into the project-sidebar-folder _(internal-drag via `ProjectDragID`/`SidebarDrop`; **flaky** — `.dropDestination`-in-`List` is intermittent on macOS, 18 Jun 2026. NB: this row is drag *into* a folder; dragging a project *out of* / *between* folders does NOT work — see note below the table)_. |
| Multiple nested projects | empty sidebar | no-entry | V1 refuses _(V2 = bulk import)_. |
| Other | any | no-entry | Refuse. |

**Drag *out of* and *between* folders — NOT modelled by this table, and RESOLVED 1 Aug 2026 (`bbda9a6a`).** This table is keyed on the *kind of thing dropped* and the *target*; it does not model where a dragged project *starts*, which is why the case sat outside it. On the AppKit outline all four now work through one unified insertion model — out of a folder, between folders, into a folder, and reorder within a scope (`DropRouting` → `ProjectIndex.apply(_:)`); see [`design-desktop-sidebar-appkit.md`](design-desktop-sidebar-appkit.md) §3.3. _Superseded (18 Jun 2026):_ ~~a project already inside a folder can't be dragged out to top level or into another folder by drag — only the Move-To menu does those. It's structural, not flakiness: the sidebar is per-container `.onMove` reorder islands + one per-folder drop target, so a drag can't cross a folder boundary, and there's no internal-move drop target at root.~~ That analysis described the SwiftUI `List` path, which is not the sidebar the app runs. This was the gap we kept re-deriving — full forensic inventory + fix: the **Sidebar Apocalypse** doc, [`private/handoffs/sidebar-apocalypse.md`](private/handoffs/sidebar-apocalypse.md) § FORENSIC. POST-TF, not a blocker.
