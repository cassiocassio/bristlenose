...
**(b) A cross-language boundary earns a decision-ledger, not just tests.** Observability is a first-class boundary concern, on a different axis from coverage. The cohort *did* catch the 404 — detection worked exactly as "the cohort is the integration test" predicts. The expense was *diagnosis*: the distance from symptom (a 404, mislabelled "transcription failed") to cause (a Typer default three layers up, across a process and a language boundary) was paid in tokens because the seam wasn't legible. The fix that "cracked it in seconds" was a *ledger*, not a test: `config.py`'s `_LAST_RESOLUTION_TRACE` ([config.py:183](../bristlenose/config.py)) records the provider+model pair **with its `source=`** at every step — and `provider=anthropic (source=cli-override)` when nobody typed `--llm` is the entire bug on one line. **The cheapest insurance against the next cross-boundary tail-chase is a "who won and why" trace at the resolution point, not another green unit.** Detection is the cohort's job; *diagnosis legibility* is the ledger's, and the doctrine under-prices it.

*Extended across the seam (8 Jun 2026):* the ledger started Python-side only — it began at `load_settings` and carried neither a Swift breadcrumb nor an explicit CLI-layer "why is `--llm` in the overrides" entry. It now carries both, so the trace reads top-to-bottom across the boundary: `step=host-defaults` (Swift origin — which provider/model/key the desktop app injected) → `step=cli-args` (the `run`/`analyze` command's forward-or-not decision, the actor that spoke uninvited in the 404) → the Python `step=0-inputs … step=4-final` chain. An initial pass *declined* the extension, arguing `step=2`'s `provider=… (source=cli-override)` already names both sources on one line. That under-counted the recurrence trigger: the Swift injection had been wrong before (the orphan `else if` that `_guard_orphan_desktop_model` exists to catch) *and* produced the 8 Jun 404 — two cross-seam incidents, so the "earn it via a named, recurring diagnosis" bar this doc applies to an XCUITest *target* was already met for a *write-once trace line* that costs almost nothing. Two design notes worth keeping: (1) the two halves use deliberately different transports — the CLI entry is a drain-once **queue** (`_PENDING_RESOLUTION_NOTES`, cleared after one read: the CLI speaks once per invocation), while the host-defaults entry is a re-read-every-call **env var** (`_BRISTLENOSE_HOST_RESOLUTION_TRACE`, never cleared: a long-lived desktop spawn's preferred provider is a standing property, so the run, then autocode, then a re-run all carry the same origin). That queue-vs-idempotent split *is* the CLI-starts-fresh-each-time vs desktop-persists-across-runs distinction, encoded in the mechanism. (2) The Python read-site is `load_settings` (the universal chokepoint every consumer passes through — run, analyze, autocode, analysis), not the run hook, so serve-side LLM consumers get the same cross-seam legibility. Security rider: the host trace emits `key=present/absent/keyless` only, never the key value — these lines ride into `bristlenose.log` and are `ps -E`-visible. See `_LAST_RESOLUTION_TRACE` ([config.py](../bristlenose/config.py)) and `BristlenoseShared.hostResolutionTrace` ([BristlenoseShared.swift](../desktop/Bristlenose/Bristlenose/BristlenoseShared.swift)).
