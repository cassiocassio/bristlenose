...
**Illustration treatment:** each takes a per-kind natural height as a *cap* (`illustrationNaturalHeight`) and self-scales below it, so the φ-geometry never reflows. Content starts at **x = 0** — the Swift frame owns the cell inset (16pt content margin, 8pt vertical rhythm); an illustration adds no horizontal padding of its own (14 Aug 2026 rule; the four older webviews still owe this pass). CTA labels are per-tool and this table is canonical for them — keep it in step with `WelcomeHomeView.swift`'s `studyTools` `linkLabel`.
