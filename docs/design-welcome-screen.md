...
previous-trued: 2026-08-20
trued-against: HEAD@main on 2026-09-20 — §Cell 1 withholding reason only; the rest of the doc carries its 2026-08-20 verdict
---
...
- _2026-09-20_ — **§Cell 1's Redact-PII withholding reason was half false.** It gave two grounds — capability absent from the binary, and no desktop control — and the first stopped being true on 12 Sep 2026 when presidio and spaCy were bundled (`bristlenose-sidecar.spec:248`, commit "bundle presidio and spaCy; the weights still download"). Only `en_core_web_lg` is excluded now, and it is data. **The decision is unchanged** and rests on the surviving ground: nothing writes `piiEnabled`, so the tool is unreachable from the UI and redaction is CLI-only. The 2 Aug entry below is left as written — it was accurate on its date. Anchors: `desktop/bristlenose-sidecar.spec:248`; `WelcomeHomeView.swift:69-78`; `docs/design-redact-pii.md`.
...
**Copy & i18n — the prose is localised (21 Sep 2026); the illustrations are not, and that is the bigger half.** The deferral this line carried for months ("do not add locale keys or wire i18n yet") is **spent**. Every slot's prose now lives under `desktop.welcome.home.*` in all 21 full locales plus the `zh-Hant-HK` override: 135 strings per locale covering the four cell tags, the 8 study-tool slots, 4 science slots, 25 tips, 3 configured-AI slots, the book-shelf captions, the ingest surtitles and the rotator's VoiceOver value. Slots carry a **key, not a string** (`SlotItem.key`, resolved by `resolve(_:_:)` at render), so reordering a pool costs no translation and a language switch re-reads the pane. CTA arrows stay in code (`cta(_:)`) — a label ending in `…` opens something here and takes no arrow. `tests/test_welcome_locale_keys.py` binds the call sites to the keys **in both directions**: a typo'd key and a stranded translation each fail the suite (it found one of each on its first run).

The geometry rule did the work it was designed for: nothing reflows, and copy was translated **to length** — Latin/Cyrillic at or under +20 % of English, ja/ko/zh-Hant at or under 60 %, with `WelcomeClauseFit`'s markers (` — `, ` – `, `; `) written into every locale's long lines so a short cell gets a shorter *complete* reading. Head-final scripts refuse the cut by design and were written short instead. **Measured in the app, 21 Sep 2026: German fits; Russian is tight but fits.** Those were the two predicted to bind first — German for compound length, Russian for the same plus Cyrillic width — so the translate-to-length discipline held at the sizes that matter. Two findings worth keeping: English's own `books.lazarus` head ("Appraisal theory", 16 chars) is **below the 24-character floor**, so that line falls to the ellipsis in English today; and the tightest ceilings sit on the *shortest* source strings, which is where the Slavic and Finnish cells bind first.

**The illustrations are the documentation, and they are still English.** This is a scope boundary, not a judgement that they matter less — the opposite. The docs site is English and will stay English, so for a researcher in any of the other twenty locales the Welcome illustrations **and the power of their worked examples are the entire product documentation they will ever read in their own language**. They carry ~80 distinct English strings: report chrome (Signal, Concentration, Agreement, Intensity, Participant, Satisfaction, "n hidden"), the seven sentiment names in the fan, five sample quotes, two theme titles with subtitles, two search-word clouds, the MCP transcript, the five sample filenames, and the four book titles — inventoried, with the open questions, in `docs/design-welcome-illustrations-i18n.md`. The second pass should be planned as the **higher-value half of this work**, not as polish. Two things make it cheaper than it looks: the chrome labels already have keys (`common:signals.*`, `enums:sentiment.*` — the settled sentiment taxonomy, never re-translate it), and the sample quotes are *research data*, so they need **writing per locale** as plausible utterances, not translating. Open questions for that pass, both deliberate: whether a book cover shows the local edition's title — decided, and narrower than it looks: only where that edition exists, per the sales-rank research in `design-i18n.md` § Third-party codebooks, which already records that Nielsen was never translated into French, and whether the MCP terminal transcript stays English as a faithful screenshot of what Claude Code actually prints.

**The retired view's locale keys are kept on purpose — do not sweep them as orphans.** _(The "deferred localisation pass" they were raw material for has now run — see above. They remain live or reserved for the reasons below.)_ `desktop.welcome.*` and `desktop.chrome.{welcomeTitle,noProjectSelected,selectProject}` survive in the 21 full locales (22 directories counting the `zh-Hant-HK` override fork — this said 20 until 21 Aug 2026; Catalan landed 14 Aug) even though `WelcomeView.swift` is gone (`zh-Hant-HK` carries only its genuine `welcome.subtitle` override and inherits the rest, per the override-fork rule). Two reasons: they are already-translated raw material for the deferred localisation pass above, and three of them are **verbatim-live copy today** — `welcome.dropFolderTitle` ≡ "Drop a folder" (`WelcomeHomeView.swift:189`), `welcome.dropFolderHint` (`:190`), `welcome.aiPrivacyLink` (`:157`). A grep for unreferenced keys will flag all of them; that grep is wrong here. (`chrome.emptyStateHint` is separately live — sidebar chrome, `ContentView.swift:1810`.)
