...
trued-against: working tree @main on 2026-08-02
---
...
**Pool** — **8 live slots** in the Swift carousel (render order below). Five carry a real drawn looping illustration (`illustration:` → `WelcomeIllustration`, built in `WelcomeIllustrations.swift`); three still carry a **draft PNG screenshot** (light-mode capture, `image:` → `Assets.xcassets` imageset name) and are being converted — **no screenshots long-term** (decided 2 Aug 2026; see `design-welcome-studytools-illustrations.md`). A ninth slot, **Redact PII**, is **withheld** — see below the table.

**Image treatment (the three remaining screenshots; going away):** rendered at **85% of native size** at the shot's own aspect ratio — *not* stretched to the cell width, never upscaled past 85% (shrinks only when the cell is narrower). A faint separator-colour keyline (0.5pt, radius 8) frames each — deliberately reading as *illustration, not live UX*. **8pt** (one macOS grid square) vertical padding top and bottom. `@2x` imagesets (Retina captures). All tunable in `SlotRotator.slotView`. **Illustrations** instead take a per-kind natural height as a *cap* (`illustrationNaturalHeight`) and self-scale below it, so the φ-geometry never reflows. CTA labels are per-tool and this table is canonical for them — keep it in step with `WelcomeHomeView.swift`'s `studyTools` `linkLabel`.
...
| 5 | Video clips | Turn selected quotes into video clips. | `welcome-clips` (PNG) | Export options → | `/docs/export-clips.html` |
| 6 | Send to Miro | Send quotes to a Miro board. | `welcome-miro` (PNG) | Connect to Miro → | `/docs/send-to-miro.html` |
...
| 8 | Ingest | Drop a folder of recordings or transcripts — Bristlenose transcribes, analyses and reports back. | `welcome-ingest` (PNG) | Import options → | `/docs/first-analysis.html` |
