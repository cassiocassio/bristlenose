...
|---|---|---|---|
| **turn** | segment, block, utterance, passage | One unbroken stretch of one speaker's speech in a transcript. **Load-bearing beyond terminology:** a menu item naming a *turn* or a *quote* moves words to a different person; an item naming a *speaker* only relabels. Locales must pin a turn-noun distinct from the quote-noun or the safety rule dies — see `glossary.csv`. | `design-people.md` §B2 |
...
| **lens** | view, page, tab | Bristlenose's word for one of the five report views (Sessions, Quotes, Codebooks, Signals, Project). **Translators: never the optical word** — Apple ships *Lens* as the camera part in all 21 of our locales (*Objektiv*, *objectif*, 鏡頭, measured from the `.loctable` corpus 21 Sep 2026). Render it as the platform's **View** noun with the lens's name (de *die Ansicht „Signale“*, fr *la vue Signaux*, sv *vyn Signaler*), or the bare name in the locale's UI-name quoting (ja 「シグナル」では…, zh-Hant 「訊號」會…). Build on the same noun as `nav.*`, never a fresh coinage — the same rule as **All Sessions** below. `glossary.csv` carries a `Lens` row per locale. | `bristlenose/locales/glossary.csv`; `design-signals-rename.md` §1 |
...
| Canonical | Forbidden | Why | Cross-ref |
|---|---|---|---|
| **moderator** | researcher, interviewer, facilitator, host (when naming the person who ran the session) | The person who ran the session — on the study-conducting side of the participant line, with the objectives and the discussion guide in hand. Every shipped surface already says Moderator. The stored enum value is `researcher` and stays internal-only — renaming it would break the `[RESEARCHER]` transcript round-trip. "Researcher" remains correct for *the tool's user*; the two senses must not be merged. | `design-people.md` §H1 |
| **participant** | interviewee, subject, respondent, user (when naming the person in the session) | **The source of the data — the subject of the study.** That, and only that, is what makes someone a participant. They know what has been shared with them; they respond to questions from the moderator and observers and do not direct the session. **Commercial relationship is irrelevant**: a participant may be paid an incentive, a moderator may be salaried or contracted, an observer may be a volunteer, and the work may sit in an agency, a university or a company — none of it bears on the classification. Carries the `p` prefix; the only speaker kind whose quotes are evidence, and the only kind anonymisation protects. | `design-people.md` §E decision 2 |
| **observer** | note-taker, silent participant, attendee | Present in the session but neither running it nor being studied — often client-side. **Never a data source.** Most say very little and many are silent throughout, never entering the transcript at all; when one does speak they do so as a collaborator of the moderator, with the goals of the project in hand. Carries the `o` prefix. Like a moderator they know the objectives and the guide, so they sit on the study-conducting side of the participant line and **are named in exports** — being an observer says nothing about who employs them. | `design-people.md` §E decision 2 |
| **speaker** | person, voice, channel, diarisation label | A turn-taking identity within one transcript. A *speaker* is scoped to a session; a *person* can hold several speaker codes across a study. Do not use the two interchangeably. | `design-people.md` §E decision 1 |
...
| **display name** | real name, actual name, first name | Editable short name, derived from the full name and overridable. Appears in exports **unless** the researcher turns anonymisation on (off by default) — codes always travel, names travel by choice. A working tool, not an identity. | `design-html-report.md` lines 156–169 |
...
| **All Sessions** | Sessions index, sessions grid, sessions table, sessions list | The user-facing name for the sessions overview (the row at the top of the macOS session-switcher popover that returns to it). Mac convention: the plural noun with "All", named by content — Mail "All Inboxes", Photos "All Photos". "Index" was rejected: it already carries a *statistical* sense in this product (Simpson's diversity index) and the two senses translate differently in every target language. Translators: build on the same noun as the Sessions lens (`nav.sessions`) — de *Alle Interviews*, fr *Tous les entretiens* — never a fresh coinage. "Sessions grid/table" remain internal implementation words only. |
