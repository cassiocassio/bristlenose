...
| 52 | `AboutPanel.tsx` — 80 English strings, and **not an i18n defect** | **The coverage sweep's biggest number was dead code.** 827 lines, zero `t()` call sites, 163 prose strings, ~1,393 words — which priced at 21 locales is ~29,000 words of translation. It is unreachable, and has been for six months. `router.tsx:53` renders `<Navigate to="/report/" replace />` for `path: "about"`, and the island mount it also has (`main.tsx:120`, `#bn-about-root`) is emitted by no HTML in the tree. Its only live caller is its own 257-line test file, which is **exactly** the tell `CLAUDE.md` records: *an exported symbol whose only remaining callers are tests*. **How it got here:** `a8ed2313` (18 Mar 2026) turned the route into a redirect because the help modal was going to absorb it; `3f49d170` then retired the help modal and sent its content to the website docs, deleting 129 `common.help.` keys. The component was never deleted with them. **The cost is not hypothetical:** the Signals rename (`2aba263b`, 20 Sep 2026) updated this file — maintenance spent on a surface nobody can reach, two days before this sweep found it. NB the *static* Jinja renderer emits its own separate about panel (`s12_render/report.py:437`), which is a different surface and a sealed byproduct. | 22 Sep 2026 | **struck 22 Sep 2026 — deleted.** 1,205 lines: `AboutPanel.tsx` (827), its test (266), and `AboutDeveloper.tsx` (112), which was referenced by **nothing at all** — not even the dead mount. Docs live on the website by decision and one current set is hard enough to keep in sync; an in-app copy of them was the surface retired twice. Not translation: seeding 29,000 words into an unreachable surface would have been the most expensive way this register has ever been wrong |

| 53 | Fixing a locale **value** reddened eight test matchers, and no locale gate can see it | **The straight-quote repair (row 17b) broke the frontend suite, and it was committed that way.** `afdfaccb` moved `codebook.delete{Group,Tag}Title`, `mergeTitle` and `nameTaken` from `"` to each locale's own pair — correct, and the whole point of the row. Eight assertions in `CodebookAuthoringParity.test.tsx` quote those rendered strings back (`/Delete "confusion"/`), so they stopped matching. **Nothing in the i18n toolchain could report it:** `check-locales.py` is key-shaped and the keys never moved; `tsc` cannot type a regex literal; pytest does not render React. The only witness is vitest, and I had not re-run it after the sweep — so the red sat on local `main` for 88 minutes. Note two of the eight were **negative** assertions (`queryByText(...).not.toBeInTheDocument()`), which went on passing for the wrong reason and could no longer fail at all. **The contrast worth keeping:** when the same afternoon's deletion orphaned `settings.configReference.heading`, row 39's gate named it in 0.3s — the mechanism works, and this row is about the gap beside it, not a hole in that one. | 22 Sep 2026 | **struck 22 Sep 2026.** Eight matchers repointed to the typographic pair the UI actually renders (`06c5c4ad`); `CodebookAuthoringParity` 21/21, full suite 1771/1771. **The durable rule — a locale *value* edit is a frontend-suite change, not just a locale change.** `grep -rln '<the old English value>' frontend/src --include='*.test.tsx'` before the sweep, and run vitest after it. Recorded as a gotcha in root `CLAUDE.md` §i18n rather than gated: a mechanical check would have to know which strings a test quotes, which is the assertion itself. |
