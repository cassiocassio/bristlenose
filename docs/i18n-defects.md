...
| 25 | `LocalizedError` conformers — 6 of **7** render English | The hand audit reported *6 of 6, no false positives*, and it was **6 of 7**: `SidecarResolveError` declares `: Error, Equatable, LocalizedError`, and the grep looked for `: LocalizedError`. `tests/test_localized_error_coverage.py` found it on its first run. That one is genuinely English-by-decision (every case is about `_BRISTLENOSE_DEV_*` env vars), so the count of real debt is unchanged — but *the audit could not have known that*, which is the point: a hand grep's corpus is its own blind spot, and the gate's is not. The remaining five are held at a ratchet rather than swept, because each is new copy in 21 locales. | 21 Sep 2026 | open — ratcheted at 5 (`docs/testing/ratchet.json`), classify-or-fail gate live |
