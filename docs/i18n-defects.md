...
| 20 | `bristlenose/locales/glossary.csv` — the `Codebook` rows for zh-Hant, ru, uk; and five locale strings in tr, ru, uk | **The glossary and the shipped files name the same noun differently, and the glossary is the stale side in three of four.** Counted across each locale's JSON on 20 Sep 2026: zh-Hant ships `編碼簿` ×27 against the glossary's `代碼簿` ×0 — and the glossary's own later `Library` row already says `編碼簿庫`, so a subsequent pass contradicted the `Codebook` row; ru ships `кодировочная книга` ×19 against `Книга кодов` ×0; uk ships `кодова книга` ×10+ against `Книга кодів` ×0. tr is the one case where the glossary is right — `Kod defteri` ×22 against two outliers. **Two separate machine-seeding errors surfaced in the same count:** ru `browseCodebooksButton` / `menu.codes.browseCodebooks` say `кодировщиков`, which means *of coders/encoders*, and uk `autocodeRefusal.alreadyRunning` / `.templateMissing` say `кодувальник`, likewise *coder*. Neither is a codebook. The two new Codes-menu keys took the **body** form in zh-Hant/ru/uk and the **glossary** form in tr, deliberately: using a term with zero occurrences would have made the menu item disagree with the lens heading the researcher just clicked from, which is worse than the drift it would have resolved. | measured 20 Sep 2026 | open — three glossary rows and five locale strings, each a locale-wide sweep rather than a key-level fix |
