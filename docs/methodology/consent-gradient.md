...
The first product surface where this principle ships concretely is the Pipeline view's editorial axes ([design-pipeline-view.md](../design-pipeline-view.md)): a **plural** `recommended` flag (Bristlenose endorses multiple in-bounds production choices and lets the researcher pick what fits their constraints) alongside a singular `default` (what BN runs if you change nothing). The plural axis is the architectural form of "researchers are adults" — Bristlenose makes the trade-offs legible without automating the choice on the user's behalf.
