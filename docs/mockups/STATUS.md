...
**All 165 are classified** as of 20 Sep 2026 — 87 IMPLEMENTED · 34 SANDPIT ·
20 SUPERSEDED · 14 PROPOSED · 6 PARKED · 1 ABANDONED · 3 *unreviewed*. A new mockup
...
| `edit-field-idioms.html` | 20 Sep 2026 | *unreviewed* — inline edit fields, one rule with two treatments and one exemption. Landed with `2593103a` ("an edit field now takes its fill from the ground it sits on") while a concurrent session held this register, so it has a row and not yet a judgement. Whoever owns that change should say which state it is in |
...
| `signals-rename-surfaces.html` | 20 Sep 2026 | PROPOSED 20 Sep 2026 · **IMPLEMENTED** 21 Sep 2026 — the review sheet for the Analysis→Signals rename: every changed UX surface drawn in the shipped design system with the exact new wording highlighted, and the surfaces that deliberately keep "Analysis" shown beside them split by sense (the run vs the interpretation) so the boundary could be confirmed as one group. Its wording is what shipped. Spec: `design-signals-rename.md` |
| `signals-focus-mode.html` | 21 Sep 2026 | PROPOSED 21 Sep 2026 — Focus Mode extended to the Signals lens: real card markup over the baked theme, Focus off beside Focus on, the finding's treatment (0.4 wayfinding vs lit) switchable. The candidate CSS is the proposal layer at the bottom of its `<style>`. Spec: `design-focus-mode-signals.md` |
