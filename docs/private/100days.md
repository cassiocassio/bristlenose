...
- **Status:** correctness slice merged 10 May 2026 (`spa-pipeline-completion-refetch`) — auto-refetch within ~3 s of completion + race-safe import-then-publish. Trust-UX polish committed 10 May 2026 on `pipeline-completion-trust-ux` (`9e173e2`, pending merge): NavBar refresh button (hidden when no run), refetch overlay (`inert` + opacity 0.7 + reduced-motion guard), inline post-zero-quotes message, cross-island contract test. Architectural reframe mid-branch: pre-pipeline / no-sessions / failed-run surfaces moved out of SPA scope into a server-rendered failure-page follow-up (`generic-failure-surface` handoff) — same surface for CLI browser and WKWebView
...
- [S3] **Multi-project support** — home screen, project list, create/switch without restart. Milestone 4 in ROADMAP. Design docs: `docs/design-project-sidebar.md` (5-phase plan), `docs/design-multi-project.md` (infra/DB/architecture). Phase 1 prompt: `docs/private/prompts/phase1-project-sidebar.md`. _Scoped 4 May 2026: Phase 1 only (list, create, switch, persistent selection). Instance DB, person linking, cross-project search, archive lifecycle deferred to beta. XL → M._
...
- [S3] **React SPA misses pipeline-completion signal** — [stage-2-prereq] 🔴 folded from 7 May walk. Drop files → pipeline runs cleanly → toolbar pill vanishes → sidebar row says "Analysed 1 min ago" → project content shows 0 sessions / 0 quotes. Reload button does not recover. Workaround: click to a different project and back triggers a remount + refetch. Diagnosis: project-content components don't subscribe to the same completion event the pill listens to. **Ship criteria:** run-completion event triggers SPA refetch (or invalidates the project's React Query cache); project view reflects analysed state without tab-away-and-back workaround; Reload button works. _Correctness slice merged 10 May 2026 (`spa-pipeline-completion-refetch`): `/api/projects/{id}/last-run` + `LastRunStore` polling + race-safe import-then-publish; auto-refetch within ~3 s. Trust-UX polish committed 10 May on `pipeline-completion-trust-ux` (`9e173e2`, pending merge): NavBar refresh button (hidden when no run), accessible refetch overlay (`inert` + opacity 0.7), inline post-zero-quotes message, cross-island contract test. Pre-pipeline / no-sessions / failed-run surfaces moved out of SPA scope into a server-rendered follow-up (`generic-failure-surface` handoff)._ Linked from Critical Path → Next
...
- [S3] **brew first-run + IKEA-call cluster** — [stage-2-prereq] 🔴 surfaced 9 May 2026 from friendly-CTO's brew install + IKEA dogfood call. 35 numbered findings decomposed into 7 chunks: A1 brew formula `[serve]` extras + man page (tap commit, S0); A2 doctor `check_serve_deps()` hard error + anaconda-shadowing warn + README Python 3.10+ caveat (S2); A3 CLI honest output — pipeline-errored summary surfaces failure cause from events log, no static-render surface, plurals helper, hide `--static`/`render` from user-facing help (S2); A4 ~~failed/empty stages don't cache as complete~~ (✓ shipped 12 May 2026 on `a4-stage-cache-honesty`) + rerun-dedup investigation #17 (deferred to A5 if scope expands) + HF-Hub warning suppression (✓ landed via separate chip merge `eb2a63e`) (S0); B1 long-audio quality — Whisper hallucination #14 + untranscribed gaps #15 + diarization-collapse #16 (S0/S1); C1 auto-run default codebook on first analysis #35 (S1, headline first-impression fix); C2 14 UX papercuts from IKEA call #18-#34 (S3, split-if-bloats). Plan + per-branch handoffs at `docs/private/plans/2026-05-meta-plan.md`, coherence doc at `docs/private/plans/2026-05-cluster-coherence.md`, handoffs at `docs/private/handoffs/[A1-C2]-*.md`. Status: A1 ready to ship tonight; **A2 ✅ shipped 12 May 2026** on `a2-install-doctor-checks` worktree — `check_serve_deps()` hard error wired into `run_all()`, brew/pipx/pip-aware fix message with zsh-glob-safe quoted brackets, README Python 3.10+ + anaconda caveat. Fix 3 (anaconda runtime check) dropped — unreachable under `requires-python = ">=3.10"`; README caveat covers the failure mode. Latent Rich-markup-eating bug fixed at three print sites (was silently dropping `[serve]` from `_install_hint()` and fix messages alike); A3 handoff full implementation spec; B1/C1/C2 skeletons pending pre-flight. Coherence doc maps cross-branch contracts (event schema, stage-success semantics, quote uniqueness, React SPA reachability). Per the friendly-CTO's onboarding-first lens — gate-level work (A-stream + B1) before UX polish (C-stream); execution order is A1 tonight, then A4+B1+A2 in parallel, then A3+C1 sequenced after A4, then C-stream parallel after C1. Cross-references: overlaps with §2 fake-success-feedback audit and §2 Pipeline diagnostic popover (Swift) entries — different bugs, same family
