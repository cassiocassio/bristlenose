...
**Captured for triage (16 May 2026, sidebar-analysing-on-reopen session):**

- **[post-TF] Workspace — real multi-project concurrency.** What's shipped under `tf-multi-project` is project navigation chrome (sidebar holds many, only one runs, switch is a chore). Real multi-project = many projects mounted, N run in parallel, switching is free. Decouple **reader** (one serve process, N projects mounted, path-mapped `/report/{slug}/`, read-only over manifests) from **worker** (one `bristlenose run` subprocess per project, semaphore-capped to `--max-workers`, default 1 CLI / 2 desktop). Manifest + events log stay as the truth surface — no new IPC. CLI gains `bristlenose workspace add/serve/run`; desktop's `ServeManager` becomes `Workspace` (mount-add on drop, URL-change on sidebar click, no respawn). Deletes the cancel-confirmation modal, `ServeManager.switchProject` teardown, single-slot FIFO, and `ProjectBookmarkLease` single-tenancy. One codebase preserves CLI ≡ desktop parity (`docs/design-modularity.md`). Sketch lives in the 16 May 2026 sidebar-analysing-on-reopen session transcript; promote to `docs/design-workspace.md` when post-TF planning starts. **Triage question is when + how, not whether** — multitasking has been a settled UX expectation since the 1989 OS/2 Warp TV ad. The cohort tells us about the analysis surface (quote / theme / tag / signal quality); they don't tell us about engineering questions like concurrency. Naming note: the alpha-era "multi-project" shorthand is fine for cohort calls but a future design doc should distinguish *project navigation chrome* (TF-era, shipped) from *workspace concurrency* (post-TF, this item).
...
- **Session-handoff sentinels** (14 May, `0054fea` + `07506dc`) — `/end-session` sign-off sentinel + `/close-branch` drift check. Workflow tooling, now load-bearing. **Durable-artefact audit extension (18 May 2026, `multi-project-cloud-evicted` branch)** — `/end-session` Phase 2 now runs a Scan A (commit bodies → `docs/design-decisions.md` for decision burials, F49-class) + Scan B (branch diff → `100days.md`/`TODO.md`/`docs/private/plans/*.md`/`CHANGELOG.md` for incidental closures; sibling to the captured `end-session-incidental-closure-detector.md` plan, merged in). Single consolidated prompt with yes / defer / no / `CLAUDE.md` / `<other path>` answers; defer is the third state so future-pickup can distinguish "false match" from "uncertain". Sentinel JSON carries `audit_version: 1`; `/close-branch` Step 3.5 reads it to re-prompt on stale-audit branches. `.claude/audit-log.jsonl` (gitignored) instruments each session for noise-budget tuning after ~10 audits. Motivated by this branch's own plan-pickup mistake: HANDOFF.md proposed `icloud.and.arrow.down` + Download button + ProgressView for cloud-evicted projects, but F49 (`e4037d5`, 15 May) had explicitly rejected exactly that affordance 3 days earlier and the decision lived only in the commit body. Memory: `feedback_handoff_not_a_spec.md`. CLAUDE.md "Branch handoff plan" reframed: handoff is the *starting brief*, not the spec — always cross-check against post-handoff commits on touched files before drafting a plan.
...
- [S3] **Multi-project support** — home screen, project list, create + switch projects. Milestone 4 in ROADMAP. Live plan: `.claude/plans/tf-multi-project.md` (supersedes the 5-phase `design-project-sidebar.md` and `design-multi-project.md` infra plan for TF scope). _Scoped 4 May 2026: Phase 1 only (list, create, switch, persistent selection). Instance DB, person linking, cross-project search, archive lifecycle deferred to beta. XL → M._ **Phase 0 + Phase 1 shipped 14 May 2026** (`206b522` schema lock + `ProjectAvailability` enum; `256e8d0` undo-toast Remove + Locate flow with Spotlight one-shot). **Phase 2 #1/#2/#3 shipped 14 May 2026** (`baf1896` merge of `multi-project-switch`) — sidebar-clicking another project tears down + respawns sidecar with HIG-correct in-flight-run confirm sheet (`ServeManager.switchProject(to:)` + escalating `shutdown(timeout:)`); `Cache-Control: no-store` middleware on `/api/projects/*`; `ProjectBookmarkLease` foundation. **Phase 2 #11 drag-onto-existing shipped 14 May 2026** in the same branch — drag-onto-project adds files via `addFiles`, project-onto-project rejected via non-modal toast, drop hit-test refactored to native per-row `.dropDestination`. **Phase 2 #14 NSFilePresenter folder watcher implemented 15–16 May 2026** on `multi-project-folder-watcher` branch — `ProjectFolderWatcher` + `SourceFilesReader` (SQLite snapshot), session-count + subtitle-delta row layout, Schema A bare-progressive-coarsen date format, semantic state colours (.red failed glyph, .orange cantFind glyph), uniform `.secondary` subtitle text, lease wiring into `ProjectIndex`, F14 pre-run gate, `NewFilesSheet` marked as TF scaffolding (migrates to SPA dashboard post-incremental-processing). Two review passes via `/usual-suspects`, 62 findings triaged. **Acceptance walk closed 16 May (post-merge resumption):** 5 of 8 steps verified ✅ (pre-flight, baseline, missing-file detection, volume-eject cantFind transition, drag-drop folder reattach); 2 deferred for design reasons (F14 gate on legacy CLI-analysed projects); Step 7 re-mount FAILED — reason classification regresses from `.unmountedVolume` to `.moved` on re-mount (chip filed); Step 8 drag-onto-with-new-mp4s not run (fixture invalidated mid-walk; richer drag-drop coverage came via Step 7 follow-up). Three pre-merge mid-walk bug fixes landed: PyInstaller + typeguard `module_collection_mode` for inflect; SQLite WAL via `?immutable=1`; `SubtitleVariant.deltaOnly` for missing-files on legacy projects. Three additional chips filed post-merge: ~~re-mount cantFind regression~~ ✅ verified 18 May 2026 — already covered by `ceb7366` `runRemountLadder`; manual walk on both ExFAT + APFS settled at rung 0 (500ms); stale `.analysing` flag on completed-project drag-drop; 3-button Stay/Cancel modal HIG papercut. AppleDouble crash scope broadened from CLI-only to also-blocks-serve-startup on exFAT volumes; memory `feedback_mac_filesystem_detritus.md` upgraded with filter-never-delete principle. ~~**Phase 3 still open**: #10 cloud-evicted project single state.~~ ✅ shipped 15 May 2026 (`b0ed701` F1 file-level eviction + `e4037d5` F49 outline `icloud` glyph as status-only — explicit download / "Always Keep Local" deferred post-TF as a context-menu story; verified during `multi-project-cloud-evicted` plan-pickup investigation 18 May 2026). **Phase 3 done.** Verification walked on BOTH volumes — ExFAT (NIKON D800 SD card, sidecar actively serving at yank-time) and APFS (Iona USB-C SSD, sidecar NOT serving this project at yank-time); both settled at rung 0 (500ms) on remount; ladder currently over-spec'd (rungs 1+2 never fired). Five sibling chips filed during the walk: (1) volume-label trailing-whitespace + asymmetric notification handling — NIKON's factory FS label is literally `"NIKON D800 "`; `didMountNotification` preserves the whitespace, `didUnmountNotification` strips it; ladder settles today only because stored `volumeName` matches one side; defensive `.trimmingCharacters` is load-bearing not hygiene; researchers' camera SD cards commonly carry whitespaced labels; (2) sidecar exits with code 10 when project folder vanishes WHILE sidecar is actively serving it, doesn't auto-click Retry when volume returns (user: *"it would be even better if it could click Retry for itself when the drive comes back"* — subscribe to availability transition `.cantFind(.unmountedVolume)` → `.ready` for selected project and call `ServeManager.start()` if failed); (3) auth token leaks into "Hide details" pane on sidecar failure (token visible in screenshot-friendly UI; redact `auth-token:` and `api-key:` lines before render — pairs with chip 2 in a single commit); (4) Bristlenose blocks graceful eject of any registered `.ready` project's volume because lease + watcher hold handles on all projects not just the selected one (release lease + pause watcher for background projects, medium-scope post-TF); (5) stale-state dashboard 500 when volume vanishes while project is dormant (not actively served) — sidecar survives but per-project state is stale; manual workaround is project-switch-and-back; fix is to trigger the same project-state refresh on availability transition `.cantFind` → `.ready` for ANY project not just the selected one (pairs with chip 2 — both subscribe to the same transition, handle different branches).
...
- **Release pipeline hardening (60-min bundle)** — surfaced by `release-pipeline-audit` 19 May 2026 (full forensic + reasoning at `docs/private/release-pipeline-audit.md`). Four small actions in one branch (`release-audit-actions` or similar): (1) add `workflow_dispatch:` trigger to `.github/workflows/release.yml` so a stuck release from the v0.15.0 debouncing gotcha can be re-fired without `git push --delete` ceremony; (2) write `scripts/release-status.sh` — one-shot reporter showing PyPI vs source version, last 3 release.yml + ci.yml runs per workflow with per-job status (visibility framed as bandwidth-saver, NOT as nag mechanism — email-on-failure and cron drift checker explicitly skipped per 19 May reframing); (3) one paragraph in `CONTRIBUTING.md` noting snap.yml is manual-only via `workflow_dispatch` and release shipping does not depend on it; (4) cross-reference to `release-status.sh` from CLAUDE.md release section. ~60 min for the bundle
- **`install-test.yml` triage** — workflow has been red for 3 consecutive runs since 19 May 2026 (`macos-pip`, `linux-pipx`, `linux-pip` jobs). Surfaced during release-pipeline-audit; separate from the 60-min hardening bundle above. Not invoked by `release.yml` so does NOT block PyPI publishing, but tests the install path against current PyPI artefacts — may surface real breakage for pipx/pip users. 10-min look at one failed run to decide real-bug vs stale-fixture, then act or park
...
- [S6] **Desktop a11y: drop-affordance VoiceOver on sidebar rows** — `ProjectRow` / `FolderRow` `.dropDestination` modifiers provide no VoiceOver announcement. Add `.accessibilityHint("Drop interview files here to add")` on `ProjectRow` and equivalent on `FolderRow`. Confirm the existing "Move to" Project menu item (`moveSelectedProject` notification, `desktop/CLAUDE.md` Wiring section) is wired and keyboard-reachable as the kbd alternative for the project→folder drag gesture. Demoted from alpha-Must 19 May 2026 — TF cohort doesn't include SR users; alpha-cost not justified.
