...
- [ ] **Bundle-size profile recorded (646 MB at C1 retest, 29 Apr 2026)** — for reference, not a blocker. The C0 "≤ 200 MB target" was a guess made before fully scoping the app's functionality; 646 MB is normal for a real local-first app that ships its own Python runtime + LLM clients + transcription. Top offenders by `du -sh _internal/*` (recorded so future excludes work knows where to look): torch 288 MB (44% of bundle), llvmlite 110 MB (17%), onnxruntime 59 MB (9%), scipy 37 MB, mlx 22 MB, cryptography 11 MB. Real path forward — alpha→GA window (6 months) — is Background Assets / lazy loading per `docs/design-modularity.md` §"trickle to full capability" and AVFoundation replacing bundled FFmpeg, **not** panic-pruning torch right before alpha. Build-time excludes are the cheap-and-cheerful option if there's spare time post-alpha-cut, but skipping them doesn't block anything (App Store cap 4 GB; 646 MB fits comfortably).
...
- [x] ~~**Empty-entitlements re-test.**~~ ✅ **Done 28 Apr 2026 — RED** (`8cfd2ee`). DLV stays. Empirical reason: PyInstaller's bundled `Python.framework/Versions/3.12/Python` carries an internal `_CodeSignature/` seal AMFI reads at dlopen, distinct from the per-binary signatures applied by `sign-sidecar.sh`. Per-Mach-O resigning under one identity is necessary but not sufficient. Path forward (parked, follow-up item below): treat Python.framework as a unit with `codesign --force` on the directory.
...
### Post-build signing verification (parked from Track C)
- [ ] **Verify `Python.framework` is signed as a framework bundle**, not just its inner Mach-Os. Run `codesign -dv <app>/Contents/Resources/bristlenose-sidecar/_internal/Python.framework/Versions/3.12` — output must show a bundle `CodeDirectory`, not just a plain Mach-O signature. Notarisation + MAS validation reject loose-Mach-O-only framework signatures. Ref: TN3127. If `sign-sidecar.sh` doesn't already sign the framework bundle path, add it before the inner-dylib loop. Related: this is the parked path that would let DLV come out of the sidecar entitlements file (per the empty-ents retest 28 Apr 2026, `8cfd2ee`).
- [ ] **Add `xcrun stapler validate -v` to `build-all.sh`** post-notarisation. Confirm the staple ticket covers the sidecar tree, not just the outer `.app`. Belt-and-braces check — notarised-but-not-fully-stapled bundles give Gatekeeper dialogs to offline users. (Only relevant when Developer ID flow is revisited; App Store flow doesn't notarise.)

### Privacy manifest follow-ups
- [x] **Add `PrivacyInfo.xcprivacy` inside the bundled Python tree** ✅ DONE 28 Apr 2026 (Track C C4, `765b111`..`f6c3170`). Single sidecar bundle-root manifest at `bristlenose-sidecar/PrivacyInfo.xcprivacy` — per-package sub-manifests rejected as gold-plating after symbol-sweep triage of 244 Mach-O / 40,460 undefined symbols showed none of Bristlenose's deps are on Apple's named hard-rejection SDK list. Coverage: DDA9.1 (FileTimestamp), E174.1 (DiskSpace), 35F9.1 (SystemBootTime). No UserDefaults — sidecar is pure Python.
- [ ] **Revisit host-app `PrivacyInfo.xcprivacy` when sandbox lands.** Adding file-access entitlements may bring new required-reason API categories into scope; re-audit the manifest then. (Track A trigger.)
- [ ] **Symbol-sweep regression gate** (parked, ~30 min). `desktop/scripts/check-privacy-symbols.sh` re-runs the `nm` sweep against the rebuilt bundle and asserts no new required-reason API symbol family appears that isn't covered by a current manifest. Catches `pip install --upgrade` introducing a new dep capability before App Store rejection does. Wire into `build-all.sh` after step [f]. Worth doing before public beta when upgrade cycles pick up.
...
- [x] ~~**Run the 8-point empty-entitlements retest**~~ ✅ **Done 28 Apr 2026 — RED** (`8cfd2ee`). DLV stays. Reason: Python.framework's nested `_CodeSignature/` seal (full diagnostic in `c3-session-notes.md`). Plan procedure preserved at `~/.claude/plans/c3-empty-ents-retest.md` for the eventual framework-level resign attempt.
- [ ] **Framework-level Python.framework resign (parked follow-up to empty-ents retest)** — `codesign --force --options=runtime --sign "Apple Distribution: …"` on `Python.framework/` as a directory unit, properly resealing the internal `_CodeSignature/`, then retry empty-ents. Non-trivial — Apple's framework versioned-symlink layout is fussy. Not on alpha critical path. Procurement-relevant: removing DLV would be a clean talking point ("zero Hardened Runtime exceptions").
...
- [ ] **BUG-1: `common.cancel` i18n key leaks raw to UI (flagged 21 Apr 2026 during C3 smoke test).** `desktop/Bristlenose/Bristlenose/ContentView.swift:316`: `Button(i18n.t("common.cancel"), role: .cancel) {}`. Key path is wrong — should be `common.buttons.cancel` (per the `<file>.<nested-keys>` convention shown in `I18n.swift:8` docstring). The actual JSON key is at `buttons.cancel` in `bristlenose/locales/en/common.json`. Lookup fails, raw string falls through to label. Visible whenever the user drops a duplicate folder onto the sidebar (Project Already Exists modal). One-character fix. Alpha-tester-visible.
