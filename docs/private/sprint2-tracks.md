...
- **C2 — Sign every binary** (~1 day). Parallel `xargs -P8 codesign --options=runtime`. Ad-hoc for local; Apple Distribution when Track A delivers cert. (Post-archive `check-release-binary.sh` gate already landed in C1.)
