"""Perm-B: baseline with the 5–12 theme-count constraint removed."""
...
from prompts import BASELINE_SYSTEM, PERM_B_USER
...
def run(corpus: list[Quote], llm: LLM | None = None) -> ThemeSet:
    ...
    user = PERM_B_USER.replace("{quotes_json}", quotes_json)
    ...
    return ThemeSet(
        prototype="perm_b",
        label="Perm-B — data-driven theme count",
        themes=themes,
        meta={
            "n_quotes": len(corpus),
            "elapsed_s": sw.lap(),
            "notes": "no 5–12 cap; let data decide",
            **llm.stats(),
        },
    )
