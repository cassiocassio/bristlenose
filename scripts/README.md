...
| [`check-providers-live.py`](check-providers-live.py) | One real call per **shipped** (provider, model) — derived from the registry and the macOS picker, so it cannot drift from what ships. Three questions per model: is the key alive, does the model still exist, does our request shape validate. Preflight's own one-token call runs first, with the model under test, so a preflight-only break (the 4 Sep 2026 ChatGPT `max_tokens` 400) is red here too. The mocked suite cannot ask any of them, and the app's "Online" light validates only the key. Needs keys; pence per run; a release gate via `check-release-ready.sh`. |
