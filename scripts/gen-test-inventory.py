...
def render(data: dict) -> str:
    ...
    for s in data["measurements"]["suites"]:
        ...
        L.append(f"| `{s['suite']}` | {s['kind']} | {size or '—'} | {s.get('basis','—')} | `{s['source']}` |")

    L += ["", "## When they run — and what happens when they fail", ""]
    for wf in st["workflows"]:
        shell = wf["default_shell"]
        L.append(f"### {wf['name']} (`{wf['file']}`)")
        L.append("")
        L.append(f"- **Triggers:** {'; '.join(wf['triggers']) or '—'}")
        if shell:
            L.append(f"- **Default shell:** `{shell}` (pipefail on)")
        elif wf["piped_steps"]:
            L.append("- **Default shell:** GitHub default (`bash -e`, **no pipefail**). "
            ...
            for n in wf["piped_steps"]:
                L.append(f"  - piped: _{n}_")
        else:
            L.append("- **Default shell:** GitHub default (`bash -e`, no pipefail) — no piped steps")
        for j in wf["jobs"]:
            bits = [f"`{j['id']}`"]
            if j["runs_on"]:
            ...
            for s in j["notable_steps"]:
                ...
                L.append(f"    - {s['name']} — {tag}, **{s['colour']}**")
        L.append("")

    L += ["## Build gates (`build-all.sh`, in order)", "",
          "| step | phase | gate | skippable |", "|---|---|---|---|"]
    for g in st["build_gates"]:
        L.append(f"| {g['id']} | {g['phase']} | {g['name']} | {'yes' if g['can_skip'] else 'no'} |")
