...
class TestCheckPii:
    def test_pii_skip_when_disabled(self) -> None:
        """PII check skipped when pii_enabled=False (the default)."""
        settings = _settings()
        result = check_pii(settings)
        assert result.status == CheckStatus.SKIP
        assert "--redact-pii" in result.detail
...
class TestRunPreflight:
    ...
    def test_analyze_checks_api_network_disk(self) -> None:
        """Deliberately NOT `pii`. check_pii probes presidio and a loadable
        spaCy model; `analyze` starts at stage 8 and touches neither, so adding
        it refused a legitimate `analyze <out>/transcripts-cooked` on any
        machine without the 400 MB model."""
        settings = _settings()
        with (
