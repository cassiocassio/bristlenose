...
These tests cover the orchestrator-level mechanism in three layers:
...
Full pipeline-walks (3-session ingest → s05 fail → no-report-on-disk) live
in CLI smoke tests rather than here; the unit layers above plus the
existing ``test_llm_concurrency.py`` coverage prove the wiring without
spinning up audio extraction, manifests, the LLM client, etc.
"""
