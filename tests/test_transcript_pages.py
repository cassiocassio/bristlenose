...
from bristlenose.stages.s12_render.html_helpers import _resolve_speaker_name
...
def _write_raw_transcripts(output_dir: Path) -> None:
    ...
    (raw_dir / "s1.txt").write_text(_RAW_TRANSCRIPT_S1, encoding="utf-8")
...
def test_transcript_page_generated(tmp_path: Path) -> None:
    ...
    assert len(paths) == 1
    assert paths[0].name == "transcript_s1.html"
...
def test_transcript_page_timecodes_with_media(tmp_path: Path) -> None:
    _write_raw_transcripts(tmp_path)
    # video_map is keyed by session_id, but data-participant uses participant_id (first p-code)
    video_map = {"s1": "file:///path/to/interview_01.mp4"}
    render_transcript_pages(
        sessions=[], project_name="Test Project", output_dir=tmp_path,
        video_map=video_map,
    )
    ...
    assert 'data-participant="p1"' in html  # participant_id from first p-code
...
def test_transcript_page_prefers_cooked(tmp_path: Path) -> None:
    ...
    cooked_dir = tmp_path / "transcripts-cooked"
    cooked_dir.mkdir(parents=True, exist_ok=True)
    (cooked_dir / "s1.txt").write_text(
        "# Transcript (cooked): s1\n"
        "# Source: interview_01.mp4\n"
        "# Date: 2026-01-20\n"
        "# Duration: 00:05:00\n\n"
